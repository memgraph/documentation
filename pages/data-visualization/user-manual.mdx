...
The data associated with your Query Collections, including saved queries and
...
- **Browser dependency:** Your access to collections is tied to the browser you
...
- **Private browsing:** Using private browsing mode will not display your
  existing collections, and any collections saved during such a session will be
  lost once the session ends.
- **User Profiles:** Different user profiles or logins within the same browser
  will each have their own set of Query Collections, separate from one another.
