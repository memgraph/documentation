...
remote storage. Remote storage should be a new Memgraph instance
or a new Memgraph database in a multi-tenant environment.
...
non-SSL connections, just omit the optional environment variables:
...
in your enterprise Memgraph instance. For better security, it
is recommended to always define usernames and passwords. However,
if it accepts non-SSL connections with no username or password,
the following command should make Lab successfully connect to
the remote storage:
