...
If you installed Memgraph using the `memgraph-platform` Docker image, like
suggested in the [Getting started](/getting-started) section, **mgconsole**
should have started automatically when you executed the `docker run` command.

If you installed Memgraph using any other Docker image or want to explicitly
start `mgconsole` from the Memgraph Platform image, you need to manually start
...
`mgconsole`,  run the `mgconsole` client from your Docker image.
...
Once you know the `CONTAINER_ID`, you can start mgconsole by running the
following command:
...
Visit the [Download Hub](https://memgraph.com/download#mgconsole) and download `mgconsole`.
