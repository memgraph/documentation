...
{<h3>Download Memgraph RPM Redhat (CentOS) package</h3>}

Depending on the RedHat version you are using, you should download the corresponding CentOS based RPM package. Memgraph provides RPM packages for CentOS 7 and CentOS 9. 
There are no specifically built RedHat packages. 
...
For example, for RedHat 9:

<CodeSnippet code="sudo wget https://download.memgraph.com/memgraph/v2.17.0/centos-9/memgraph-2.17.0_1-1.x86_64.rpm" page="install-memgraph/redhat" os="redhat"/>
...
mind that the linked configuration guide is not written for RedHat, steps will be 
