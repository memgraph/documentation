...
**1.** Start Ubuntu by running the following command from **PowerShell**:

```console
ubuntu2004
```
<Callout type="info">
If you have some other version of Ubuntu, and not Ubuntu 20.04 start thatone instead. You can also run it from the Start menu.
</Callout>

**2.** Install Memgraph database using the latest Memgraph Ubuntu package and by running the
...
```
<Callout type="info">
You can also obtain Memgraph package using the [direct download link](/getting-started/install-memgraph/direct-download-links):
```
...
[Memgraph database](https://memgraph.com/product/) is a native, in-memory graph
