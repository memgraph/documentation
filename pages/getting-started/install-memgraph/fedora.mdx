---
title: Install Memgraph on Fedora from RPM package
description: Install Memgraph from RPM package on Fedora simply and efficiently using our documentation. 
---
...
{<h3 className="custom-header">Download Memgraph RPM Fedora package</h3>}

Downloaded the latest Memgraph RPM Fedora package from the [Memgraph Download
...
For example, for Fedora 42:

<CodeSnippet code="sudo wget https://download.memgraph.com/memgraph/v3.7.0/fedora-42/memgraph-3.7.0_1-1.x86_64.rpm" page="install-memgraph/fedora" os="fedora"/>
...
``` console
sudo dnf install -y ./memgraph-3.7.0_1-1.x86_64.rpm
```
...
mind that the linked configuration guide is not written for Fedora, steps will be 
