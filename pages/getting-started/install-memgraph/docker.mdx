...
similar to `memgraph-2.20.0-docker.tar.gz` (the exact file name varies based on
...
```terminal
docker load -i memgraph-2.20.0-docker.tar.gz
```
...
You can also consider using standalone `mgconsole` image as described in the [CLI section](/getting-started/cli).
