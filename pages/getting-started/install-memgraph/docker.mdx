...
just run the database using the `memgraph` image.
...
the Docker image. When you're developing or if you're new to Memgraph we
recommend using the `memgraph-platform` image. It includes the database and all
the tools you might need to analyze your data, such as command-line interface
...
Docker image that includes the database and the algorithms, or just run the
