...
<Callout type="info">
**Using a coding agent?** Connect it to the Memgraph docs MCP server so it
answers from the current docs. In Claude Code:

```bash
claude mcp add --transport http memgraph-docs https://mcp-docs.memgraph.com
```

For Cursor, VS Code and other clients, see [MCP server for
docs](/ai-ecosystem/mcp#mcp-server-for-docs).
</Callout>
