...
similar to `memgraph-2.17.0-docker.tar.gz` (the exact file name varies based on
...
The downloaded `.tar.gz` file needs to be uncompressed so that you get a file named `memgraph-2.17.0-docker.tar`. Do not unpack the `.tar` archive. This process varies depending on your operating system:

- **macOS/Linux**: Use the terminal with the following command:

    ```terminal
    gunzip memgraph-2.17.0-docker.tar.gz
    ```
...
If you don't already have it, download and install 7-Zip from [7-zip
website](https://www.7-zip.org/) and do the following:

  1. Launch the 7-Zip File Manager from the Start menu.
  
  2. In 7-Zip File Manager, navigate to the folder containing the
     `memgraph-2.17.0-docker.tar.gz` file.
  
  3. Select the `memgraph-2.17.0-docker.tar.gz` file and click on the `Extract`
     button at the top.
  
  ![](/pages/getting-started/install-memgraph/docker/7zip-step-1.png)

  4. Choose the destination directory and click `OK` to start the extraction.

  ![](/pages/getting-started/install-memgraph/docker/7zip-step-2.png)
...
```terminal
docker load -i memgraph-2.17.0-docker.tar
```
...
```terminal
docker run -p 7687:7687 -p 7444:7444 --name memgraph memgraph:2.17.0
```

<Callout>
The `:2.17.0` part in the `docker run` command refers to the tag of the Docker
