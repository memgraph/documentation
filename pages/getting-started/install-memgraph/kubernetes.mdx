...
<Callout type="info"> 
When installing a chart it is a good practice to specify exact version which you want to use. If you use `latest` tag, this could lead you
into problems since on the pod restart, the new image will get pulled.
</Callout>
...
If you are using IngressNginx and doing an upgrade, its attached public IP shouldn't change between upgrades. Tt will change only if added some updates to it doing the release
but then it will be documented.
...
<Callout type="info"> 
K8s uses a concept called 'Storage Object in Use Protection'. It means that the deletion of PVCs will be postponed until its attached pod is deleted. In the same way, the deletion of
PVs will be postponed until its corresponding PVC gets deleted.

If deleting PVCs is stuck, try to remove its finalizers by running:
