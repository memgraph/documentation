...
- `IngressNginx`: Using only one load balancer you can access all instances from outside. [Read more](https://github.com/memgraph/helm-charts/tree/main/charts/memgraph-high-availability/aks#service-type--ingressnginx) on how to use it and set-up on the Azure AKS cloud platform.
...
Read more in the [guide](#example-of-using-memgraph-ha-chart-with-ingressnginx-resource) for installing Memgraph HA with `IngressNginx`. 
...
to any coordinator or data instance by distinguishing bolt ports. The first step is to install Memgraph HA chart:
...
If you get stuck, more info can be found on the [Kubernetes user guide](https://kubernetes.github.io/ingress-nginx/user-guide/exposing-tcp-udp-services/). Save and close the file. 
