...
Since HA Memgraph deploys multiple pods, you can control how they are distributed in the cluster. By default, the Memgraph HA Helm chart has affinity enabled. If you start with affinity disabled, Kubernetes will schedule the pods on any node in the cluster.

The Memgraph HA Helm chart provides the following node affinity options:

- `default`:  Default is set with `memgraph.affinity.enabled` to `true`. Default affinity will try to
 schedule the data pods and coordinator pods on the nodes where there is no
 other pod with the same role. If there is no such node, the pods will still 
 be scheduled on the same node. 
- `unique`: This is set with the `memgraph.affinity.enabled` and
  `memgraph.affinity.unique` to `true`. This will try to deploy the data pods
 and coordinator pods on different nodes in the cluster so that each pod is on
 a unique node. If there are no sufficient nodes, this deployment will fail.
- `parity`: This is set with the `memgraph.affinity.enabled` and
  `memgraph.affinity.parity` to `true`. This will try to deploy the data pods
 and coordinator pods on the same node with maximum one coordinator and one
 data pod on the node. If there are no sufficient nodes to deploy the pods, this
 deployment will fail.
- `nodeSelection`: This is set with the `memgraph.affinity.enabled` and
  `memgraph.affinity.nodeSelection` to `true`. This will try to deploy the data
 pods and coordinator pods on the nodes with specific labels. You can set the
 labels with the  `memgraph.affinity.dataNodeLabelValue` and
  `memgraph.affinity.coordinatorNodeLabelValue` parameters. If all the nodes with 
 labels are occupied by the pods with the same role, the deployment will fail.
