...
<Callout type="info"> During the usage of `nodeSelection` affinity, make sure
that the nodes are properly labeled, the default key for the role label is
`role`, and default values are `data-node` and `coordinator-node`. Labels can be
added to nodes using the `kubectl label nodes <node-name> <key>=<value>`
command.
[Here](https://github.com/memgraph/helm-charts/tree/main/charts/memgraph-high-availability/aks)
is an example of deploy Memgraph HA cluster in AKS  </Callout>
