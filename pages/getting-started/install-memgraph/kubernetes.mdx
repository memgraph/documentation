...
- [Helm chart for Memgraph high availability
  cluster](#memgraph-high-availability-helm-chart)
...
All instances are started as StatefulSet with 1 Pod. The pod has 2 or 3 containers depending on whether the sysctlInitContainer.enabled is used. The **init** container
...
- **IngressNginx**: Using only one load balancer you can access all instances from outside. You can find [here](https://github.com/memgraph/helm-charts/tree/main/charts/memgraph-high-availability/aks#service-type--ingressnginx)
more info about how to use it and set-up on the Azure AKS cloud platform.
...
The guide for installing Memgraph HA with **IngressNginx** is available at the end of this section. 
...
to create the user with the password.
...
Note that the only part which you need to change from this template is the bolt server address. What you will enter here depends on the type of the external network you are using
for accessing instances from outside. 

#### Example of using Memgraph HA chart with 'IngressNginx' resource
...
to any coordinator or data instance by distinguishing bolt ports. The 1st step is to install Memgraph HA chart:
...
Next, install IngressNginx resource using:
...
After installing Memgraph HA Chart and IngressNginx resource, we need to customize a configuration of the newly created Controller and Deployment K8s object. 
Open the ingress-nginx-controller service for editing by running:
...
The only remaining step is to connect Memgraph instances. For that, we need to find out which external IP will a LoadBalancer use. 
