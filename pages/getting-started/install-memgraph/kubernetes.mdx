...
reclaim policy set on your storage class. If you delete `PersistentVolumeClaim` without having `Retain`
reclaim policy, you will lose your data because `PersistentVolume` will get
deleted too. The other option except creating new storage class is to patch your existing
storage class with the `Retain` policy since the default K8s policy is `Delete`. The patching
can be done using the following bash script:
