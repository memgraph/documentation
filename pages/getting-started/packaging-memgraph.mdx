...
This guide will show you how to package Memgraph for one of the Linux distributions supported by 
Memgraph. There are two main ways to package Memgraph:

- Using the `mgbuild.sh` script, which builds Memgraph in a Docker container. (Recommended)
- Manually, using CMake (Advanced)
...
[`build.sh`](/getting-started/build-memgraph-from-source#use-build.sh-script), or 
[`conan` and `cmake`](/getting-started/build-memgraph-from-source#use-conan-and-cmake), one can use 
CMake to build a Memgraph package for a Linux distribution.
