...
This should display the version of Docker installed on your machine. For example:
...
Here's how you can manage the Docker service on Windows using PowerShell:

### Open PowerShell as an administrator

- Right-click the Start button and select "Terminal (Admin)" or search
  for "PowerShell" in the start menu, right-click "Windows PowerShell", and
...
We recommend using the `memgraph-platform` image as it includes all the tools
you might need to analyze your data.
...
<Callout type="info">
Stopping the container isn't the same thing as [deleting it](#containers-and-data-persistence).
</Callout>
...
{<h3>Place yourself in the local directory where you want to copy the file.</h3>}

{<h3>Copy the file from the container to your current directory with the command:</h3>}
...
{<h3>Copy the file from your current directory to the container with the command:</h3>}
