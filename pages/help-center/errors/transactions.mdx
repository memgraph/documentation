...
2. **Cannot get unique access to the storage. Try stopping other parallel queries. Note: Periodic snapshots also hold storage access.**
...
Users can fine-tune the timeout at runtime using:

```cypher
SET DATABASE SETTING 'storage.access_timeout_sec' TO '5';
```

Alternatively, it can be set at startup via the `--storage-access-timeout-sec` flag. Longer timeouts will result in fewer access timeouts, but can lead to worse responsiveness from the database, as workers wait longer before failing.
