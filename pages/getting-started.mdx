...
To manage the containers with Docker Compose, position yourself within that
folder. Most commonly used commands are `docker compose start`, `stop`, `down`
and `restart`. To learn how to manage these containers with Docker, follow the
[First steps with Docker guide](/getting-started/first-steps-with-docker).
</Steps>

The above installation process is a quick start, but make sure you explore
[other installation
options](/getting-started/install-memgraph#installation-options) and choose the
