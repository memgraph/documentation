...
On the Memgraph Platform instance run the Memgraph Lab navigate to the `Export` section. Here, you can export the database dump to a file with a `.cypherl` extension. 
Access the Memgraph MAGE instance using the Memgraph Lab desktop application and navigate to the `Import` section. Use the `.cypherl` dump to import the database state into Memgraph MAGE instance.

</Callout>

For more details on using .cypherl take a look at documentation on [working with .cypherl files](/data-migration/cypherl).
