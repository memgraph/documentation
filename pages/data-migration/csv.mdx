...
- `<csv-location>` is a string of the location to the CSV file.<br/> Without a
  ...
  Docker](/getting-started/install-memgraph/docker#how-to-copy-files-from-and-to-a-Docker-container)
...
speed up data import, if you [create indexes](/fundamentals/indexes.md) on
...
One file contain data we will use to create nodes labeled `:People`, and the
...
- [`people_relationships_nh.csv`](https://public-assets.memgraph.com/import-data/load-csv-cypher/one-type-nodes/no-header/people_relationships.csv)<br/>
  ...
  If you are working with Docker, copy the files from your local directory into the Docker container so Memgraph can access them. 
...
<Tabs items={['CSV files with a header', 'CSV files without a header']}>
  ...
  ### Create indexes
    Creating an index on a property used to connect nodes with relationships, 
    in this case, the `id`, will improve import speed, especially with large datasets:
...
<Steps>
  ...
  - [`people_nodes.csv`](https://public-assets.memgraph.com/import-data/load-csv-cypher/multiple-types-nodes/people_nodes.csv) is used to create nodes labeled `:Person`.<br/> The files contain the following data: 
    ```csv
    id,name,age,city
    ...
    104,Lucy,21,Paris
    ```
- [`restaurants_nodes.csv`](https://public-assets.memgraph.com/import-data/load-csv-cypher/multiple-types-nodes/restaurants_nodes.csv) is used to create nodes labeled `:Resturants`.<br/> The files contain the following data: 
    ```csv
    id,name,menu
...
- [`people_relationships.csv`](https://public-assets.memgraph.com/import-data/load-csv-cypher/multiple-types-nodes/people_relationships.csv) is used to connect people with the `:IS_FRIENDS_WITH` relationship.<br/> The files contain the following data:
    ```csv
    ...
    100,103,2001
    ```
-  [`restaurants_relationships.csv`](https://public-assets.memgraph.com/import-data/load-csv-cypher/multiple-types-nodes/restaurants_relationships.csv) is used to connect people with restaurants using the `:ATE_AT` relationship.<br/> The files contain the following data:
    ```csv
  ...
  the Docker container so Memgraph can access them.
  ...
  ### Set indexes
  Especially with large datasets, it's beneficial to create indexes on a
  property that will be used to connect nodes and relationships, in this case,
  the `id` property of the `:Person` nodes.
