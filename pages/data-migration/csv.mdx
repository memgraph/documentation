...
- `<csv-location>` is a string of the location of the CSV file.<br/> Without a
  ...
  Docker](/getting-started/first-steps-with-docker#copy-files-to-a-docker-container)
...
```cypher
LOAD CSV FROM "https://memgraph.com/docs/data/load-csv-cypher/one-type-nodes/people_nodes_wh.csv" WITH HEADER AS row
...
disk space for large files. The download can be interrupted using `TERMINATE
TRANSACTIONS <tx_id>` without waiting for the full download to complete. To
find the transaction ID, use [`SHOW
TRANSACTIONS`](/fundamentals/transactions#show-transactions).
...
<Steps>
  ...
  <Tabs items={['CSV files with a header', 'CSV files without a header']}>
    ...
    <Tabs.Tab>
  - [`people_nodes_wh.csv`](/data/load-csv-cypher/one-type-nodes/people_nodes_wh.csv)<br/>
    The file contains the following data:
      ...
      104,Lucy,Person
      ```
  - [`people_relationships_wh.csv`](/data/load-csv-cypher/one-type-nodes/people_relationships_wh.csv)<br/>
    The file contains the following data:
      ```plaintext
      id_from,id_to,type
  ...
  <Tabs.Tab>
    - [`people_nodes_nh.csv`](/data/load-csv-cypher/one-type-nodes/people_nodes_nh.csv)<br/>
      The file contains the following data:
        ```plaintext
        100,Daniel
        101,Alex
    ...
    - [`people_relationships_nh.csv`](/data/load-csv-cypher/one-type-nodes/people_relationships_nh.csv)<br/>
      The file contains the following data:
        ```plaintext
        100,101
        100,102
  ...
  the Docker container](/getting-started/first-steps-with-docker#copy-files-to-a-docker-container)
...
- [`people_friendship.csv`](/data/load-csv-cypher/all-data-one-file/people_friendship.csv). The file contains the following data:
...
<Steps>
  ...
  - [`people_nodes.csv`](/data/load-csv-cypher/multiple-types-nodes/people_nodes.csv) is used to create nodes labeled `:Person`.<br/> The file contains the following data:
    ```csv
    id,name,age,city
    ...
    104,Lucy,21,Paris
    ```
- [`restaurants_nodes.csv`](/data/load-csv-cypher/multiple-types-nodes/restaurants_nodes.csv) is used to create nodes labeled `:Restaurants`.<br/> The file contains the following data:
    ```csv
    id,name,menu
...
- [`people_relationships.csv`](/data/load-csv-cypher/multiple-types-nodes/people_relationships.csv) is used to connect people with the `:IS_FRIENDS_WITH` relationship.<br/> The file contains the following data:
    ```csv
    ...
    100,103,2001
    ```
-  [`restaurants_relationships.csv`](/data/load-csv-cypher/multiple-types-nodes/restaurants_relationships.csv) is used to connect people with restaurants using the `:ATE_AT` relationship.<br/> The file contains the following data:
    ```csv
