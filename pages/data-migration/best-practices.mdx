...
**To speed up import, use in-memory analytical storage mode**. 
...
and it is safe to use in-memory analytical storage mode, even in concurrent import scenarios.  
...
transaction to create 10k or 100k edges. Running batch of CREATE statements in a single transaction will
be a magnitude slower than using `UNWIND`, even worse if you are running a single transaction
for each CREATE statement. 
...
queries won’t differ compared to `process_chunk` described above.  But if 
Memgraph is `IN_MEMORY_TRANSACTIONAL` mode, the execution of relationship 
creation queries can run into [`SerializationError`](#how-to-handle-a-serializationerror).
If errors happens, the chunk just need to be retried after certain period of time, here is the example:
...
fire of to minutes quite fast. The `jitter` is added to avoid the retries at the similar time.
...
def process_chunk(query, create_list, max_retries=100, initial_wait_time=0.200, backoff_factor=1.1, jitter_factor=0.1):
    ...
    return time_end - time_start
```
Also, the code above is using and [explicit transaction](../client-libraries/python.mdx#explicit-transactions), which is necessary for handling the conflicts. They 
can also be handled with the [managed transactions](../client-libraries/python.mdx#managed-transactions).
 [Implict (autocomit)](../client-libraries/python.mdx#implicit-transactions) transactions are not recommended for handing the conflicts.
