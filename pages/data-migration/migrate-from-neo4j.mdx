...
- [Latest `memgraph/memgraph-mage` Docker
  image](/getting-started/install-memgraph/docker#run-memgraph-mage-docker-image)
  and [`memgraph/lab` Docker
  image](/getting-started/install-memgraph/docker#run-memgraph-lab-image) (or
  [Memgraph Lab
  Desktop](/data-visualization/install-and-connect#install-memgraph-lab-desktop-application)
  app).
...
1. Run Memgraph 
    ...
    The easiest way to run Memgraph MAGE and Memgraph Lab is by running
    installation scripts that use a pre-prepared `docker-compose.yml` file by
