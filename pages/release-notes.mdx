...
For step-by-step instructions on upgrading standalone instances or
high-availability clusters, see the [Upgrades](/database-management/upgrades)
guide.
</Callout>

## 🚀 Latest release

### Memgraph v3.14.0 - October 21st, 2026

{<h4 className="custom-header">⚠️ Breaking changes</h4>}

- `SHOW TRANSACTIONS` and `SHOW ACTIVE USERS INFO` each gain a `database`
  column with the name of the database the session is using. `SHOW ACTIVE
  USERS INFO` also gains `database marked for deletion`. Clients that parse
  these results by column position need to account for the new fields.
  [#4571](https://github.com/memgraph/memgraph/pull/4571)
- A pattern that names an already-bound variable is now a node test, including
  in plain `MATCH`. After `OPTIONAL MATCH`, `MATCH (b)` no longer keeps a null
  `b` row; it drops it (or errors if `b` is not a node). In `EXISTS` / `COUNT` /
  `COLLECT`, a pattern may also no longer reuse a name from outside the body.
  Rename those variables, or drop a no-op `MATCH` on a bound name.
  [#4707](https://github.com/memgraph/memgraph/pull/4707)
- Comparing lists or maps that hold `null` now returns `null`, the same as
  `null = null`, instead of treating them as equal. Update filters and
  conditions that assumed `[null] = [null]` or `{a: null} = {a: null}` was
  true. `DISTINCT` and grouping still treat those values as the same key.
  [#4794](https://github.com/memgraph/memgraph/pull/4794)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- `AFTER COMMIT` triggers no longer run for transactions that fail to commit
  (for example a STRICT_SYNC write that aborts). They fire only when the
  commit succeeds. On shutdown or `DROP TRIGGER`, any queued trigger work that
  cannot run is logged instead of being discarded silently.
  [#4565](https://github.com/memgraph/memgraph/pull/4565)
- Switching a database to on-disk storage mode is less likely to fail because of
  brief concurrent activity (for example a metrics scrape or a Lab connection
  between queries). Those no longer block the switch immediately. If another
  session still holds the database after a short wait, the switch fails as
  before, because exclusive access is required.
  [#4569](https://github.com/memgraph/memgraph/pull/4569)
- Dropping or renaming a multi-tenant database with a tenant profile now
  persists correctly, including across crashes and replication.
  [#4570](https://github.com/memgraph/memgraph/pull/4570)
- Dropping an index is safe to roll back and to replay after recovery, including
  when drops run alongside other writers or each other.
  [#4741](https://github.com/memgraph/memgraph/pull/4741)
- Bolt stays usable when a query fails after streaming a large result.
  [#4758](https://github.com/memgraph/memgraph/pull/4758)
- Creating or resuming a database at the memory limit no longer fails while
  setting up stream support.
  [#4785](https://github.com/memgraph/memgraph/pull/4785)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Queries that set a timeout do less bookkeeping while they run, because the
  limit is checked against a shared clock instead of a per-query timer.
  [#4662](https://github.com/memgraph/memgraph/pull/4662)
- `KSHORTEST` path expansion is much faster and uses less memory, especially
  for larger `k`, and equality and ordering comparisons between values are
  faster.
  [#4733](https://github.com/memgraph/memgraph/pull/4733)
  [#4760](https://github.com/memgraph/memgraph/pull/4760)
- Commits skip uniqueness and existence constraint checks for writes that
  cannot affect any constraint, so concurrent writers wait less on validation.
  [#4769](https://github.com/memgraph/memgraph/pull/4769)
- Building Memgraph from source can reuse the compiler cache across different
  checkout paths of the same repository.
  [#4795](https://github.com/memgraph/memgraph/pull/4795)

### Lab v3.14.0 - October 21st, 2026

<LabReleasesClient version="3.14.0" />

## Previous releases

### Memgraph v3.13.1 - September 14th, 2026

{<h4 className="custom-header">✨ New features</h4>}

- Added experimental, Enterprise-only FIPS 140-3 support: a new 
  `memgraph/memgraph:<version>-fips` Docker image bundling the validated OpenSSL
  FIPS provider, and a `--fips-mode=true` flag that restricts password hashing 
  to PBKDF2-SHA256 and TLS to 1.2 or higher.
  [#4664](https://github.com/memgraph/memgraph/pull/4664)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fix stale `MemoryRes` value in `SHOW METRICS INFO`, caused by this metric only 
  being updated when metrics were scraped from the HTTP metrics endpoint.
  [#4793](https://github.com/memgraph/memgraph/pull/4793)
- Fixed `SHOW METRICS INFO` failing with "Metrics registry has no database with 
  UUID" on a database that recovered from a snapshot written under a different 
  UUID, such as a replica recovering from its main. 
  [#4805](https://github.com/memgraph/memgraph/pull/4805)

### Lab v3.13.1 - September 11th, 2026

<LabReleasesClient version="3.13.1" />

### Memgraph v3.13.0 - September 9th, 2026

{<h4 className="custom-header">⚠️ Breaking changes</h4>}

- The APOC compatibility names `apoc.convert.fromJsonList` and
  `apoc.convert.toJson` now resolve to the C++ `convert` module
  (`convert.from_json_list` / `convert.to_json`) instead of the Python
  `json_util` implementations. Output can differ — especially for nodes,
  relationships, paths, points, and temporals. Call `json_util.*` explicitly if
...
- The `COORDINATOR` privilege was removed — it never gated any operation. It is
  no longer accepted in `GRANT` / `DENY` / `REVOKE` statements, which now fail
  with a syntax error, and it is no longer reported by `SHOW PRIVILEGES` or
  included in `GRANT ALL PRIVILEGES`. Remove any `GRANT COORDINATOR` /
  `DENY COORDINATOR` / `REVOKE COORDINATOR` statements from provisioning scripts
  and update tooling that compares against the full privilege list. No data
  migration is required: a stale grant in an existing deployment is never
  reported and never checked. The new `COORDINATOR_READ` and `COORDINATOR_WRITE`
  privileges are not a replacement — they are coordinator-only and are rejected
  on data instances.
  [#4399](https://github.com/memgraph/memgraph/pull/4399)
- Connecting to a coordinator with a username and password is no longer always
  accepted. Once `--auth-module-mappings` is set, the enterprise license is
  valid, **and** a role holding `COORDINATOR_WRITE` exists, basic authentication
  is denied in favor of SSO. Until all three hold, the previous passthrough
  behavior is unchanged.
  [#4399](https://github.com/memgraph/memgraph/pull/4399)
- Inside a bare `CALL {}` subquery, a pattern that mentions an outer variable
  without importing it now opens a fresh binding instead of silently reusing
  the caller’s. Queries that relied on the old leak return different results
  with no error. Import explicitly with `CALL (n) {`, `CALL (*) {`, or a
  leading `WITH n`. Some invalid pattern-comprehension shapes that used to
  abort the server now raise a clear query error instead (create+reference in
  the same clause, reusing a bound relationship, variable-length expand from a
  node written in the same query part).
  [#4487](https://github.com/memgraph/memgraph/pull/4487)
- In `path.expand` / `path.expand_config` / `path.subgraph_*` relationship
  filters, `<TYPE>` now means incoming `TYPE` (the old reciprocal-pair reading
  is gone). `*` in a label filter is a wildcard, not a literal label. Unknown
  config keys are errors instead of being ignored. Calls that relied on the
  old readings can return different results or fail without a code change.
  [#4530](https://github.com/memgraph/memgraph/pull/4530)
...
- `TERMINATE TRANSACTIONS` now requires transaction ids to parse in full. Ids
  ...
  `18446744073709551615`; both now raise an error.
  [#4534](https://github.com/memgraph/memgraph/pull/4534)
- `size()`, `substring()`, `left()`, and `right()` now count characters, not
  bytes. For non-ASCII text, lengths and cut positions can change (for example
  `size('中')` is `1`, not `3`). Recheck any stored results that depended on
  the old byte offsets.
  [#4586](https://github.com/memgraph/memgraph/pull/4586)
- A number that starts with `0` is now octal, so `09` and `018` are parse
  errors instead of `9.0` and `18.0`. Drop the leading zero for decimal, or
  keep it for octal (`010` is still `8`).
  [#4588](https://github.com/memgraph/memgraph/pull/4588)
- A `RETURN` inside `EXISTS { ... }` now affects the answer. Queries that
  relied on the body `RETURN` being ignored may return different results after
  upgrading.
  [#4597](https://github.com/memgraph/memgraph/pull/4597)
- Whole-number strings that do not fit in a 64-bit integer now raise an error
...
- `single()` returns `null` when the predicate is `null` for any list element,
  instead of `true`. Queries that treated a null predicate as a match will see
  different results. Wrap with `coalesce(single(...), false)` if those rows
  should be treated as not matching.
  [#4590](https://github.com/memgraph/memgraph/pull/4590)
- `split('', ...)` returns `['']` rather than an empty list, so a non-null
  string always yields at least one field. Update callers that assumed splitting
  an empty string produced no fields.
  [#4591](https://github.com/memgraph/memgraph/pull/4591)
...
- The `cross_database` module (and the deprecated `migrate.*` aliases) now
  requires Memgraph Enterprise. Community deployments that called these
  procedures need an Enterprise license after upgrading.
  [#4726](https://github.com/memgraph/memgraph/pull/4726)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fixed a query planning crash when `extract()` was used inside `WHERE` (for
  example `WHERE 'x' IN extract(v IN n.list | v.field)`). Those queries now
  run as written.
  [#4081](https://github.com/memgraph/memgraph/pull/4081)
- Identifiers may now include currency symbols such as `$` anywhere except as
  the first character (for example `price$`), matching openCypher. Whitespace
  between `$` and a parameter name (for example `$ 1`) is also handled
  correctly. Previously both cases could confuse the query stripper and crash
  the server.
  [#4283](https://github.com/memgraph/memgraph/pull/4283)
  [#4285](https://github.com/memgraph/memgraph/pull/4285)
- Text and vector search now respect fine-grained label and property
  ...
  every indexed property is readable.
  [#4316](https://github.com/memgraph/memgraph/pull/4316)
- Running many one-off queries that differ only in large inlined values (for
  example embeddings) no longer lets the query cache grow without bound and
  exhaust memory. The cache is capped by `--query-ast-cache-max-size` (default
  `1000`; `0` disables it), and each entry also uses less memory.
  [#4334](https://github.com/memgraph/memgraph/pull/4334)
  [#4445](https://github.com/memgraph/memgraph/pull/4445)
- Fixed parallel index scans (`USING PARALLEL EXECUTION`) so a filter that
  evaluates to `null` returns no rows instead of treating the filter as absent
  and returning every matching edge or node.
  [#4406](https://github.com/memgraph/memgraph/pull/4406)
- The `collections.*` functions and procedures now accept `NULL` list and scalar
  arguments instead of failing argument validation. Each function returns a
  defined result (for example `collections.sum(null)` → `null`,
  `collections.sort(null)` → `[]`, `collections.contains(null, x)` → `false`),
  matching the compatibility-layer behavior.
  [#4416](https://github.com/memgraph/memgraph/pull/4416)
- Fixed stale coordinator entries in the HA routing table and a possible
  coordinator crash on `REMOVE COORDINATOR`. Coordinators now deduplicate
  entries when applying Raft logs and snapshots.
  [#4444](https://github.com/memgraph/memgraph/pull/4444)
- Fixed `path.subgraph_all` and `path.subgraph_nodes` so their config options
  work as expected.
  [#4447](https://github.com/memgraph/memgraph/pull/4447)
- Under heavy concurrent load, operations that need exclusive access to the
  database could hang indefinitely while shared readers kept arriving. Those
  exclusive waiters now take priority so the hang no longer happens.
  [#4450](https://github.com/memgraph/memgraph/pull/4450)
- Fixed concurrent `SET STORAGE MODE` handling so mode switches are safe under
  load.
  [#4451](https://github.com/memgraph/memgraph/pull/4451)
- Fixed a crash when `LOAD PARQUET` hit invalid values (for example a time
  outside Memgraph’s supported range). The query now fails with an error
  instead of taking down the server.
  [#4454](https://github.com/memgraph/memgraph/pull/4454)
- Fixed `LOAD PARQUET` silently storing wrong timestamps or durations when
  converting large temporal values from Parquet. Those cases now fail with a
  clear query error instead of corrupting data.
  [#4455](https://github.com/memgraph/memgraph/pull/4455)
- Fixed a rare crash in garbage collection under heavy concurrent writes when
  indexes or unique constraints are defined.
  [#4475](https://github.com/memgraph/memgraph/pull/4475)
- Fixed pattern comprehensions in `WITH … WHERE` / `ORDER BY` so they correlate
  with the bound variable instead of scanning the whole graph; fixed `CALL {}`
  overwriting outer variables it did not import; and fixed related wrong
  results for named paths and `WITH` + `ORDER BY` + `WHERE`.
  [#4487](https://github.com/memgraph/memgraph/pull/4487)
...
- Pattern comprehensions now see writes made earlier in the same query part.
  Reading a list like `[(p)-[:ACTED_IN]->(m) | m]` after a `SET` / `CREATE` no
  longer returns empty or wrong results. Expanding from a node that a preceding
  clause deleted now raises an error instead of counting relationships that no
  longer exist.
  [#4499](https://github.com/memgraph/memgraph/pull/4499)
- Fixed TTL sometimes never running after it was configured or enabled, so
  expired data is cleaned up as scheduled.
  [#4513](https://github.com/memgraph/memgraph/pull/4513)
- `SET COORDINATOR SETTING 'instance_health_check_frequency_sec' TO '0'` is now
  rejected with an invalid-argument error instead of crashing the coordinator
  later (for example on `REGISTER INSTANCE` or restart).
  [#4522](https://github.com/memgraph/memgraph/pull/4522)
...
- Fixed several `path` module issues: traversals honor query timeout /
  `TERMINATE TRANSACTIONS` / shutdown; deep expansions error instead of
  crashing; `path.create` no longer aborts on a missing `rel` key; and filter /
  hop-bound edge cases that produced wrong or empty results.
  [#4530](https://github.com/memgraph/memgraph/pull/4530)
- `TERMINATE TRANSACTIONS` no longer reports `killed: true` for a transaction
  ...
  [#4534](https://github.com/memgraph/memgraph/pull/4534)
- Fixed a crash when writing a property covered by a nested index if the value
  has nothing at the indexed path (for example `CREATE (:T {m: 1})` with an
  index on `:T(m.k)`).
  [#4550](https://github.com/memgraph/memgraph/pull/4550)
- `path.slice` clamps out-of-range `offset` / `length` instead of throwing, and
  `path.slice`, `path.elements`, and `path.combine` accept a null path. A
  negative `length` other than `-1` (to the end) now returns a subpath with no
  relationships rather than the rest of the path.
  [#4552](https://github.com/memgraph/memgraph/pull/4552)
- `*KSHORTEST|k` now returns up to `k` paths for each input row, not for the
  whole query. `|k` can also be an expression from the current row (for example
  `-[*KSHORTEST|n.k]->`). With an inlined edge property map, only matching
  paths count toward `k`.
  [#4559](https://github.com/memgraph/memgraph/pull/4559)
- Negative or empty `*KSHORTEST` depth bounds and a negative `|k` now raise a
  query error instead of returning no rows (or scanning the whole graph first).
  [#4559](https://github.com/memgraph/memgraph/pull/4559)
- `EXISTS { ... }` subquery bodies now behave as expected: bodies that always
  match evaluate to `true` instead of crashing the server, and `EXISTS` in
  `WITH` projections and `ORDER BY` no longer fails with a spurious aliasing
  error.
  [#4560](https://github.com/memgraph/memgraph/pull/4560)
...
- Filters on a variable imported into a scoped `CALL (v) { ... }` subquery now
  use a label-property or `id()` index instead of scanning every node or edge
  once per input row. `MATCH ()-[e:TYPE]->() WHERE id(e) = ...` no longer
  returns an edge of the wrong type.
  [#4561](https://github.com/memgraph/memgraph/pull/4561)
- Variables imported by `CALL (v1, v2) { ... }` and `CALL (*) { ... }` now stay
  bound after a `WITH` inside the subquery. Previously an inner `WITH` that
  omitted them could return wrong rows, overwrite the outer variable, or
  `CREATE`/`MERGE` a new node instead of using the imported one.
  [#4578](https://github.com/memgraph/memgraph/pull/4578)
- Fixed crashes and unbounded index growth in `IN_MEMORY_ANALYTICAL` when
  indexes are enabled. Index entries are now removed before the nodes or edges
  they point to are deleted.
  [#4580](https://github.com/memgraph/memgraph/pull/4580)
- `reverse()` now reverses characters, not bytes, so accented text, Chinese,
  and emoji come back readable instead of as broken UTF-8.
  [#4585](https://github.com/memgraph/memgraph/pull/4585)
- `size()`, `substring()`, `left()`, and `right()` now work on whole
  characters, so cuts no longer split multi-byte text into invalid UTF-8.
  [#4586](https://github.com/memgraph/memgraph/pull/4586)
- Integer literals with a leading zero are no longer treated as floating-point
  values, so they get a proper integer range check instead of a silent
  approximate double.
  [#4588](https://github.com/memgraph/memgraph/pull/4588)
- `toInteger()` now converts whole-number strings exactly. Values near the
  64-bit limits no longer round to a neighbouring integer (for example the
  largest integer no longer comes back as the smallest).
  [#4589](https://github.com/memgraph/memgraph/pull/4589)
- An invalid timezone now raises a clear argument error instead of "An
  unknown exception occurred". UTC offsets such as `+01:00` and `Z` are
  accepted as well as timezone names.
  [#4592](https://github.com/memgraph/memgraph/pull/4592)
- A `RETURN` inside `EXISTS { ... }` is now planned instead of ignored, so
  `DISTINCT`, `SKIP`, `LIMIT`, and aggregation in the body affect the answer.
  Results can change without an error — recheck any query that relied on the
  body `RETURN` being ignored.
  [#4597](https://github.com/memgraph/memgraph/pull/4597)
- Replication and coordination now log cluster state changes at `INFO` instead
  of `TRACE`, so you can see failover, role, and leadership changes without
  enabling per-delta trace logs. The default log level stays `WARNING`.
  [#4599](https://github.com/memgraph/memgraph/pull/4599)
- Query log masking no longer leaks part of a secret when the value contains
  quotes or is missing a closing quote. Those values are now fully redacted in
  logs.
  [#4629](https://github.com/memgraph/memgraph/pull/4629)
- `EXPLAIN` no longer shows a `Limit` above an `EXISTS { ... }` or
  `COUNT { ... }` subquery branch. That operator did not actually limit the
  branch, so the plan looked wrong. Query results are unchanged.
  [#4630](https://github.com/memgraph/memgraph/pull/4630)
- Type constraints now accept zoned datetimes whose timezone is a numeric UTC
  offset, such as `+01:00`, instead of rejecting them as the wrong type.
  [#4635](https://github.com/memgraph/memgraph/pull/4635)
- Near the memory hard limit, Memgraph no longer crashes by refusing allocations
  the runtime needs for itself. Query allocations are still limited as before.
  [#4646](https://github.com/memgraph/memgraph/pull/4646)
- Repeated runs of variable-length expands that return distinct vertices (for
  example with `DISTINCT`) are faster, because those queries no longer replan
  on every execution.
  [#4650](https://github.com/memgraph/memgraph/pull/4650)
- Setting a list property that is also covered by a vector index no longer
  crashes the server when the node already has a typed-list constraint. The
  value is checked like any other list.
  [#4653](https://github.com/memgraph/memgraph/pull/4653)
- List comprehensions whose `WHERE` predicate is `NULL` for an element now
  omit that element and keep the rest of the list, instead of returning
  `NULL` for the whole result. A `WHERE` predicate that is neither a boolean
  nor `NULL` is rejected with an error.
  [#4665](https://github.com/memgraph/memgraph/pull/4665)
- `LOAD PARQUET` and `LOAD JSONL` accept URLs of any length, including long
  signed URLs. A download in progress can be stopped with
  `TERMINATE TRANSACTIONS`, and failures that cannot succeed on retry are
  reported as client errors rather than transient ones.
  [#4672](https://github.com/memgraph/memgraph/pull/4672)
- Lists that hold large integers are compared and sorted correctly. Values that
  differed only in the high bits no longer compared equal.
  [#4712](https://github.com/memgraph/memgraph/pull/4712)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Memgraph is now built with a distro-agnostic toolchain, so DEB and RPM
  packages install on a wider range of Linux distributions that meet
  GLIBC >= 2.31 and Python >= 3.10.
  [#3587](https://github.com/memgraph/memgraph/pull/3587)
- `node2vec` and `node2vec_online` now use a C++ word2vec implementation instead
  of the unmaintained Python `gensim` package. Usage is unchanged, expect
  faster runs.
  [#4319](https://github.com/memgraph/memgraph/pull/4319)
- MAGE is now built and packaged from the same unified CMake / Conan tree as
  Memgraph. Build it with `build.sh --mage on` or `--mage only`; the old
  `mage/setup` Python script is removed. Only affects people building MAGE from
  source — prebuilt packages are unchanged.
  [#4348](https://github.com/memgraph/memgraph/pull/4348)
- `build.sh --target` now accepts several targets in one invocation (for
  example `--target memgraph memgraph__unit`), so you can build multiple
  targets with a single configure step instead of one run per target.
  [#4404](https://github.com/memgraph/memgraph/pull/4404)
- Queries that use query-module functions or `CALL` procedures are now
  plan-cached instead of being re-parsed and re-planned on every run. Module
  reloads stay safe; only module-dependent queries are invalidated, and the
  next execution picks up updated code.
  [#4461](https://github.com/memgraph/memgraph/pull/4461)
  [#4482](https://github.com/memgraph/memgraph/pull/4482)
- In in-memory transactional mode, `CREATE INDEX` / `CREATE CONSTRAINT` no
  longer stall behind garbage collection (and GC no longer waits on those DDL
...
- `SHOW TRIGGERS` and `DROP TRIGGER` no longer take a storage write lock.
  `CREATE TRIGGER` uses a read accessor only. Clients that poll
  `SHOW TRIGGERS` often (for example Memgraph Lab) no longer contend with
  writers for that reason.
  [#4495](https://github.com/memgraph/memgraph/pull/4495)
- Connecting Lab is less expensive under load: its connect-time health-check
  queries no longer open a storage transaction or pin a tenant.
  [#4496](https://github.com/memgraph/memgraph/pull/4496)
- The self-contained Memgraph+MAGE offline installer is quieter (installer noise
  goes to a log file) and is published with the other release artifacts on the
  [download hub](https://memgraph.com/download).
  [#4500](https://github.com/memgraph/memgraph/pull/4500)
- Garbage collection is faster on databases with many indexes: it only visits
  indexes that recent writes could have touched.
  [#4508](https://github.com/memgraph/memgraph/pull/4508)
- Recovery from WAL files is roughly 35% faster. Each WAL file now records its
  ...
  [#4528](https://github.com/memgraph/memgraph/pull/4528)
- Path config filters are stricter: empty or bare filter entries, `*` in a
  relationship filter, and blank sequence steps are rejected instead of
  producing wrong results. On `path.subgraph_nodes` / `path.subgraph_all`,
  `bfs` and `uniqueness` are accepted and ignored (those procedures are always
  breadth-first, one visit per node).
  [#4530](https://github.com/memgraph/memgraph/pull/4530)
- Queries that filter with `IN` on an indexed label and property (for example
  `MATCH (x:L) WHERE x.p IN ["a", "b"]`) now get accurate cardinality estimates,
  so the planner picks better index plans for those filters.
  [#4538](https://github.com/memgraph/memgraph/pull/4538)
- Writing and loading a snapshot no longer fills the operating system's file
  cache, so more memory stays free for queries while it happens. This works
  automatically; `--storage-snapshot-writeback-window-mib` tunes how much of a
  snapshot is cached before being written to disk (`0` restores the old
  behaviour).
  [#4546](https://github.com/memgraph/memgraph/pull/4546)
- Variable-length expands that return distinct vertices (for example with
  `DISTINCT`) no longer walk every edge-unique path. Memgraph can use a
  pruning breadth-first search instead, which is much faster on wide or dense
  graphs.
  [#4558](https://github.com/memgraph/memgraph/pull/4558)
- `CONTAINS`, `STARTS WITH`, and `ENDS WITH` on an indexed label and property
  now use that index instead of scanning every node.
  [#4562](https://github.com/memgraph/memgraph/pull/4562)
- Replicating edge deletions is much faster on replicas. Large deletes that
  were previously quadratic on the replica now scale with the number of edges
  deleted, as they do on MAIN.
  [#4564](https://github.com/memgraph/memgraph/pull/4564)
- `WHERE NOT NOT n.prop = 42` (and similar double negation) now uses a property
  index instead of scanning the label. Results are unchanged.
  [#4594](https://github.com/memgraph/memgraph/pull/4594)
- In `IN_MEMORY_ANALYTICAL`, `CREATE INDEX` and `DROP INDEX` no longer block
  read queries. You can also build several supported indices in parallel, while
  writes are still held off until the build finishes.
  [#4633](https://github.com/memgraph/memgraph/pull/4633)
- Fixed unnecessary work on property writes when fine-grained property
  permissions are not configured.
  [#4639](https://github.com/memgraph/memgraph/pull/4639)
...
- Commits on MAIN with replicas are faster: encoding work for replicas runs in
  parallel with writing the local WAL, so commit time no longer grows as
  steeply with the number of registered replicas.
...
- Indexed `CONTAINS`, `ENDS WITH`, and `REGEX` filters are faster when many
  ...
  [#4647](https://github.com/memgraph/memgraph/pull/4647)
- Aggregations such as `count()`, `sum()`, and `avg()` are faster, especially
  without a grouping key, because they do less work per input row. Results are
  unchanged.
  [#4655](https://github.com/memgraph/memgraph/pull/4655)
- `path.expand_config`, `path.expand`, `path.subgraph_nodes`, and
  `path.subgraph_all` are substantially faster, especially with selective
  filters or a single relationship direction.
  [#4727](https://github.com/memgraph/memgraph/pull/4727)
- Cluster management queries forwarded from a follower coordinator to the leader
  ...
  so a Bolt session can no longer block indefinitely against a leader that is
  reachable but stuck. The same applies to MAIN-to-REPLICA system transaction
  messages (auth, multi-tenancy, parameters), which now time out after 10s and
  defer to system recovery. The unused `EnableWritingOnMainRpc` and its
  Prometheus counters were removed.
  [#4399](https://github.com/memgraph/memgraph/pull/4399)
...
- Added global vertex-property indexes with `CREATE GLOBAL INDEX ON :(prop)`
  (and `DROP GLOBAL INDEX ON :(prop)`). They speed up property lookups across
  all nodes when there is no shared label, for example
  `MATCH (n) WHERE n.uid = '123' RETURN n`.
  [#4353](https://github.com/memgraph/memgraph/pull/4353)
- Added `collections.disjunction`, `collections.subtract`, and
  `collections.duplicates` to the MAGE `collections` module for set-style list
  difference, subtraction, and finding repeated values.
  [#4415](https://github.com/memgraph/memgraph/pull/4415)
- Added `map.get` and `map.merge_list` to the MAGE `map` module. Existing
  helpers such as `map.merge` and `map.set_key` now treat a `null` map as empty
  (and a `null` key as a no-op) instead of failing, and accept nodes or
  relationships by using their property maps.
  [#4417](https://github.com/memgraph/memgraph/pull/4417)
- Added `text.compare_cleaned(text1, text2)` to the MAGE `text` module. It
  compares two strings for equality after keeping only ASCII letters and digits
  (lowercased) and dropping everything else; `NULL` arguments return `false`.
  [#4435](https://github.com/memgraph/memgraph/pull/4435)
- C++ query-module authors can return a value whose type is only known at
  runtime with a single `Result::SetValue(value)` call, instead of switching
  on the value's type to pick a typed overload.
  [#4436](https://github.com/memgraph/memgraph/pull/4436)
- Added `convert.from_json_map`, `convert.from_json_list`, `convert.to_map`, and
  `convert.to_json` for parsing and serializing JSON, with an optional `path`
  selector on the from_* helpers and structured output for graph and temporal
  types from `to_json`.
  [#4443](https://github.com/memgraph/memgraph/pull/4443)
- Added `search.node` and `search.node_all` procedures that find nodes by a
  `{label: property}` map, a comparison operator, and a value. A property list
  per label is OR-ed; `search.node` de-duplicates by node while
  `search.node_all` keeps duplicates. An existing label-property index is used
  automatically when available.
  [#4460](https://github.com/memgraph/memgraph/pull/4460)
- `--coordinator-id=0` is now a valid coordinator id (previously rejected).
  [#4483](https://github.com/memgraph/memgraph/pull/4483)
...
- [SSO authentication on high availability
  coordinators](/clustering/high-availability/coordinator-authentication)
  (Enterprise). Coordinators authenticate OIDC, SAML and Kerberos connections
  against a Raft-replicated set of roles and enforce two privileges,
  `COORDINATOR_READ` and `COORDINATOR_WRITE`. Manage them with `CREATE ROLE`,
  `DROP ROLE`, `SHOW ROLES`, `GRANT` / `REVOKE COORDINATOR_READ |
  COORDINATOR_WRITE | ALL PRIVILEGES`, and `SHOW PRIVILEGES FOR ROLE` on any
  coordinator — follower queries are forwarded to the leader. Basic
  authentication stays open until a `COORDINATOR_WRITE` role exists, so no
...
- `EXISTS { ... }` and `exists((a)-[]->(b))` now work in `WITH` / `RETURN`
  projections, `WITH ... WHERE`, `ORDER BY`, and aggregate arguments, not only
  in a `MATCH` `WHERE`. You can write
  `WITH p, EXISTS { MATCH (p)-[:ACTED_IN]->() } AS hasActed` instead of
  rewriting it as an `OPTIONAL MATCH` plus a count.
  [#4504](https://github.com/memgraph/memgraph/pull/4504)
- Added `TERMINATE TRANSACTIONS "*"`, which terminates every transaction the
  caller is authorized to terminate across all databases, instead of naming ids
  one by one. The caller's own transaction is skipped, transactions it may not
  terminate are omitted from the result, and rows come back ordered by
  ascending transaction id. The wildcard cannot be combined with ids.
  [#4534](https://github.com/memgraph/memgraph/pull/4534)
...
- Added property-value descriptions: attach a human-readable label to a
  specific property value with `SET DESCRIPTION ON PROPERTY … VALUE …`, resolve
  it in queries with `description(property, value)`, and manage them with the
  existing `SHOW` / `DELETE DESCRIPTION` commands.
  [#4526](https://github.com/memgraph/memgraph/pull/4526)
- Added `path.expand_config(start, config)` and extended the path config map
  with node-identity filters, `limit`, `uniqueness` (including `NODE_GLOBAL`),
  and repeating label/relationship sequences. `path.subgraph_nodes` and
  `path.subgraph_all` accept the new filter keys too.
  [#4530](https://github.com/memgraph/memgraph/pull/4530)
- Added `--storage-omit-vector-index-properties-on-return` (also a runtime
  database setting, default `false`). When enabled, properties backed by a
  vector index are omitted from whole nodes and relationships returned to the
  client, which cuts payload size for embeddings. Explicit access such as
  `n.embedding` still returns the value; dumps, WAL, and replication are
  unchanged.
  [#4556](https://github.com/memgraph/memgraph/pull/4556)
- `*KSHORTEST` now accepts a filter lambda, `-[*KSHORTEST (r, n | pred)]->`, so
  you can narrow the search while expanding instead of filtering whole paths
  afterwards.
  [#4559](https://github.com/memgraph/memgraph/pull/4559)
- `EXISTS { ... }` can now be used inside a `CASE` expression, including in the
  condition (for example `CASE WHEN EXISTS { ... } THEN ... END`).
  [#4596](https://github.com/memgraph/memgraph/pull/4596)
- Added `COUNT { ... }`, which counts the rows a subquery body produces (for
  example `RETURN COUNT { MATCH (p)-[:KNOWS]->(f) }`). Use it instead of
  building a list with a pattern comprehension and calling `size()`.
  [#4598](https://github.com/memgraph/memgraph/pull/4598)
- If a write cannot reach every SYNC replica but still commits on MAIN (and
  reachable replicas), the query no longer fails with an exception. A WARNING
  notification is returned instead, so the client result matches what was
  actually committed. Writes that are rolled back everywhere (for example
  STRICT_SYNC) still raise an error as before.
  [#4601](https://github.com/memgraph/memgraph/pull/4601)
- Added `COLLECT { ... }`, which returns a list built from a subquery body's
  single return column, evaluated once per outer row (for example
  `RETURN COLLECT { MATCH (p)-[:KNOWS]->(f) RETURN f.name }`). An empty match
  yields `[]` rather than `null`.
  [#4632](https://github.com/memgraph/memgraph/pull/4632)
- Added
...
<LabReleasesClient version="3.13.0" />
...
- The deprecated high availability startup flags `--instance-down-timeout-sec`
  ...
  use `SET COORDINATOR SETTING 'instance_down_timeout_sec' TO '<value>'` and
  `SET COORDINATOR SETTING 'instance_health_check_frequency_sec' TO '<value>'`
...
- Added property-based access control for nodes and relationships. `GRANT`,
  `DENY`, and `REVOKE` now accept a property list (`{prop, ...}` or `{*}` for all
  properties) alongside existing label and permission rules, so you can
  restrict which properties a user or role can read or write on matching nodes.
  [#4156](https://github.com/memgraph/memgraph/pull/4156)
- Added opt-in lightweight edges via `--storage-light-edge=true` to cut roughly
  24B of memory overhead per relationship when you need properties on edges.
  Edges are pool-allocated instead of stored in a dedicated skip list container,
  while traversals and pattern matching behave as before. Enable
  `--storage-enable-edges-metadata=true` if your workload resolves edges by
  internal ID.
  [#4228](https://github.com/memgraph/memgraph/pull/4228)
- Added `SUSPEND DATABASE` and `RESUME DATABASE` for multi-tenant Enterprise
  deployments. Suspend tears down a tenant's in-memory storage to a durable COLD
  shell to free RAM while keeping its data on disk; resume rebuilds it HOT with
  all data intact. Operations replicate across the cluster like `CREATE`/`DROP
  DATABASE`.
  [#4280](https://github.com/memgraph/memgraph/pull/4280)
...
- Added `text_search.fuzzy_phrase_search` and
  `text_search.fuzzy_phrase_search_edges` for typo-tolerant phrase matching on a
  single property. Queries use `data.<property>:<terms>` with ordered, adjacent
  terms and prefix matching on the last term, so `data.title:big bad wo` can
  match `big bad wolf` or `big bad world` when `fuzzy_distance` allows a few
  edits across the phrase.
  [#4304](https://github.com/memgraph/memgraph/pull/4304)
- Added a cluster-wide coordinator read-only setting synced across coordinators,
  so operators can enable global read-only mode from any coordinator in the
  cluster.
  [#4330](https://github.com/memgraph/memgraph/pull/4330)
- Added the `--storage-allow-recovery-failure` flag (default `false`) so an
  in-memory database that fails durability recovery on startup comes up in a
  `broken` state instead of crashing the whole process. A broken database is
  empty, leaves its on-disk files untouched, and rejects data queries; healthy
  databases keep working. Health is reported via a new `Health` column in `SHOW
  DATABASES` and a `health` row in `SHOW STORAGE INFO`. Recover a broken database
  in place with `RECOVER SNAPSHOT`, restore the data directory from a backup, or
  drop it; on a cluster, a broken replica database self-heals from the main while
  a broken main stays main (no automatic failover).
  [#4284](https://github.com/memgraph/memgraph/pull/4284)
...
- Text search and regex queries return results faster, especially regex searches
  with large match sets. Regex top-N queries now stop once the configured limit
  is reached instead of scanning every match, so broad patterns on large indexes
  complete much sooner.
  [#4286](https://github.com/memgraph/memgraph/pull/4286)
- `MATCH (n) WHERE id(n) IN <list>` and label property `IN <list>` filters now
  use indexed scans when the list is a parameter or expression, instead of
  scanning the full graph. Bulk id lookups and parameterized membership filters
  run much faster on large databases.
  [#4299](https://github.com/memgraph/memgraph/pull/4299)
- Label-property indexes for one or two properties now store entries inline
  instead of allocating a separate heap buffer per entry, reducing memory
  overhead for the most common index configurations.
  [#4308](https://github.com/memgraph/memgraph/pull/4308)
- Reduced query overhead when property-based access control is enabled by
  caching the fine-grained auth checker per session and skipping property
  permission checks when no property-level restrictions apply.
  [#4309](https://github.com/memgraph/memgraph/pull/4309)
- Production Docker images now ship with supply chain attestations and SPDX
  SBOMs, signed with cosign. You can verify image signatures and retrieve the
  SBOM for installed images to support security auditing and compliance
  workflows.
  [#4337](https://github.com/memgraph/memgraph/pull/4337)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fixed `replace()` causing an infinite loop and OOM when the search string is
  empty. Empty search now inserts the replacement at every character boundary
  with bounded allocation, so queries such as `replace("abc", "", "-")` return
  `"-a-b-c-"` instead of crashing the server.
  [#4269](https://github.com/memgraph/memgraph/pull/4269)
...
- Fixed user and role data being corrupted during rolling HA upgrades between
  Memgraph versions. Auth entities now carry a version field and are migrated
  on deserialize, so fine-grained access grants replicated from an older MAIN
  are preserved after failover to an upgraded REPLICA.
  [#4301](https://github.com/memgraph/memgraph/pull/4301)
- Fixed duplicated coordinator entries in the routing table when re-adding a
  coordinator that was the current leader.
  [#4311](https://github.com/memgraph/memgraph/pull/4311)
- Fixed negative replication lag values in the routing table that could produce
  invalid routing entries when re-adding a coordinator or during failover.
  [#4312](https://github.com/memgraph/memgraph/pull/4312)
- Fixed MAIN instances hanging on shutdown while snapshot or WAL replication is
  still in progress. Shutdown now aborts the replication client so the instance
  stops immediately instead of waiting for replication to finish.
  [#4314](https://github.com/memgraph/memgraph/pull/4314)
- Fixed replication failures during WAL recovery when file-bearing RPC messages
  were split across network reads. Consumed message data is now dropped from the
  buffer so files are not parsed twice.
  [#4317](https://github.com/memgraph/memgraph/pull/4317)
- Fixed a rare server crash under heavy concurrent write load caused by a
  heap-use-after-free between transaction commit and storage garbage collection.
  Committed deltas are now registered before the commit is marked finished, so
  fast-discard and GC no longer free memory still referenced by in-flight
  transactions.
  [#4323](https://github.com/memgraph/memgraph/pull/4323)
- Fixed edge property index scans scaling linearly with index size for bounded
  lookups. Scans now seek directly to the lower bound instead of walking from
  the head of the skiplist.
  [#4326](https://github.com/memgraph/memgraph/pull/4326)
- Fixed several planner bugs that caused wrong or missing rows for edge
  `id(e) IN [...]`, edge property `IN` lists with duplicates, `ORDER BY` over
  unwind-fed index scans, composite-index nullable ordering, and nested edge
  property filters.
  [#4336](https://github.com/memgraph/memgraph/pull/4336)
- Fixed snapshot and WAL transfer files not being cleaned up when the final RPC
  response fails to send. Files are now moved to the `.old` directory regardless
  of send success, preventing orphaned durability files on replicas.
  [#4347](https://github.com/memgraph/memgraph/pull/4347)
- Track memory reclamation when jemalloc's background dirty-page decay returns
  pages to the OS. This caused reported memory usage to exceed actual memory
  usage. [#4396](https://github.com/memgraph/memgraph/pull/4396)

### Lab v3.12.1 - August 28th, 2026

<LabReleasesClient version="3.12.1" />

### Lab v3.12.0 - July 15th, 2026
...
- `schema.node_type_properties()` and `schema.rel_type_properties()` return
  additional columns and changed types. `propertyTypes` is now `List<String>`
...
- Bare `SHOW STORAGE INFO` now returns only instance-level metrics. Per-database
  ...
  <name>`. Several global columns were renamed (`global_memory_tracked` →
...
- `schema.node_type_properties()` and `schema.rel_type_properties()` now return
  ...
  and `propertyTypes` is a proper `List<String>`. `rel_type_properties()`
...
- Fixed Memgraph and MAGE Docker containers failing to start when there are no
...
- Coordinators ignore periodic storage snapshots (no user data to snapshot), and
  `--init-file` / `--init-data-file` are rejected on coordinators and on data
  instances that join an HA cluster (`--management-port` set). Remove those
  flags from coordinator and HA data-instance configs before upgrading, and run
  any bootstrap Cypher against the MAIN after the cluster is formed instead of
  relying on startup init files.
  [#4035](https://github.com/memgraph/memgraph/pull/4035)
- Fine-grained label and edge-type permissions now support explicit `DENY`
  semantics (including `DENY ALL`/`*`) which take precedence over `GRANT`. The
  `UPDATE` permission is split into granular capabilities: `SET LABEL`, `REMOVE
  LABEL`, `SET PROPERTY`, `CREATE EDGE`, `DELETE EDGE`. Existing grants that
  relied on `GRANT NOTHING` / `REVOKE NOTHING` must be migrated to `DENY`.
  [#3789](https://github.com/memgraph/memgraph/pull/3789)
- `SHOW STORAGE INFO` output renames three fields and adds one new field:
  `disk_usage` → `global_disk_usage`, `memory_tracked` → `global_memory_tracked`,
  `allocation_limit` → `global_runtime_allocation_limit`, and adds
  `global_license_allocation_limit`. Update any tooling or scripts that parse
  these field names.
  [#3999](https://github.com/memgraph/memgraph/pull/3999)

{<h4 className="custom-header">✨ New features</h4>}

- Coordinators now accept read-only system queries (`SHOW CONFIG`, `SHOW LICENSE
  INFO`, `SHOW BUILD INFO`, `SHOW STORAGE INFO`) plus `SET DATABASE SETTING` and
  `RELOAD SSL`, so you can inspect configuration or reload TLS from a
  coordinator without opening a session to a data instance.
  [#4035](https://github.com/memgraph/memgraph/pull/4035)
- Added `SHOW QUERY CALLABLE MAPPINGS` to list all query procedure and function
  aliases with their original source names and types (`procedure` / `function`),
  so you can audit callable mappings without digging through configuration.
  [#4014](https://github.com/memgraph/memgraph/pull/4014)
- Added `WHERE` clause support directly after `YIELD` in procedure calls:
  `CALL mg.procedures() YIELD * WHERE name = 'mg.procedures' RETURN name`. This
  lets you filter procedure results inline instead of wrapping the call in a
  `WITH … WHERE` subquery.
  [#4010](https://github.com/memgraph/memgraph/pull/4010)
- Memgraph now warns (or errors with `--strict-flag-check`) when unexpected
  positional arguments appear after startup flag parsing, catching common
  mistakes like `--flag false` (which silently ignores `false` and sets the flag
  to `true`). Default behavior is strict in development builds; release builds
  emit a warning.
  [#4009](https://github.com/memgraph/memgraph/pull/4009)
- Added `--data-dir-lock-acquisition-timeout-sec` to retry acquiring the
  data-directory file lock for a configurable duration before giving up. This
  prevents spurious startup failures when a pod is force-killed and the old lock
  has not been released yet (e.g. on CephFS or other distributed storage with
  propagation delay). [#4021](https://github.com/memgraph/memgraph/pull/4021)
- Replication error messages now identify exactly which REPLICA failed and why
  (replica name, replication mode, failure reason) instead of generic "at least
  one SYNC/STRICT_SYNC replica has not confirmed" messages, making it faster to
  pinpoint replication issues.
  [#3985](https://github.com/memgraph/memgraph/pull/3985)
- Added `RELOAD BOLT_SERVER TLS` Cypher command to reload SSL/TLS certificates
  at runtime without restarting Memgraph. Existing connections keep the old
  certificate; new connections use the reloaded one. Useful for Let's Encrypt
  renewals and compliance certificate rotations.
  [#3962](https://github.com/memgraph/memgraph/pull/3962)
- `WITH CONFIG` maps (e.g. `CREATE VECTOR INDEX … WITH CONFIG {…}`) now accept
  query parameters as values, so you can pass `{dimension: $dim, capacity: $cap}`
  instead of hard-coding values in the config literal.
  [#3959](https://github.com/memgraph/memgraph/pull/3959)
- `instance_down_timeout_sec` and `instance_health_check_frequency_sec` are now
  Raft-replicated coordinator runtime settings changeable via
  `SET COORDINATOR SETTING` without restarting the cluster. The old
  `--instance-down-timeout-sec` and `--instance-health-check-frequency-sec`
  flags are deprecated and ignored.
  [#3949](https://github.com/memgraph/memgraph/pull/3949)
- HA coordinator cluster operations (register, unregister, promote, demote)
  now follow a Raft-first pattern: the state is committed to the Raft log and
  acknowledged by a majority before returning success. RPCs to data instances
  are retried automatically by the reconciliation loop, so transient network
  issues no longer require manual intervention.
  [#3942](https://github.com/memgraph/memgraph/pull/3942)
  [#3922](https://github.com/memgraph/memgraph/pull/3922)
- Point2D, Point3D, and Enum types are now fully supported in the MGP C, C++,
  and Python query module APIs. Query modules can create, copy, compare, and
  inspect these types as `mgp_value`, fixing crashes in `schema.node_type_properties()`
  on graphs that use spatial or enum properties.
  [#3980](https://github.com/memgraph/memgraph/pull/3980)
- Added `schema.node_type_properties` and `schema.rel_type_properties` config
  argument to match the Neo4j specification, improving compatibility with the
  Neo4j ODBC BI Connector. Also adds the `apoc.version` mapping.
  [#4000](https://github.com/memgraph/memgraph/pull/4000)
- Added server-side descriptions: annotate labels, edge types, properties, and
  databases with human-readable strings that are persisted and visible in
  `SHOW SCHEMA INFO`. Useful for documenting your graph schema directly in the
  database.
  [#3894](https://github.com/memgraph/memgraph/pull/3894)
- Vector edge indexes now use the single-store pattern (storing only a
  `VectorIndexId` in edge properties rather than duplicating vector data),
  matching vertex vector indexes. Snapshot serialization is backward-compatible.
  [#3929](https://github.com/memgraph/memgraph/pull/3929)
- Added GNN import/export module to MAGE: export the graph to PyTorch Geometric
  (PyG) or TensorFlow GNN (TF-GNN) JSON format, run your GNN pipeline
  externally, then write predictions back with `pyg_import` / `tf_import`.
  [#3803](https://github.com/memgraph/memgraph/pull/3803)
- Added Kerberos SSO authentication module: clients can authenticate using
  Kerberos service tickets via GSSAPI, with role mapping through LDAP group
  membership or static principal-to-role configuration.
  [#3916](https://github.com/memgraph/memgraph/pull/3916)
- Added descending (DESC) label-property index support. `ORDER BY … DESC`
  queries can now be served directly by a DESC index, eliminating the sort.
  Create with `CREATE INDEX ON :Label(prop) WITH CONFIG {"order": "DESC"}`.
  [#3996](https://github.com/memgraph/memgraph/pull/3996)
- Added `CALL (<variable-list>) { <subquery> }` syntax for variable-scoped
  subqueries, letting you explicitly control which outer-scope variables are
  accessible inside a `CALL` subquery block.
  [#4073](https://github.com/memgraph/memgraph/pull/4073)
- Added Neo4j-compatible `CREATE INDEX [name] FOR … ON …` and
  `CREATE CONSTRAINT [name] FOR … REQUIRE …` syntax for creating indexes and
  constraints, alongside the existing Memgraph `ON :Label(prop)` / `ASSERT`
  syntax. Both syntaxes are now accepted. Named indexes and constraints emit a
  warning, as names are not yet stored.
  [#4043](https://github.com/memgraph/memgraph/pull/4043)
- Added per-database memory tracking and Tenant Profiles. Every allocation is
  now attributed to its database across three categories — graph storage,
  vector index embeddings, and query execution — visible via
  `SHOW STORAGE INFO ON DATABASE <name>` and `SHOW MEMORY INFO`.
  Create named memory-limit profiles with
  `CREATE TENANT PROFILE "<name>" WITH MEMORY LIMIT <size>`, attach them to
  databases with `SET TENANT PROFILE`, and manage them with `ALTER`, `DROP`,
  `REMOVE TENANT PROFILE FROM DATABASE`, and `SHOW TENANT PROFILES`. Profiles
  are persisted across restarts and replicated to replicas.
  [#3952](https://github.com/memgraph/memgraph/pull/3952)
- Three built-in roles (`admin`, `readonly`, `readwrite`) are now created
  automatically on first enterprise user creation, with the first user assigned
  the `admin` role. Roles can be granted and revoked incrementally using
  `GRANT ROLE[S] <roles> TO <user>` and `REVOKE ROLE[S] <roles> FROM <user>`.
  [#3889](https://github.com/memgraph/memgraph/pull/3889)
- Added compatibility for the Apache Spark Neo4j Connector 5.4.0 by aliasing
  additional query procedures, so PySpark workloads that rely on the connector
  work out of the box.
  [#4091](https://github.com/memgraph/memgraph/pull/4091)
- The embeddings query module can now compute text embeddings via remote APIs
  (OpenAI, Cohere, and other providers supported by `litellm`). Export the
  relevant API key when launching the MAGE container and set `model_name` to
  `"{provider}/{model-name}"` to use a remote model instead of a local one.
  [#4064](https://github.com/memgraph/memgraph/pull/4064)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Text index search performance is significantly improved: GIDs are now returned
  directly from the Rust layer instead of full JSON documents, and Tantivy
  searchers are pinned per transaction for snapshot-consistent reads.
  [#3963](https://github.com/memgraph/memgraph/pull/3963)
- Upgraded Tantivy to 0.25, bringing ~30% faster text index imports and ~20%
  better search throughput under contention.
  [#3927](https://github.com/memgraph/memgraph/pull/3927)
- `ORDER BY` is now eliminated when an index scan already provides the required
  ascending order, avoiding an unnecessary sort. With `LIMIT`, this means the
  planner stops after N rows instead of sorting the entire result set first.
  [#3950](https://github.com/memgraph/memgraph/pull/3950)
- Added `AI_PLATFORM` license type that enforces memory limits only on graph
  memory, leaving vector index memory unconstrained up to the system
  `--memory-limit`. Useful when running large vector workloads alongside graph
  data without hitting the overall allocation cap.
  [#3999](https://github.com/memgraph/memgraph/pull/3999)
- Most third-party build dependencies are now managed through Conan 2 packages,
  making builds more reproducible and the contributor setup easier. See the
  updated [build-from-source guide](/getting-started/build-memgraph-from-source)
  for changes to the build workflow.
  [#4002](https://github.com/memgraph/memgraph/pull/4002)
- Additional third-party libraries (gflags, NuRaft, Pulsar client, RocksDB,
  usearch) are now managed as Conan recipes, removing the `init` script
  and the `libs/` directory from the build process.
  [#4023](https://github.com/memgraph/memgraph/pull/4023)
- `SHOW INDEX INFO`, `SHOW CONSTRAINT INFO`, `SHOW NODE LABELS`, and
  `SHOW EDGE TYPES` no longer open a storage transaction, reducing lock
  contention and improving responsiveness on busy instances.
  [#4006](https://github.com/memgraph/memgraph/pull/4006)
- Reduced garbage collector contention under high-concurrency supernode
  workloads by moving delta processing to a local copy under a short lock,
  preventing queries from waiting on garbage collection to complete.
  [#4119](https://github.com/memgraph/memgraph/pull/4119)
- Improved garbage collector performance after concurrent import of datasets
  with a significant number of edges on one or more supernodes by avoiding an
  O(n) delta chain walk when unlinking non-sequential deltas.
  [#4126](https://github.com/memgraph/memgraph/pull/4126)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fixed a potential OOM crash when dropping a large vector index: the internal
  transfer from indexed format back to property store now manages peak memory
  usage so the operation completes cleanly even under tight memory limits.
  [#4001](https://github.com/memgraph/memgraph/pull/4001)
- `RELOAD BOLT_SERVER TLS` is now supported on coordinator instances, so you can
  rotate TLS certificates cluster-wide without opening a session to each data
  instance separately.
  [#4003](https://github.com/memgraph/memgraph/pull/4003)
- Fixed `refactor.merge_nodes` discarding relationship properties: all
  properties on merged relationships are now correctly copied to the new
  relationship.
  [#3948](https://github.com/memgraph/memgraph/pull/3948)
- Fixed `ORDER BY` failing for list values; Cypher list ordering semantics
  (lexicographic / dictionary order) are now applied correctly.
  [#3877](https://github.com/memgraph/memgraph/pull/3877)
- Fixed the `link_prediction` MAGE module returning incorrect ROC-AUC scores
  when all labels belong to a single class (returns a neutral 0.5 AUC instead
  of erroring or producing inconsistent results).
  [#3845](https://github.com/memgraph/memgraph/pull/3845)
- Fixed a use-after-free crash on replicas receiving a snapshot: `Clear()` now
  also discards pending GC deltas, preventing stale pointers from being
  dereferenced on the next transaction commit.
  [#4013](https://github.com/memgraph/memgraph/pull/4013)
- Fixed a crash in HA coordinators caused by a dangling pointer in
  `ReplicationInstanceConnector`; concurrent coordinator operations are now safe
  and cluster membership changes complete without instability.
  [#3922](https://github.com/memgraph/memgraph/pull/3922)
- Fixed a bug where `std::thread::hardware_concurrency()` returning 0 on some
  machines caused RPC and communication servers to be created with zero worker
  threads, leading to silent failures. A safe fallback of 2 is now used.
  [#3982](https://github.com/memgraph/memgraph/pull/3982)
- Fixed replication network channels not being cleanly destroyed during
  shutdown, which could block shutdown or leave sockets open.
  [#4005](https://github.com/memgraph/memgraph/pull/4005)
- Fixed a crash that could occur when calling `SetValueForce` on storage
  failures during a scheduled license revalidation.
  [#4004](https://github.com/memgraph/memgraph/pull/4004)
- Fixed a TCP segmentation bug in SLK multi-file stream processing: when a file
  transition arrives in a fragmented TCP segment, `CheckStreamStatus` now
  validates metadata completeness before signalling `NEW_FILE`, preventing
  incorrect stream state.
  [#3925](https://github.com/memgraph/memgraph/pull/3925)
- Fixed a crash if the telemetry server fails to initialize: the exception is
  now caught, logged, and the instance shuts down gracefully instead of
  terminating abruptly.
  [#4026](https://github.com/memgraph/memgraph/pull/4026)
- Transient disk failures when opening WAL or snapshot output files are now
  handled without crashing; failed snapshots are skipped and retried at the next
  scheduled interval (WAL failures remain fatal).
  [#4025](https://github.com/memgraph/memgraph/pull/4025)
- Fixed log rotation being skipped after instance restarts: spdlog in-memory
  rotation state is now re-initialized on startup so daily log files are
  rotated correctly even after frequent restarts.
  [#4019](https://github.com/memgraph/memgraph/pull/4019)
- Fixed a crash when calling `std::abort` while using an async logger: removing
  the `spdlog::shutdown()` call before abort eliminates a window where the
  logger could segfault, making core dumps more useful for diagnosis.
  [#4034](https://github.com/memgraph/memgraph/pull/4034)
- Fixed a network descriptor leak in the epoll listener and a potential crash
  if `epoll_create1` fails; the latter now throws an exception instead of
  crashing the database.
  [#4032](https://github.com/memgraph/memgraph/pull/4032)
- Fixed fine-grained access control roles leaking across databases: permission
  checks now evaluate only the roles that apply to the currently active
  database, preventing unintended privilege escalation for users with
  per-database roles.
  [#4042](https://github.com/memgraph/memgraph/pull/4042)
- Fixed a replication lag counter overflow that could occur when a REPLICA
  temporarily had more committed transactions than the new MAIN. REPLICAs in
  this transient state are now excluded from the routing table until they
  converge.
  [#4040](https://github.com/memgraph/memgraph/pull/4040)
- Fixed the MAGE DEB package post-install script installing Python packages for
  the wrong architecture on ARM machines.
  [#4093](https://github.com/memgraph/memgraph/pull/4093)
- Fixed index and constraint CREATE/DROP operations not being reverted on abort,
  which could leave ghost entries blocking subsequent retries. This was possible
  only when using STRICT_SYNC replication.
  [#4074](https://github.com/memgraph/memgraph/pull/4074)
- `FREE MEMORY` now reclaims significantly more memory after heavy update/delete
  workloads. Also fixed stale label-property index entries accumulating in
  `IN_MEMORY_ANALYTICAL` mode when repeatedly setting indexed properties.
  [#4071](https://github.com/memgraph/memgraph/pull/4071)
- Fixed a hang or crash when using `LIMIT` on both sides of a `UNION` or
  `UNION ALL` query.
  [#4065](https://github.com/memgraph/memgraph/pull/4065)
- Fixed rare crashes that could occur during shutdown or under heavy load after
  concurrent edge imports.
  [#4020](https://github.com/memgraph/memgraph/pull/4020)
- Fixed a crash or broken memory state caused by jemalloc's lazy thread-local
  storage initialization: if a thread's first allocation triggered an
  allocation-limit check before TLS was ready, jemalloc entered an
  unrecoverable state. Each thread now forces an initial allocation to
  guarantee TLS is set up before limits are enforced.
  [#4095](https://github.com/memgraph/memgraph/pull/4095)
- Hardened the embedded Python runtime against crashes and resource contention
  when running Python query procedures that use libraries like NumPy, PyTorch,
  or DGL. Shutdowns are now more reliable (no more sporadic fatal errors on
  exit) and the dependency scanner no longer pollutes the interpreter's main
  namespace.
  [#4038](https://github.com/memgraph/memgraph/pull/4038)
- Fixed misclassified task priorities that caused `BEGIN`, `COMMIT`, and `ABORT`
  queries to run on reserved high-priority worker threads. Under heavy parallel
...
- HA coordinator data directories from previous versions contain corrupted
  cluster configuration log entries that cannot be recovered. When upgrading to
  3.9, all coordinator data directories must be cleaned and coordinators
  restarted fresh; run setup queries again on the new coordinator leader. Data
  instances are not affected. See the [migration
  guide](/clustering/high-availability/migrating-to-v3-9-ha).
  [#3900](https://github.com/memgraph/memgraph/pull/3900)

{<h4 className="custom-header">✨ New features</h4>}

- Vector indexes can now be created with a function in the `WITH CONFIG`
  clause: `CREATE VECTOR INDEX ... WITH CONFIG fun();` so you can generate
  index configuration dynamically instead of using a static map literal.
  [#3801](https://github.com/memgraph/memgraph/pull/3801)
- Added `--storage-floating-point-resolution-bits` to choose double (64), float
  (32), or half (16) precision for floating-point property values system-wide,
  so you can reduce memory use for graphs with many float properties when lower
  precision is acceptable.
  [#3817](https://github.com/memgraph/memgraph/pull/3817)
- Added coordinator setting `deltas_batch_progress_size` to control how many
  deltas a replica processes before sending an in-progress heartbeat during
  replication. HA clusters can tune it to avoid RPC timeouts on large
  transactions (lower value) or reduce overhead on fast replicas (higher
  value). Set via `SET COORDINATOR SETTING`; default is 100000 (unchanged
  behavior for existing clusters).
  [#3797](https://github.com/memgraph/memgraph/pull/3797)
- `SHOW TRANSACTIONS` now lists transactions in committing and aborting phases
  (in addition to running), and shows any snapshot creation in progress with
  phase and progress in metadata, so you can see what is running on the
  database at a glance. [#3833](https://github.com/memgraph/memgraph/pull/3833)
- Added `--logger-type` flag (`sync` or `async`, default `sync`). When set to
  `async`, log messages are written by a background thread instead of inline,
  reducing latency on hot paths under high throughput. Default behavior is
  unchanged; use `--logger-type=async` to opt in.
  [#3844](https://github.com/memgraph/memgraph/pull/3844)
- Added `--log-retention-days` to control how many days daily log files are
  kept before automatic removal. Default is 35 (unchanged from previous
  behavior); set the flag to tune retention for storage or compliance.
  [#3862](https://github.com/memgraph/memgraph/pull/3862)
- `--storage-access-timeout-sec` can now be updated at runtime via the
  `storage.access_timeout_sec` database setting, so you can increase the
  timeout when you hit storage timeout errors without restarting the instance.
  [#3882](https://github.com/memgraph/memgraph/pull/3882)
- Added server-side global parameters: define named values that persist across
  sessions and are resolved in queries. Use `SET GLOBAL PARAMETER`, `UNSET
  GLOBAL PARAMETER`, and `SHOW PARAMETERS` to manage them, and reference values
  in Cypher with `$param` (client-provided parameters take precedence). Global
  parameters are replicated and recovered on restart, so you can use them as a
  durable, cluster-wide store for frequently used query values.
  [#3717](https://github.com/memgraph/memgraph/pull/3717)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- `SHOW STORAGE INFO` now splits memory into `graph_memory_tracked` and
  `vector_index_memory_tracked`, so you can see how much memory vector indices
  use versus graph data and better plan capacity or troubleshoot memory usage.
  [#3847](https://github.com/memgraph/memgraph/pull/3847)
- Renamed the `SHOW STORAGE INFO` key from `embeddings_memory_tracked` to
  `vector_index_memory_tracked` so the name reflects that it accounts for all
  vector index memory, not only embeddings data.
  [#3863](https://github.com/memgraph/memgraph/pull/3863)
- ARM platforms now install the correct Go and C# binaries for driver tests, so
  driver tests can run on ARM in addition to x86.
  [#3776](https://github.com/memgraph/memgraph/pull/3776)
- Error messages on storage access timeout are now more descriptive so you can
  identify and troubleshoot lock or contention issues more easily.
  [#3778](https://github.com/memgraph/memgraph/pull/3778)
- Reduced memory use for vertices and edges by packing boolean values in the
  delta pointer layout (8 bytes saved per vertex and per edge) with no
  performance impact. [#3814](https://github.com/memgraph/memgraph/pull/3814)
- When `--storage-snapshot-on-exit` is false, any in-progress periodic snapshot
  is aborted on shutdown so the instance stops faster instead of waiting for
  the snapshot to finish. The snapshot digest is now updated only after a
  successful write, so a failed or aborted snapshot no longer causes the next
  periodic snapshot to be skipped.
  [#3823](https://github.com/memgraph/memgraph/pull/3823)
- License selection now considers the database, environment variables, and CLI
  together, automatically choosing the license with the furthest expiry (with
  source priority to break ties), so you no longer need to clear old database
  settings for a newer CLI or env license to apply. Malformed or expired keys
  are rejected immediately when set, and reported license info now matches
  actual feature availability.
  [#3859](https://github.com/memgraph/memgraph/pull/3859)
- EdgeSetProperty WAL delta has been expanded. This speeds up edge recovery and
  replication. Nothing to do, this is all automatic and back-compatible.
  [#3799](https://github.com/memgraph/memgraph/pull/3799)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Improved query planning for BFS with index lookup: a cost model now chooses
  between bidirectional and single-source BFS so you get better execution plans
  for shortest-path queries when both source and destination use an index.
  [#3751](https://github.com/memgraph/memgraph/pull/3751)
- Fixed a bug where `schema.assert` could not be used as a property name or
  value, which blocked backup and restore in some `DUMP DATABASE` scenarios.
  [#3772](https://github.com/memgraph/memgraph/pull/3772)
- When TLS/SSL certificate or key loading fails, Memgraph now logs an error
  message so you can see why startup or connection failed and fix path or
  permission issues. [#3795](https://github.com/memgraph/memgraph/pull/3795)
- With `--data-recovery-on-startup` enabled, the system now recovers both users
  and databases as system metadata on startup, so all system metadata is
  restored consistently; user data is still not recovered.
  [#3807](https://github.com/memgraph/memgraph/pull/3807)
- Plan caching is now enabled again for queries that use BFS, fixing a
  performance regression where a new plan was created for every such query.
  [#3821](https://github.com/memgraph/memgraph/pull/3821)
- User and role details that apply only to enterprise licenses are now
  persisted when switching between enterprise and community, so they are not
  lost and apply again after upgrading back to an enterprise license.
  [#3867](https://github.com/memgraph/memgraph/pull/3867)
- `SHOW SCHEMA INFO` now correctly respects fine-grained label-based access
  control (LBAC) READ permissions, so restricted users see only the schema
  elements they are allowed to access.
  [#3870](https://github.com/memgraph/memgraph/pull/3870)
- Edge removals in the text index are now fully tracked when using `DETACH
  DELETE`, so `SHOW INDEX INFO` reports accurate edge counts instead of
  outdated metadata (search results were already correct).
  [#3878](https://github.com/memgraph/memgraph/pull/3878)
- Concurrent operations on vector indexes (nodes and edges) are now less
  contention-prone: internal locking allows multiple writers in parallel and
  reserves exclusive access only for resize and removal. Garbage collection of
  deleted entries is targeted instead of scanning the full index, improving GC
  performance and fixing a potential crash when vertices or edges were deleted
  in analytical mode before the vector index was cleaned up.
  [#3856](https://github.com/memgraph/memgraph/pull/3856)
- Fixed a bug where the parallel version of `DISTINCT` could invalidate other
  cursors in the query and cause failures. PARALLEL EXECUTION with `DISTINCT`
  now runs correctly and faster.
  [#3876](https://github.com/memgraph/memgraph/pull/3876)
- Fixed inconsistent ordering in the parallel `DISTINCT` cursor, which could
  make PARALLEL EXECUTION output unstable. `DISTINCT` used in PARALLEL
  EXECUTION now produces correct, stable results.
  [#3815](https://github.com/memgraph/memgraph/pull/3815)
- Fixed a bug in HA replication where concurrent edge deltas could be processed
  twice during durability traversal, causing replica inconsistency.
  [#3836](https://github.com/memgraph/memgraph/pull/3836)
- Fixed a crash that could occur when queries using Python user-defined
  functions were cleaned up on internal worker threads. Memgraph now acquires
  the Python GIL before releasing Python object references during query
  teardown, preventing segfaults when using Python query modules with custom
  functions. [#3837](https://github.com/memgraph/memgraph/pull/3837)
- Fixed a rare race in durability when a transaction that created concurrent
  edges aborted while the delta chain was being processed, which could cause
  duplicate or missed edge-delta encoding and durability integrity issues.
  [#3839](https://github.com/memgraph/memgraph/pull/3839)
- Fixed a segfault that could occur when closing Python query modules during
  Memgraph shutdown. A missing Python interpreter state check is now performed
  before cleanup so shutdown completes cleanly when Python modules are loaded.
  [#3842](https://github.com/memgraph/memgraph/pull/3842)
- Fixed a segfault during vector index garbage collection when vertices or
  edges were being inserted concurrently, which could occur with bulk
  create/delete/create workloads.
  [#3846](https://github.com/memgraph/memgraph/pull/3846)
- Fixed a data race in the high-availability coordinator's cluster
  configuration handling. Concurrent config loads and saves are now properly
  synchronized, preventing inconsistent state and coordinator instability
  during cluster membership changes.
  [#3850](https://github.com/memgraph/memgraph/pull/3850)
- Shutdown on SIGTERM and SIGINT no longer runs inside a signal handler, where
  logging and mutex use were unsafe. Signals are now consumed on the main
  thread so the full shutdown sequence runs in normal context, making shutdown
  more robust against rare crashes or hangs.
  [#3851](https://github.com/memgraph/memgraph/pull/3851)
- Fixed a Python binding assertion caused by unhandled import errors. When the
  system encounters errors (for example when over the memory limit), Python will
  fail to load modules. This was not gracefully handled and would cause
  termination. The Python bindings are now more robust.
  [#3864](https://github.com/memgraph/memgraph/pull/3864)
- Fixed a bug where periodic commit failure caused a double abort (which in
  turn caused an assert). Periodic commit failure is a special case and is now
  handled correctly by the interpreter. Users can run periodic commit without
  worrying about query failures.
  [#3868](https://github.com/memgraph/memgraph/pull/3868)
- Fixed a bug where incorrect queries using CALL + UNION would not throw an
  exception but would just produce wrong results.
  [#3869](https://github.com/memgraph/memgraph/pull/3869)
- Fixed occasional "Operation not permitted" error when vertices with
  concurrently imported edges are indexed.
  [#3871](https://github.com/memgraph/memgraph/pull/3871)
- Communication data parsing is now more robust when values do not match the
  expected type, so you get fewer unexpected communication errors between
  clients and the database.
  [#3873](https://github.com/memgraph/memgraph/pull/3873)
- The schema query module now works with ZonedDateTime properties, so you can
  use schema assertions and schema-based workflows (such as backup and restore)
  on graphs that store temporal data with time zones.
  [#3874](https://github.com/memgraph/memgraph/pull/3874)
- Fixed a bug where HA coordinator Raft cluster configuration log entries were
  not persisted correctly, causing coordinators to crash-loop on restart if
  uncompacted configuration entries existed on disk — most commonly triggered
  when a newly added coordinator restarts before a Raft snapshot compacts those
  entries. Users running HA clusters who experience a coordinator crash-loop
  after adding a new coordinator should upgrade to this version; all
  coordinator data directories should be wiped before restarting and setup
  queries should be run again on the new coordinator leader.
  [#3900](https://github.com/memgraph/memgraph/pull/3900)
- Fixed a crash in the `embeddings` query module when multiple Bolt sessions
  called `embeddings.text()` or `embeddings.node_sentence()` in parallel. Model
  initialization is now thread-safe with a global cache using double-checked
  locking, preventing race conditions and eliminating redundant model
  instantiation per request.
  [#3914](https://github.com/memgraph/memgraph/pull/3914)
- Python module cleanup during shutdown no longer crashes the process;
  previously, filesystem operations that failed while removing `__pycache__`
  directories would throw exceptions inside a destructor, triggering
  `std::terminate` and an abrupt abort — now those operations use non-throwing
  error-code overloads and log warnings instead, so shutdown completes
  gracefully even when cached files have already been removed.
  [#3908](https://github.com/memgraph/memgraph/pull/3908)
- Fixed a crash that could occur when a coordinator's internal Raft consensus
  thread encountered an error during startup or operation — the process would
  terminate with a segmentation fault instead of exiting cleanly. This affected
  HA deployments where a coordinator node hit an unexpected state in its
  background commit path. No user action is required; the fix ensures the
  coordinator process exits immediately and cleanly without triggering a
  cascading failure in global cleanup.
  [#3904](https://github.com/memgraph/memgraph/pull/3904)
- Fixed a race condition during graceful shutdown (SIGTERM) of
  high-availability data instances where the internal management RPC server
  could process incoming coordinator health-check requests after the
  replication state had already been destroyed, resulting in a crash. The
  shutdown sequence now correctly stops the management server before tearing
  down replication state, so users running HA clusters should no longer observe
  unexpected crashes or error logs when stopping data instances with SIGTERM.
  No configuration changes or user action is required.
  [#3903](https://github.com/memgraph/memgraph/pull/3903)
- Fixed a bug in vector index garbage collection where the `equal_range`
  iterator incorrectly yielded tombstone (deleted) entries as live matches,
  causing double-frees that corrupted size accounting and led to crashes. Users
  running workloads with frequent deletions on vector indexes were at risk of
  hitting this crash. [#3899](https://github.com/memgraph/memgraph/pull/3899)

### Lab v3.9.0 - March 18th, 2026

<LabReleasesClient version="3.9.0" />

### Memgraph v3.8.1 - February 17th, 2026

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fixed using `LOAD CSV` via SSL, by adding missing `ca-certificates` package
  to the Memgraph Docker image
  [#3793](https://github.com/memgraph/memgraph/pull/3793)
- A data instance could get into a deadlock state when some specific timings
  are triggered. This is now fixed so users should be able to always start a
  data instance without any deadlocks occurring.
  [#3787](https://github.com/memgraph/memgraph/pull/3787)
- Fix so that concurrent edge imports will no longer delay garbage collection.
  This resulted in temporary memory spikes after import, until the periodic
  garbage collector could free up memory.
  [#3784](https://github.com/memgraph/memgraph/pull/3784)

### Memgraph v3.8.0 - February 11th, 2026

{<h4 className="custom-header">⚠️ Breaking changes</h4>}

- Switching to analytical mode now fails if constraints exist. Creating
constraints or text indexes in analytical mode is now blocked. Workflows that
relied on these operations will need to be updated.
[#3461](https://github.com/memgraph/memgraph/pull/3461)
- Removed deprecated flags `--auth-module-executable`,
`--bolt_session_inactivity_timeout`, and `--storage_parallel_index_recovery`.
Users should migrate to `--auth-module-mappings` and
`--storage-parallel-schema-recovery` respectively.
[#3474](https://github.com/memgraph/memgraph/pull/3474)
- The auth module fix introduced the new message ID protocol. All the external
auth modules have to be updated to support the new protocol.
[#3755](https://github.com/memgraph/memgraph/pull/3755)

{<h4 className="custom-header">✨ New features</h4>}

- Concurrent edge creation on supernodes is no longer treated as a serialization
error, allowing truly concurrent edge-heavy batch imports and improving import
speeds for high-degree vertices by eliminating transaction aborts and retries.
[#3103](https://github.com/memgraph/memgraph/pull/3103)
- Added `toFloatList()`, `toBooleanList()`, and `toIntegerList()` functions for
converting list elements to the specified types.
[#3318](https://github.com/memgraph/memgraph/pull/3318)
- Introduced parallel Cypher execution as an Enterprise feature. Queries can opt
in via `USING PARALLEL EXECUTION` (with an optional thread count) to run scan
and aggregation phases on multiple worker threads for faster execution of large
queries. Requires the `PARALLEL_EXECUTION` privilege.
[#3394](https://github.com/memgraph/memgraph/pull/3394)
- Simplified data import processes by introducing the native `LOAD JSONL`
clause, enabling users to ingest JSON Lines files directly into the database.
This eliminates the need for external preprocessing or custom transformation
scripts, significantly reducing the time and complexity required to import
large, line-delimited datasets.
[#3410](https://github.com/memgraph/memgraph/pull/3410)
- Parquet files can now be downloaded from any `http(s)/ftp` server directly
when using `LOAD PARQUET` clause.
[#3444](https://github.com/memgraph/memgraph/pull/3444)
- Streamlined data ingestion workflows by allowing the direct loading of JSONL
files from HTTP and FTP servers. This update includes a new
`file.download_conn_timeout_sec` run-time setting, giving users greater control
over connection stability and preventing hung processes during remote data
transfers. [#3453](https://github.com/memgraph/memgraph/pull/3453)
- Added support for loading JSONL and CSV files directly from S3. Users can now
load graphs from more sources. Furthermore, these features improve the so-called
on-demand capabilities of Memgraph.
[#3463](https://github.com/memgraph/memgraph/pull/3463)
- Extended pattern comprehension support to work correctly in all query clauses
(`RETURN`, `WITH`, `WHERE`, `ORDER BY`, `UNWIND`, `FOREACH`, `CREATE`, `SET`,
`MERGE`) and added support for nested pattern comprehensions.
[#3495](https://github.com/memgraph/memgraph/pull/3495)
- Simplified High Availability (HA) deployments by introducing the `UPDATE
CONFIG FOR` command. By allowing the leader coordinator to modify Bolt server
settings across instances, this feature enables a one-step installation process
for HA Helm charts. [#3530](https://github.com/memgraph/memgraph/pull/3530)
- Enhanced visibility into session identity and access control by introducing
the `username()` and `roles()` built-in functions. These functions allow users
...
- Temporal functions (`date`, `duration`, `localdatetime`, `datetime`, and
`localtime`) now accept null as an argument, returning null instead of throwing
an error. [#3714](https://github.com/memgraph/memgraph/pull/3714)
- Added `load_from_str()` function to the `json_util` query module, allowing
JSON to be loaded directly from a string instead of a file.
[mage#530](https://github.com/memgraph/mage/pull/530)
- Added an LLM query module with a completion function that uses LiteLLM to send
text to an LLM and receive a completion. This enables LLM-powered summarization
or text generation directly from Cypher; users configure their provider API key
and optionally the model or API endpoint before calling the function.
[#3761](https://github.com/memgraph/memgraph/pull/3761)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Optimized vector property storage by eliminating data duplication between the
property store and vector index. Vector data is now stored only once in the
index, significantly reducing memory usage for vector-indexed properties.
[#3291](https://github.com/memgraph/memgraph/pull/3291)
- Updated telemetry to include anonymous data for role counts and label/node
distributions. [#3424](https://github.com/memgraph/memgraph/pull/3424)
- Added a caching mechanism for regexes. Queries that use regexes inside the
where clause (=~ operator) should be significantly faster.
[#3468](https://github.com/memgraph/memgraph/pull/3468)
- Reduced size of PropertyValue, meaning less memory is used during a query
[#3473](https://github.com/memgraph/memgraph/pull/3473)
- Changed dependency manager used in GraphQL tests to `pnpm`, which is hardened
against any supply-chain vulnerabilities in the npm ecosystem.
[#3501](https://github.com/memgraph/memgraph/pull/3501)
- Strengthened database security by adding a `SECURITY` clause to triggers,
allowing administrators to define whether a trigger executes with the
permissions of the user who created it (`DEFINER`) or the user who triggered the
event (`INVOKER`). This provides granular control over access rights and
prevents unauthorized data modifications during automated workflows.
[#3503](https://github.com/memgraph/memgraph/pull/3503)
- Merged MAGE and mgcxx repos into the memgraph repo. From the usage or
packaging perspective, everything stayed the same. From the internal development
perspective, it should be easier to develop memgraph because everything is
centralized now. [#3574](https://github.com/memgraph/memgraph/pull/3574)
- Boosted performance during snapshot and WAL file processing by optimizing
buffer reuse. By eliminating unnecessary buffer discards when data is already
within the loaded range, users will experience more efficient file loading and
faster system recovery times.
[#3591](https://github.com/memgraph/memgraph/pull/3591)
- Simplified disaster recovery and cloud integration by enabling the `RECOVER
SNAPSHOT` query to pull data directly from S3, HTTP, HTTPS, or FTP sources. This
eliminates the need for manual staging of snapshot files and supports flexible
AWS authentication via config maps, environment variables, or run-time settings.
[#3596](https://github.com/memgraph/memgraph/pull/3596)
- Added two new flags to control RocksDB diagnostic:
`--storage-rocksdb-info-log-level` and
`--storage-rocksdb-enable-thread-tracking`. It's possible to reduce the
gettimeofday/gettid syscalls overhad from RocksDB making Memgraph faster in
cases when that overhead is visible under your workload.
[#3609](https://github.com/memgraph/memgraph/pull/3609)
- Improved system availability during schema modifications by allowing read
operations to run concurrently with constraint creation and removal. This change
replaces the requirement for UNIQUE access with READ_ONLY access, preventing
read queries from being blocked during these operations.
[#3615](https://github.com/memgraph/memgraph/pull/3615)
- Improved concurrency for snapshot creation by switching from WRITE to READ
access. Snapshots no longer block READ_ONLY operations (e.g., creating
constraints), allowing them to execute simultaneously.
[#3625](https://github.com/memgraph/memgraph/pull/3625)
- Integrated [mgcxx](https://github.com/memgraph/mgcxx) into the Memgraph repo
to simplify development and reduce build complexity.
[#3636](https://github.com/memgraph/memgraph/pull/3636)
- Upgraded cuGraph integration from RAPIDS 22.02 / CUDA 11.5 to RAPIDS 25.12 /
CUDA 13.1, bringing GPU-accelerated graph algorithms up to date with current
NVIDIA tooling. [#3709](https://github.com/memgraph/memgraph/pull/3709)
...
[#3746](https://github.com/memgraph/memgraph/pull/3746)
- Fixed performance regression for vector index creation and import by disabling
SimSIMD for the vector index metric and simplifying index storage to a single
map keyed by index ID, so vector index build and import are faster again while
keeping SIMD benefits. [#3754](https://github.com/memgraph/memgraph/pull/3754)
- Fixed spelling errors across MAGE codebase and documentation.
[mage#227](https://github.com/memgraph/mage/pull/227)
- Improved error messages for `LOAD PARQUET`, `LOAD JSONL`, `LOAD CSV`, and
remote snapshot loading so users can diagnose load failures more easily.
[#3758](https://github.com/memgraph/memgraph/pull/3758)
- Memgraph packages now include the latest `mgconsole` binary. See [mgconsole
v1.5.0](https://github.com/memgraph/mgconsole/releases/tag/v1.5.0) for
improvements.  [#3764](https://github.com/memgraph/memgraph/pull/3764)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Comparisons of incompatible types (e.g., string vs number) now return null
instead of raising an error, allowing incompatible comparisons to be captured
and handled in Cypher rather than causing queries to abort.
[#2974](https://github.com/memgraph/memgraph/pull/2974)
- Fixed replica recovery using an incorrect snapshot timestamp, which could
cause WAL files to be skipped during the recovery process. The system now
correctly uses the durable timestamp instead of the start timestamp when
creating recovery steps for replicas.
[#3431](https://github.com/memgraph/memgraph/pull/3431)
- Analytical mode now enforces stricter safety rules. Users can no longer switch
to analytical mode if constraints exist, create constraints or text indexes in
analytical mode, and a snapshot is automatically created when switching back to
transactional mode. These changes prevent unsafe or corrupted states caused by
unsupported analytical-mode operations.
[#3461](https://github.com/memgraph/memgraph/pull/3461)
- Fixed bug where `CALL { RETURN 1 AS x } RETURN *;` returned an empty set
instead of the subquery result. `RETURN *` now correctly includes variables from
`CALL` subqueries. [#3481](https://github.com/memgraph/memgraph/pull/3481)
- Removed manual clean-up of composite and nested indices in end-to-end tests.
Manual deletion of such indices are no longer needed, since`GQLAlchemy` now
handles these automatically.
[#3482](https://github.com/memgraph/memgraph/pull/3482)
- Fixed data races in vertex edge list access, gatekeeper notification timing,
and signal handler by adding proper locking and removing non-async-signal-safe
operations. [#3484](https://github.com/memgraph/memgraph/pull/3484)
- Fixed non-repeatable reads in strict sync replication mode. Transactions on
replicas now correctly maintain read consistency during two-phase commit.
[#3504](https://github.com/memgraph/memgraph/pull/3504)
- Fixed various unlikely crashes related to concurrency and thread-safety.
[#3509](https://github.com/memgraph/memgraph/pull/3509)
- Accessing properties of a deleted node/relationship (e.g., `MATCH (n) DETACH
DELETE n RETURN n {.*};`) now raises an error instead of returning invalid data.
If you need properties from deleted objects, capture them before deleting.
[#3522](https://github.com/memgraph/memgraph/pull/3522)
- Fixed triggers not checking label-based access control. Users should expect
appropriate access controls under triggers.
[#3569](https://github.com/memgraph/memgraph/pull/3569)
- Increased cluster stability during role transitions by ensuring smooth
demotion of the main instance (even during a long-running write transaction).
The system now waits for all active write transactions to finish before
finalizing a demotion, preventing GID (Global ID) overlaps and ensuring data
consistency even when handling long-running transactions.
[#3594](https://github.com/memgraph/memgraph/pull/3594)
- Reduced memory overhead on strict sync replicas by ensuring transaction deltas
are released immediately upon completion. This optimization prevents temporary
memory spikes and ensures more stable, predictable resource usage by bypassing
the delay of the standard garbage collection cycle.
[#3595](https://github.com/memgraph/memgraph/pull/3595)
- Fixed potential crash caused by recursive malloc during memory allocation
hooks. TLS allocation is now forced at startup to prevent this issue.
[#3601](https://github.com/memgraph/memgraph/pull/3601)
- Optimized disk space usage by ensuring aborted snapshots are deleted
immediately upon failure. This eliminates storage "pollution" and provides more
efficient, predictable disk management.
[#3602](https://github.com/memgraph/memgraph/pull/3602)
- Fixed potential memory leak during network resolution failure. Users can
expect that the number of opened files won't grow even in extreme cases when
Memgraph reports system errors during network resolution.
[#3620](https://github.com/memgraph/memgraph/pull/3620)
- Enhanced storage efficiency in HA environments by automatically clearing
`tmp` directories on restart and deleting WAL files immediately after request
processing. This ensures more predictable disk usage and prevents storage
accumulation from temporary files.
[#3621](https://github.com/memgraph/memgraph/pull/3621)
- Fixed a deadlock issue in strict sync replication where the cached accessor
used in two-phase commit was not properly destroyed when a replica gets
promoted. This prevents deadlocks in scenarios where a replica was demoted,
promoted, and demoted again before the previous commit finished.
[#3628](https://github.com/memgraph/memgraph/pull/3628)
- Fixed race condition when adding log sinks by using thread-safe
`dist_sink_mt`.  Logging is also more efficient with a single mutex instead of
three.  [#3634](https://github.com/memgraph/memgraph/pull/3634)
- Fixed text index cleanup to correctly release all Tantivy resources before
removing index directories. Deleting text indexes no longer fails.
[#3641](https://github.com/memgraph/memgraph/pull/3641)
- Fixed crash when removing properties on type-constrained vertices. Setting a
property to NULL (i.e., removing it) on a vertex with a type constraint no
longer causes an assertion failure. Type constraints enforce the property's type
only when a value is present, they don't require the property to exist.
[#3644](https://github.com/memgraph/memgraph/pull/3644)
- Fixed crash when renaming a non-existing database to its current name.
Renaming a database to the same name now throws an exception instead.
[#3647](https://github.com/memgraph/memgraph/pull/3647)
- Fixed crash caused by corrupted expressions during query planning. Queries
with `OPTIONAL MATCH`, `CALL` subqueries, and `WHERE` clauses containing `AND`
with `id()` no longer crash the server.
[#3649](https://github.com/memgraph/memgraph/pull/3649)
- Fixed an issue where the `migrate` module would fail when returning more than
1000 results. [#3706](https://github.com/memgraph/memgraph/pull/3706),
[mage#711](https://github.com/memgraph/mage/pull/711)
- Fixed `path.subgraph_all` to properly expand beyond the start node when
`filterStartNode == false`, even if the start node doesn't meet `labelFilter`
criteria. Negative values for `maxHops` are now ignored.
[#3707](https://github.com/memgraph/memgraph/pull/3707),
[mage#613](https://github.com/memgraph/mage/pull/613)
- Fixed an issue where snapshot files could be unintentionally deleted during
normal snapshot creation due to an incorrect abort check. This could result in a
“Snapshot creation successful” message even though no snapshot file was actually
written to disk. Snapshot creation is now reliable and only deletes files when
an abort is explicitly requested.
[#3711](https://github.com/memgraph/memgraph/pull/3711)
- Improved index selection to prefer indices that satisfy more query filters,
reducing post-filtering overhead. Composite indices are now correctly chosen
when they provide better selectivity.
[#3712](https://github.com/memgraph/memgraph/pull/3712)
- Fixed segfault on shutdown when running many databases (500+). Global
replication clients are now properly shut down before databases are destroyed.
[#3713](https://github.com/memgraph/memgraph/pull/3713)
- Improved snapshot and WAL retention policy. WAL files with incorrect UUIDs are
now deleted, and corrupted or incomplete snapshots are treated like any other
snapshot, keeping only the latest `--storage-snapshot-retention-count` files
regardless of validity. [#3718](https://github.com/memgraph/memgraph/pull/3718)
- Fixed coordinator crash when adding itself as a coordinator instance fails.
The system now throws an exception instead of terminating, preserving the valid
coordinator state. [#3727](https://github.com/memgraph/memgraph/pull/3727)
- Fixed `--also-log-to-stderr` flag not being respected during Memgraph
initialization. Users can now see startup logs and crash messages in the
terminal when this flag is enabled.
[#3728](https://github.com/memgraph/memgraph/pull/3728)
- Enterprise license key and organisation name set through environment variables
or command-line flags are now visible in `SHOW DATABASE SETTINGS` output. This
helps users verify their license configuration is correctly applied.
[#3730](https://github.com/memgraph/memgraph/pull/3730)
- Fixed a potential crash when accessing server endpoint before the server was
fully initialized. [#3743](https://github.com/memgraph/memgraph/pull/3743)
- Fixed data instance registration when a main instance already exists. The
coordinator now correctly sends registration information to new data instances.
[#3744](https://github.com/memgraph/memgraph/pull/3744)
- Aligned Bolt exception handling across all query paths. Replication failures
now always surface as `ClientError` instead of varying between `ClientError` and
`TransientError` depending on transaction mode, so clients can handle
replication errors consistently.
[#3747](https://github.com/memgraph/memgraph/pull/3747)
- Stale responses from auth modules are now ignored. Previously, stale data
could be read from the module depending on communication errors, potentially
leading to logging in as the wrong user. External auth modules must be updated
to support the new message ID protocol.
[#3755](https://github.com/memgraph/memgraph/pull/3755)
- Fixed boolean arguments being negated when executing Rust query module
procedures. [mage#447](https://github.com/memgraph/mage/pull/447)
- `toFloat` and `toFloatList` now accept Boolean values, converting `true` to
`1.0` and `false` to `0.0`, so expressions and pipelines that expect floats no
longer fail when given Booleans.
[#3759](https://github.com/memgraph/memgraph/pull/3759)
- The MAGE Python embeddings module now correctly handles `excluded_properties`
when Cypher passes it as a tuple. Users can rely on property exclusion when
generating embeddings without encountering runtime errors.
[#3763](https://github.com/memgraph/memgraph/pull/3763)
- Fixed deadlock when using `LIMIT` in queries that perform `LOAD PARQUET` or
other load operations. Load queries with `LIMIT` now complete without hanging.
[#3758](https://github.com/memgraph/memgraph/pull/3758)
- Fixed `VectorIndexId` to `mgp_value` type conversion so query modules and
procedures that work with vector indexes receive and return correct values
instead of incorrect or failing conversions.
[#3767](https://github.com/memgraph/memgraph/pull/3767)
- Fixed `roles()` and `username()` returning wrong values when used inside
triggers with `SECURITY DEFINER`. Triggers now report the definer's identity
...
- Changed: Uses documented grammar for fine-grained edge permissions, such as
  `GRANT READ ON EDGES OF TYPE :FOLLOWS TO alice` or
  `GRANT UPDATE ON EDGES OF TYPE * TO bob`. Due to the new syntax, this is a
  breaking change.[#3457](https://github.com/memgraph/memgraph/pull/3457)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Forced the GC to acquire a shared lock in analytical mode, preventing
  unnecessary blocking of other transactions and improving responsiveness.
  A new runtime flag, `storage-gc-aggressive`, allows users to opt into
  ...
  [#3448](https://github.com/memgraph/memgraph/pull/3448)
- Edge creation is refactored to ensure skip list GC doesn't run while vertex
  ...
  performance. [#3441](https://github.com/memgraph/memgraph/pull/3441)
- Improved skip list GC performance by optimizing how deleted objects are
  tracked. This results in faster cleanup and reduced overhead during intensive
  write workloads. [#3421](https://github.com/memgraph/memgraph/pull/3421)

### MAGE v3.7.1 - November 25th, 2025

- Version bump to v3.7.1 to match the latest Memgraph version.

### Lab v3.7.1 - December 19th, 2025

<LabReleasesClient version="3.7.1" />

### Lab v3.7.0 - December 5th, 2025

<LabReleasesClient version="3.7.0" />

### Memgraph v3.7.0 - November 19th, 2025

{<h4 className="custom-header">⚠️ Breaking changes</h4>}

- IMPORTANT: Changed fine-grained access configuration syntax and semantics to be more
  flexible. Permissions can be set on groups of labels matching any or exactly
  what is set in the `GRANT` statement. `CREATE`, `READ`, `UPDATE`, and
  `DELETE` permissions are no longer organised in a hierarchy and can be
  ...
  permissions based on the new rules. To migrate, please follow the [MIGRATION
  GUIDE](https://memgraph.com/docs/database-management/authentication-and-authorization/mlbac-migration-guide).
  [#3343](https://github.com/memgraph/memgraph/pull/3343)
- Changed the way Memgraph is built (only relevant if you are building Memgraph
  from source) and dropped support for Debian 11.
  [#2691](https://github.com/memgraph/memgraph/pull/2691)

{<h4 className="custom-header">✨ New features</h4>}

- Enable InList optimisation (caching a set for O(1) lookup) on more
  expressions. Now `x IN range(0,1000)` and other expressions benefit from this
  optimisation. [#3347](https://github.com/memgraph/memgraph/pull/3347)
- Added support for loading Parquet files directly from the local filesystem or
  S3-like storage. Users can now use `LOAD PARQUET FROM /path/to/file.parquet;`
  ...
  [#3356](https://github.com/memgraph/memgraph/pull/3356)
  [#3378](https://github.com/memgraph/memgraph/pull/3378)
  [#3395](https://github.com/memgraph/memgraph/pull/3395)
- Allow the use of a self-signed identity server when authenticating via custom
  SSO. Users can now add their identity server's certificates via the
  `MEMGRAPH_SSO_CUSTOM_OIDC_EXTRA_CA_CERTS` env variable.
  [#3389](https://github.com/memgraph/memgraph/pull/3389)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Added new toolchain (v7) with OpenMP, updated `clang`, `cmake`, `boost` and
  ...
  [#2691](https://github.com/memgraph/memgraph/pull/2691)
- Expanded the list of supported Linux distributions by modifying bash scripts
  ...
  supported issue. [#3231](https://github.com/memgraph/memgraph/pull/3231)
- Introduced more precise and efficient internal list representations instead
  ...
  [#3282](https://github.com/memgraph/memgraph/pull/3282)
- Changed fine-grained access configuration syntax and semantics to be more
  flexible. Permissions can be set on groups of labels matching any or exactly
  what is set in the `GRANT` statement. `CREATE`, `READ`, `UPDATE`, and
  `DELETE` permissions are no longer organised in a hierarchy and can be
  granted or revoked independently. This is a breaking change, and the
  ...
  [#3343](https://github.com/memgraph/memgraph/pull/3343)
- Added `relwithdebinfo` builds which use `malloc` instead of `jemalloc`; also
  included `heaptrack` in all `relwithdebinfo` Docker images.
  [#3346](https://github.com/memgraph/memgraph/pull/3346)
- Improved `RECOVER SNAPSHOT` by not backing up snapshot and WAL files
  ...
  [#3354](https://github.com/memgraph/memgraph/pull/3354)
  [#3355](https://github.com/memgraph/memgraph/pull/3355)
- Optimize how the WAL file gets finalised. If the replication process isn't
  ...
  data. [#3381](https://github.com/memgraph/memgraph/pull/3381)
- Vector index recovery performance has been improved by fixing an issue where
  ...
  required. [#3390](https://github.com/memgraph/memgraph/pull/3390)
- Added a script to `RelWithDebInfo` Docker images so that users can easily run
  Memgraph using `gdb`, save a core dump and output a backtrace to the terminal
  by simply overriding the entry point.
  [#3428](https://github.com/memgraph/memgraph/pull/3428)

{<h4 className="custom-header">🐞 Bug fixes</h4>}

- Fixed an issue where the Cartesian-to-HashJoin rewrite produced incorrect
  ...
  [#3337](https://github.com/memgraph/memgraph/pull/3337)
- Fixed incorrect list comprehension aggregation logic that caused crashes in
  certain queries. List comprehension queries now execute correctly without
  errors. No user action is required.
  [#3338](https://github.com/memgraph/memgraph/pull/3338)
- Fixing Memgraph running under k8s. It was impossible to move the file from
  `/tmp` to `/var/lib/memgraph` when Memgraph is deployed on K8s because `/tmp`
  and `/var/lib/memgraph` were on different partitions. Data directory
  (`/var/lib/memgraph`) is used through PVC, while /tmp is locally attached
  storage. The replica instance will now save incoming WAL files and snapshots
  to the `tmp` folder, located in the data directory. Previously, it was using
  `/tmp`, but then an issue arose when attempting to move files from `/tmp` to
  the data directory because the directories were located on different
  partitions. [#3359](https://github.com/memgraph/memgraph/pull/3359)
- Fixed certain list comprehensions that were previously not possible by fixing
  the input of the operator tree. Queries like `RETURN [()-->()|1];` should
  work now. [#3365](https://github.com/memgraph/memgraph/pull/3365)
- Fixed dropping of the global edge index, which wasn't properly working,
  causing a replica to crash. Users can now expect normal working when dropping
  the global edge index.
  [#3382](https://github.com/memgraph/memgraph/pull/3382)
- Fixed the case when edge indices skip deleted edges regardless of transaction
  ...
  [#3383](https://github.com/memgraph/memgraph/pull/3383)
- Fixed an issue of planner incorrectly discarding edge type filters when using
  global edge property indices `CREATE GLOBAL EDGE INDEX ON :(p);`. Queries
  that use a global edge index no longer ignore edge type filters.
  [#3392](https://github.com/memgraph/memgraph/pull/3392)
- Replica will now send an `InProgressRpc` message when it takes more time to
  make deltas durable. Previously, deltas might have been processed quickly,
  but due to writing to disk, the `MAIN` process thought the replica was down.
  This caused transactions to be aborted in `STRICT SYNC` mode. On the other
  hand, in `SYNC` mode, the transaction would be committed, with the exception
  being thrown, which would confuse the user about what actually happened in
  the system. [#3400](https://github.com/memgraph/memgraph/pull/3400)
- Fixed an issue where `SHOW INDEX INFO` would fail due to the presence of a
  global edge index. [#3411](https://github.com/memgraph/memgraph/pull/3411)
- Fixed an issue with operator generation in EXISTS subqueries when no
  ...
  [#3412](https://github.com/memgraph/memgraph/pull/3412)
- Fixed decoding of the extended property value type for lists was incorrect
  because the list type was decoded multiple times: once in
  `DecodeExpectedPropertyType` and again in `SkipPropertyValue`, resulting in
  an error. [#3429](https://github.com/memgraph/memgraph/pull/3429)
- Fixed server crashing when a transaction completes by catching a
  ...
  [#3439](https://github.com/memgraph/memgraph/pull/3439)

### MAGE v3.7.0 - November 19th, 2025

{<h4 className="custom-header">✨ New features</h4>}

- Added `collections.frequencies_as_map` function to count element occurrences
  in a list. [#628](https://github.com/memgraph/mage/pull/628)

{<h4 className="custom-header">🛠️ Improvements</h4>}

- Updated the toolchain used to build MAGE. The usages should remain the same,
  but please note that if you are building MAGE from source, you must start
  using the latest toolchain (v7).
  [#670](https://github.com/memgraph/mage/pull/670)
- Added `relwithdebinfo` builds that use `malloc` instead of `jemalloc`; also
  included `heaptrack` in all `relwithdebinfo` Docker images.
  [#681](https://github.com/memgraph/mage/pull/681)
- Added an experimental Ubuntu-24.04 DEB package to the daily build for
  installing all MAGE modules.
  [#678](https://github.com/memgraph/mage/pull/678)
- Added a script to `RelWithDebInfo` Docker images so that users can easily run
  Memgraph using `gdb`, save a core dump and output a backtrace to the terminal
  by simply overriding the entry point.
...
### Lab v3.6.2 - October 31st, 2025

<LabReleasesClient version="3.6.2" />
...
- Transitioned from statically linking to dynamically linking `libstdc++`. This
  ...
  breaking issues. [#3061](https://github.com/memgraph/memgraph/pull/3061)
- System queries (auth, replication, multi-tenant) are not targeting the
  ...
  "memgraph", run `SHOW PRIVILEGES FOR user ON DATABASE memgraph`
