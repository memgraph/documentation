...
- Added durability for the coordinator log store and snapshots. The log store
  and snapshots are now stored under the same RocksDB instance with versioning
  in place, and they are serialized and stored to disk. The state manager has
  been upgraded to include versioning and now stores log_id, prev_log_id,
  async_replication details, voted_for, and term. A new flag has been introduced
  to enable storing all this information to disk. Additionally, the
  `ManagementServerConfig` now uses endpoints instead of IP addresses.
