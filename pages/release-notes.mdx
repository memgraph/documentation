...
Make sure to have `--data-recovery-on-startup` flag set to `True` and to 
[backup your data](/database-management/backup-and-restore) before upgrading 
between versions. 
...
the [braking changes in the configuration
...
loss, depending on the configurations of snapshots creation. 

If you think you have experienced data loss and added
`--data-recovery-on-startup=true`, stoping Memgraph creates a new snapshot by
...
`var/lib/memgraph/snapshots` folder, depedning on how long Memgraph was running
and how many times it was restarted.

To prevent the data loss in all cases, before upgraded to Memgraph v2.18.0,
create a
