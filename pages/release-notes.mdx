...
- **`Node Properties()` and `Relationship Properties()` getter from the [C++
  ...
  std::unordered_map to speed execution time. This is a breaking change so check
  your C++ Query modules for possible code changes.**
  [#1131](https://github.com/memgraph/memgraph/pull/1131)

- The import of relationships in [on-disk storage
  mode](/storage-memory-usage#on-disk-transactional-storage-mode) can be
  improved by switching to `READ ONLY VERTEX MODE;`. When not in `READ ONLY
  VERTEX MODE;`, the engine can import around 180 relationships per second. When
  switching to `READ ONLY VERTEX MODE;` the engine can import around 7500
...
- There has been various updates that improved performance: 
...
- In the Enterprise Edition, Memgraph now [exposes system
  metrics](/configuration/exposing-system-metrics via an HTTP endpoint, so
