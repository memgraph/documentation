...
- The deprecated high availability startup flags `--instance-down-timeout-sec`
  and `--instance-health-check-frequency-sec` have been removed. They were
  deprecated in 3.10 and already ignored since then. Configure these values at
  runtime with `SET COORDINATOR SETTING 'instance_down_timeout_sec' TO
  '<value>'` and `SET COORDINATOR SETTING 'instance_health_check_frequency_sec'
  TO '<value>'` on any coordinator instead. Remove these flags from your
  startup configuration, otherwise the coordinator will fail to start.
