...
- The Lab web application can now be started on a
  custom base path using the environment variable
  `BASE_PATH`, which is useful for reverse proxy
  configurations.
- In multi-tenant environments, a search bar is
  added to filter databases by name.
- You can now view the complete Memgraph URI and
  Memgraph version by clicking on the connection
