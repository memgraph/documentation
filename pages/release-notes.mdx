...
- The [`regex`] procedure of the `text` query module enables the search for
  regex subexpression in a text using the [C++
...
- Now you can check information about node labels and edge types by running the
  [schema-related queries](/querying/schema#schema-related-queries) 
...
- Existence and unique [constraints](/fundamentals/constraints) can now also be
  ...
  this behaviour, set the [`storage-parallel-schema-recovery` configuration]
  flag to `true`. As the new configuration flag includes the constraints, the
