...
- Coordinator has become durable, due to additional durability for the RAFT's
logs and snapshots. The logs and snapshots are stored under the same
durability folder `coordinator/raft_data/logs`, whereas coordinator
configuration is in separate folder `coordinator/raft_data/network` which is
made durable for all time. A new flag `--ha-durability` is introduced which
enables turning off durability for logs and snapshots.
