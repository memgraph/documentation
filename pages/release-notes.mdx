...
`coordinator/raft_data/logs` folder. The coordinator configuration is securely
maintained in the `coordinator/raft_data/network` folder, which is kept
constantly durable. A new flag, `--ha-durability`, allows for optional
deactivation of this log and snapshot durability if required.
[#2029](https://github.com/memgraph/memgraph/pull/2029)

- Removed deprecated command-line flags to streamline configuration and improve
  system performance. The following flags are no longer supported:
  `--storage-recover-on-startup`, `--auth-module-create-missing-user`,
  `--auth-module-create-missing-role`, `--auth-module-manage-roles`,
...
- `SHOW INSTANCES` now returns non-resolved IP addresses when using DNS in an HA
cluster. DNS is resolved every time data is streamed using RPC connections,
ensuring seamless reconnections in Kubernetes clusters even if a Pod is
restarted. The `IPFamily` attribute is no longer persisted as part of the
`Endpoint` class. 
[#2051](https://github.com/memgraph/memgraph/pull/2051)

- A new flag, `--nuraft-log-file`, has been added to explicitly manage logs
  coming from NuRaft.
...
configuration options for health pings are valid before starting the Memgraph
...
users can now set the `MEMGRAPH_NURAFT_LOG_FILE` environment variable to
[control the location of NuRaft logs](/clustering/high-availability).
...
`SHOW INSTANCES`. This enhancement provides clearer visibility into the roles of
instances within a high-availability cluster. 
[#2150](https://github.com/memgraph/memgraph/pull/2150)

- Metrics can now be fetched via the HTTP server during runtime if a Memgraph
Enterprise license is enabled.
[#2067](https://github.com/memgraph/memgraph/pull/2067)

- Memgraph now supports Single Sign-On (SSO) modules. This feature allows for
custom authentication schemes, enabling users to log in using user-defined
authentication modules. Upon receiving a Bolt message for authorization,
Memgraph will utilize the specified auth module for the login process. The
`--auth-module-executable` flag has been deprecated and replaced by
`--auth-module-mappings=basic`
[#1990](https://github.com/memgraph/memgraph/pull/1990)

- Users can now connect to Memgraph without specifying a database. If a user has
a default database, they will connect to that database. If not, they will
connect without a database defined, allowing only database-less queries. Users
can also define the database via the client, executing each query against the
specified database. Multi-tenant authentication will be executed upon the first
query execution. 
...
of hops (edges crossed) during graph traversals. This enhancement helps in
...
in `SHOW STORAGE INFO` and the metrics server. This allows users to monitor
memory usage more effectively and helps in optimizing performance.
...
using the SAML protocol. This module validates SAML responses and supports
identity providers such as Microsoft Entra ID and Okta.
[#2049](https://github.com/memgraph/memgraph/pull/2049)

- Added support for Single Sign-On (SSO) using OIDC and OAuth2.0 protocol.
[#2045](https://github.com/memgraph/memgraph/pull/2045)

- The support for the Enum datatype has been added. Enums can be used to enhance
query performance and reduce memory usage compared to string representations.
[#2039](https://github.com/memgraph/memgraph/pull/2039)

- Add support for edge type property index.
[#1964](https://github.com/memgraph/memgraph/pull/1964)

- Added functionality that allows each user to change their own password.
[#2097](https://github.com/memgraph/memgraph/pull/2097)

{/* Development PR not merged yet

- Added functionality to hide sensitive information, specifically passwords, in
query logs. Passwords in queries are now masked with asterisks (`*`).
...
- Added a PostgreSQL migration module to MAGE that enables direct migration from
a PostgreSQL database to Memgraph. This new module streamlines the data import
process by reducing the need for intermediate steps, such as using CSV files.
Users can now migrate tables, including nodes and relationships, directly into
Memgraph. 
