...
- `toInteger()` now converts whole-number strings exactly. Values near the
  ...
  [#4589](https://github.com/memgraph/memgraph/pull/4589)
- `single()` now returns `null` when the predicate is `null` for any element
  (for example `single(x IN [null, 1] WHERE x > 0)`), instead of claiming
  `true`. Wrap with `coalesce(single(...), false)` if you want those rows
  treated as not matching.
  [#4590](https://github.com/memgraph/memgraph/pull/4590)
- `split('', ',')` now returns `['']` rather than an empty list, so a non-null
  string always yields at least one field.
