...
- The [`text` module](/query-modules/python/text.md) simplifies string
...
- LLM prompts returned by the [`llm_util`
  module](/query-modules/python/llm-util.md) now list node and relationship
...
- The new [`ON_DISK_TRANSACTIONAL` storage
  mode](/fundamentals/storage) allows you to store data on disk
...
- You can now check the current [storage
  mode](/fundamentals/storage) and [isolation
...
- You can now choose between [two different storage modes](/fundamentals/storage):
...
- Added support for temporal types, a feature that allows the user to manipulate
  ...
  the [reference guide](/getting-started/data-types)
