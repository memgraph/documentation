...
## Memgraph v2.15.1 - Mar 13, 2024

{<h3> New features and improvements </h3>}

- Add durable text indices and [basic text search](/configuration/text-search)
...
- Introduced enhancements for multi-tenancy support. The new `SHOW DATABASE;`
  ...
  multitenant databases. [#1550](https://github.com/memgraph/memgraph/pull/1550)

- When using replication, flag `--replication-restore-state-on-startup` needs to
  be set to `true`.  [#1707](https://github.com/memgraph/memgraph/pull/1707)
 
- Introduced enhancements for multi-tenancy support. The new `SHOW DATABASE;`
  command reveals the currently used database, returning NULL if none is active.
  `SHOW DATABASES;` has been updated to display only the existing set of
  multitenant (MT) databases.
...
- Added coordinator instance for managing HA cluster.
...
- Added support for restart of replication instances. MAIN can restart as MAIN
  ...
  REPLICA always stays REPLICA.
  [#1672](https://github.com/memgraph/memgraph/pull/1672)

- Added automatic failover from a single coordinator.
  [#1646](https://github.com/memgraph/memgraph/pull/1646)

- Added flags `--raft-server-id` and `--raft-server-port` for creating
  coordinators. [#1687](https://github.com/memgraph/memgraph/pull/1687)

- When using an authentication module, the module needs to return. That role is
  the only way to define privileges for the connection. Improved the
  authentication module with better user and role handling mechanisms. Key
