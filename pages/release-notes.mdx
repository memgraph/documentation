...
- Added `--storage-lock-timeout-sec` to retry acquiring the data-directory file
  lock for a configurable duration before giving up. This prevents spurious
  startup failures when a pod is force-killed and the old lock has not been
  released yet (e.g. on CephFS or other distributed storage with propagation
  delay).
  [#4021](https://github.com/memgraph/memgraph/pull/4021)
