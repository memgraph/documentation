...
- Fixed `convert_c.to_tree()` memory leak. Using `to_tree` will not 
  indefinably increase the memory. 
[#3333](https://github.com/memgraph/memgraph/pull/3333)
...
- Fixed `datetime()` calls with only a `timezone` parameter (e.g., 
  ...
  Breaking: this changes the behaviour of calls like 
  `datetime({timezone:"Europe/Brussels"})`: if you need the previous behaviour,
