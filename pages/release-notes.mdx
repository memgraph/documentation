...
- Added automatic index creation for node labels and edge types. Two new
  configuration flags control this feature for labels and edge-types
  automatically encountered by the database.
  [#1841](https://github.com/memgraph/memgraph/pull/1841)
- Added support for `bolt+routing` when connecting to a cluster of Memgraph instances.
  [#1796](https://github.com/memgraph/memgraph/pull/1796)
...
- The `schema.node_type_properties()` and `schema.rel_type_properties()`
  functions now work as equivalents to Neo4j's functions.
  [#1718](https://github.com/memgraph/memgraph/pull/1718)
- Merging node or edge with null property now raises an error.
