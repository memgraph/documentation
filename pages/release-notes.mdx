...
- High-availability automatic failover became the stable feature 🎉
  ...
  Please visit [high-availability](/clustering/high-availability) page to learn more about
  the feature. [#2479](https://github.com/memgraph/memgraph/pull/2479)
...
- There was a bug in handling double quotes when hiding passwords under logs;
  that's not fixed, so you don't have to worry that the passwords will get
...
- Properly handle failures in WAL replication. An interpreter will now throw an
  ...
  [#2467](https://github.com/memgraph/memgraph/pull/2467)
- The system will now correctly in situations where main gets re-elected as
  main again. [#2472](https://github.com/memgraph/memgraph/pull/2472)
- The `SHOW INSTANCES` query will now run correctly when the leader becomes a
  follower. [#2474](https://github.com/memgraph/memgraph/pull/2474)
- Fix that will not allow a sync replica to be READY while being behind by
  aligning the recovery timestamp check with timestamp modifications.
  [#2476](https://github.com/memgraph/memgraph/pull/2476)
- When a MAIN misses a failover and is selected to be the new main, Raft
  generates a new UUID. Previously, replicas wouldn't update their durability,
  and MAIN wouldn't update its clients. Now, when swapping UUIDs, durability
  and active clients get updated.
  [#2478](https://github.com/memgraph/memgraph/pull/2478)
- Users must set `--coordinator-hostname` when starting a coordinator in HA.
  Before, the database crashed; now, it logs an error message.
  [#2482](https://github.com/memgraph/memgraph/pull/2482)
- Instance-level clients can now safely be closed after committing. Before, a
  promotion request would execute in parallel with committing and would cause
  the locking of a data instance.
  [#2485](https://github.com/memgraph/memgraph/pull/2485)
