...
- You can change the color of nodes and edges in the graph view using
  the color picker.
- You can change the nodes and edges label shown in the graph view to
...
- You can fine tune permissions of the GraphChat to run only read, create,
...
- Query request limit is increased from 100kB to 20MB to enable you to
  run a lot more queries at once from query execution editor.

{<h3> Bug fixes </h3>}

- Query execution editor loads the previous state as expected now.
- You can select a property type for source and target nodes when
  creating edges via CSV import.
- Unselected CSV column in CSV import will not result in CSV error
  anymore.
- Current database in multi-tenancy environment is shown correctly
