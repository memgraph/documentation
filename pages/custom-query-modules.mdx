...
Before considering developing your custom query module, many [graph algorithms and utility
...
For example, Python execution is a bit slower due to the nature of dynamic languages, but it is easier to write and debug. On the other hand, query modules written in C, C++, and Rust are faster but harder to write and debug.

Use each of the guides to ease your development process.

Every single Memgraph installation comes with the example files. For C, C++ query modules there is a `example_c.c`, `example_cpp.cpp` files and
located in the `/usr/lib/memgraph/query_modules/src`, the shared library files based on the mentioned source files are in the `/usr/lib/memgraph/query_modules`, and are called `example_c.so` and `example_cpp.so`.
