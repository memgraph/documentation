...
Before considering devloping your custom query module, many [graph algorithms and utility
...
If they does not suit your needs, you can modifed them or implement a custom query modules. Memgraph supports extending
...
To develope your custom Query module procedures you can follow the guides for each language:
...
For example, Python execution is a bit slower due to nature of dynamic langauge, but it is easier to write and debug. On the other hand, query modules written in C, C++, Rust are faster, but harder to write and debug.

Use each of the guides to ease your developement process.

Every single Memgraph installation comes with the examaple files. For C, C++ query modules there is a `example_c.c`, `example_cpp.cpp` files and
located in the `/usr/lib/memgraph/query_modules/src`, the shared library files based on mentioned source files are in the `/usr/lib/memgraph/query_modules`, and are called `example_c.so` and `example_cpp.so`.

For Python query modules there is a `example.py` file located in the `/usr/lib/memgraph/query_modules/`, there is no shared library files for Python query modules since source is interpreted at runtime.

They were provided as examples of query modules for you to examine and learn how to use them in addtion to linked guides above. 
...
# Modifing the existing query modules 

If some of the query modules from the MAGE library do not suit your needs, but are very similar, you can modify them and make a contribution to Memgraph Mage library.
...
<Tab> 
        ...
        When developing with Mage, take a look at the basis of devloped [algorithms and utility procedures](https://github.com/memgraph/mage) and extend based on that. 

        If you are not extending the existing query modules, you can start from scratch by folowing this guides or use the API docs and knowledge to develop your own query modules. 
