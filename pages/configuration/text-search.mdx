...
Text search is an experimental feature introduced in Memgraph 2.15.1. To use it,
start memgraph with the `--experimental-enabled=text-search' flag.

</Callout>

Text search lets you look up nodes with properties containing specific content.
...
This statement creates a text index named `complianceDocuments` for nodes having
the `Report` label:
...
If one attempts to create an index with the same name twice, the statement will
fail. 

## What is indexed

For a given node, a text index that applies to it stores all of its properties
that have a text-indexable type: `String`, `Integer`, `Float`, or `Boolean`.
...
Querying text indices is accomplished with query procedures.
...
The `text_search.search` procedure looks for text-indexed nodes that match the
given query.
...
The syntax of the `search_query` parameter is available
[here](https://docs.rs/tantivy/latest/tantivy/query/struct.QueryParser.html).
If the query contains property names, prefix them with `data.`.
...
Regex seearch always applies to all properties, i.e. property names should not
be supplied in the search query.
...
aggregation queries contain property names, prefix them with `data.`.
...
Being an experimental feature, text search doesn’t support all usage modalities
in Memgraph yet. Refer to the table below for an overview:
