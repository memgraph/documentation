...
Check the exampe of creating [a replication
...
Also, you can register REPLICA by using DNS names. In that case, the socket address must be a string value as follows:
...
or if you set REPLICA roles using port `10000`, you can define the socket address specifying only the valid domain name, for example:
...
```
docker run -p 7688:7687 -p 3001:3000 memgraph/memgraph-platform
```

REPLICA instance 2:

```
docker run -p 7689:7687 -p 3002:3000 memgraph/memgraph-platform
```

You can connect to each instance with the [Memgraph Lab](/data-visualization)
...
You can connect to each instance using the Memgraph Lab desktop appliation, or any
...
match the [IP addresses on your cluster](/getting-started/install-memgraph/docker#how-to-retrieve-a-docker-container-ip-address).
...
![](/pages/configuration/replication/memgraph-replication-CAP-theorem.png)
...
![](/pages/configuration/replication/memgraph-replication-async-sync.png)
...
![](/pages/configuration/replication/workflow_diagram_data_definition_creation.drawio.png)

**SYNC REPLICA going down when dropping index, uniqueness constraint or existence constraint**

![](/pages/configuration/replication/workflow_diagram_data_definition_dropping.drawio.png)

**SYNC REPLICA going down adding/updating/deleting data**

![](/pages/configuration/replication/workflow_diagram_data_manipulation.drawio.png)
...
![](/pages/configuration/replication/memgraph-replication-async.png)
...
![](/pages/configuration/replication/memgraph-replication-sync-process.png)
...
![](/pages/configuration/replication/memgraph-replication-buffer.png)
...
![](/pages/configuration/replication/memgraph-replication-ids.png)
