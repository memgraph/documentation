...
mode](/configuration/replication#replication-modes) (SYNC or ASYNC) and
...
images](/getting-started/install-memgraph/docker#run-a-memgraph-docker-image) to
...
with the MAIN instance](/configuration/replication#synchronizing-instances).
...
If you need to define additional ports or volumes, check [Docker run options for Memgraph images](/getting-started/install-memgraph/docker#run-a-memgraph-docker-image). 
...
match the [IP addresses on your cluster](/getting-started/install-memgraph/docker#retrieve-a-docker-container-ip-address).
