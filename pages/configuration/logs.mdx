...
<Steps>
    ...
    if you want to ignore messages about internal lab work or too much information
    on storage, indexes, rules, and triggers, you can use this grep command:

    ```bash
    grep -v -E "SHOW STORAGE INFO|APP_INTERNAL_EXEC_VAR|SHOW INDEX INFO|SHOW CONSTRAINT INFO|SHOW TRIGGERS" /var/log/memgraph/memgraph.log
    ```
    ...
    Note: Replace `/var/log/memgraph/memgraph.log` with the actual path to your log
    file if you have configured a different log file location using the `--log-file`
    configuration flag.
