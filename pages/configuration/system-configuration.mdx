...
### Recomended values for the `vm.max_map_count` parameter

Based on available amount of RAM this are the recomended values:
...
Once you run the command from the table you also need to run `sudo sysctl -p`
to reload the system configuration files, including `/etc/sysctl.conf`, into the
current running kernel without the need to reboot the system. 
