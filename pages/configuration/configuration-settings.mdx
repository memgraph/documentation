...
<Tabs items={["Docker", "Docker Compose" , "Linux"]}>
    ...
    <Tab>
        ...
        <Steps>
            ...
            Start Memgraph with a `docker-compose.yml` file.

            {<h3>Find container ID</h3>}

            Open a new terminal and find the `CONTAINER ID` of the Memgraph 
...
### Changing the configuration settings via another configuration file 

Additional configurations can be specified by including another
...
Before the running of Memgraph, you can set the following environment variables to created a default user. 
...
<Tabs items={["Docker", "Docker Compose" , "Linux"]}>
    ...
    ```yaml
        ...
        After you have added the environment variables, restart the instance by
        running the following command:
        ```
        docker restart <CONTAINER_ID>
        ```
        
        To find the `CONTAINER_ID`, open a new terminal and run the following 
        command:
        ```
        docker ps
        ```
