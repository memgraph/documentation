...
Memgraph has a set of configuration options that can be fine-tuned for specific need. 
...
The `memgraph.conf` file is the persistancy for configuration, and the way you
can change the configuration settings depends on the way you are using Memgraph
and configuration settings you want to change.

Most of the configuration changes need to happen before Memgraph is started, 
there is a set of configuration settings can be changed during **runtime**. 
...
<Tabs items={["Docker", "Docker Compose" , "Linux"]}>
    ...
    <Tab>
        Once Memgraph is installed on your Linux machine, and running, if you want to change the configuration settings, you can do the following:
        
        Stop the Memgraph instance, if it is running by running the following command:
        ```bash
        sudo systemctl stop memgraph
        ```
        Open the configuration file with your favorite text editor. The configuration file is available at `/etc/memgraph/memgraph.conf`.

        Modify it, save the changes and start the insance by running the following command:
...
### Changing the configuration settings via file 

Additional configuration can be specified by including another
