...
The `memgraph.conf` file is the persistency for configuration, and the way you
can change the configuration settings depending on the way you are using Memgraph
and configuration settings you want to change.

Most of the configuration changes need to happen before Memgraph is started, 
there is a set of configuration settings that can be changed during **runtime**. 
...
<Tabs items={["Docker", "Docker Compose" , "Linux"]}>
    ...
    <Tab>
        Once Memgraph is installed on your Linux machine. 
