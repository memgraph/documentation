...
If current MAIN is down more than `--instance-down-timeout-sec`, at that point, failover starts. From alive REPLICAs coordinator chooses from one of the REPLICAs a new potential MAIN. 
This instance is potential MAIN as failover can stil fail. Before promoting REPLICA to MAIN, coordinator sends RPC request to each alive REPLICA
...
Once the old MAIN gets back up, coordinator sends RPC request to demote old MAIN to REPLICA. 
...
Each REPLICA has UUID of MAIN it listens. That means if at one point network partition happens and coordinator can't talk to MAIN, but 
MAIN can talk to each REPLICA, we will have network partition. From coordinator's point of view that MAIN is unavailable.
That is the reason why before failover happens to new MAIN, each REPLICA gets new UUID that no current MAIN has. Before failover happens
coordinator generates a new UUID which new MAIN will get to use, once each alive replica acknowledges it received new UUID. In RPC
request to promote itself to MAIN, REPLICA instance also receives the generated UUID.

### Demoting old MAIN to REPLICA
When old MAIN rejoins cluster, the coordinator sends two RPC requests in given order:
1. Demote MAIN to REPLICA
2. Send the UUID of the new MAIN which MAIN demoted to REPLICA should listen to
If the MAIN gets demoted, but RPC request to change UUID of the new MAIN doesn't succeed, the coordinator will retry both actions until success.

### Replication concerns
On failover, the current logic is to choose first available instance from alive REPLICAs to promote to new MAIN. For promotion to MAIN
to successfully happen, the new MAIN figures out if REPLICA is behind (has less up-to-date data) or has data which the new MAIN doesn't have.
If REPLICA has data which MAIN doesn't have, in that case REPLICA is in diverged-from-MAIN state. If at least one REPLICA is in diverged-from-MAIN
