...
own Raft log. Operations saved into the Raft log are those which are related to cluster management. Memgraph doesn't have its implementation of the Raft protocol, 
...
first sends a ROUTE bolt message to any coordinator instance. The coordinator replies to the message by returning the routing table with 3 entries specifying
...
data instance is the only writeable instance, REPLICAs are readable instances and COORDINATORs behave as routers. You can check more details
here [ROUTE msg](https://neo4j.com/docs/bolt/current/bolt/message/#messages-route).
...
At the moment of registration, the instance which you want to unregister must not be MAIN because unregistering MAIN could lead to an inconsistent cluster state. 
...
which network ports are they using for managing cluster state, and whether are they considered alive from the coordinator's
...
From alive REPLICAs coordinator chooses a new potential MAIN. More on details how the new MAIN is chosen in the next chapter.
...
When failover is happening some REPLICAs can be down also. From the list of alive REPLICAs, a new MAIN is chosen. First, the leader coordinator contacts each alive REPLICA to get info about each database's last commit timestamp.
...
To ensure the atomicity of each action, the leader coordinator wraps each of the following actions with locks:
 ...
 - failover procedure
 - force reset of the cluster (described in next chapter)
...
If REPLICA was down at one point, MAIN could have changed. When REPLICA gets back up, it doesn't listen to any MAIN until the coordinator send an RPC request to REPLICA to start listening to MAIN
with the given UUID.

### Replication concerns

#### Force sync of data 
||||||| parent of 4c73dd8 (Add Bolt routing)
