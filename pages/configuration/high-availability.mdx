...
You can start data instance and coordinator instance using either environment flags or configuration flags.
...
Memgraph data instance needs to set --replication-restore-state-on-startup=true and --data-recovery-on-startup=true flags. 
These two flags enable to recover all the necessary data and durability configurations for Memgraph to continue the replication process, 
...
These two flags are tied to the high availability feature and enable the coordinator to connect to the data instance and allow the Memgraph data instance to use the high availability feature.
```
...
Coordinator ID servers as identifiers and the coordinator port is used for synchronization and log replication between coordinators. Coordinator IDs and coordinator ports must be different for all coordinators.

### Env flags

There is additional way to set high availability instances using only environment variables. When setting it up, you need to use set up high availability part for coordinator and/or data instance 
either using only environment variables or configuration flags.

You can't mix and match different configuration flags with enviroment variables for following settings:
...
Here are environment variables you need to use to set data instance using only environment variables:
...
There is additional enviroment variable you can use to set path to file with cypher queries used to start high availability cluster.
Here you can use queries we define in next chapter called User API.

```
export MEMGRAPH_HA_CLUSTER_INIT_QUERIES=<file_path>
```
After coordinator instance is started, Memgraph will run queries one by one from this file to set up high availability cluster.
...
In case MAIN is already set in cluster, replica instance will be also registered on MAIN.
