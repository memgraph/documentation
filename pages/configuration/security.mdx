...
on [managing user privileges](/configuration/security)
...
You can choose between `bcrypt`, `sha256`, and `sha256-multiple` password encryption algorithms. SHA256 offers better performance compared to the more secure but less performant bcrypt. Change the encryption algorithm by setting the [`--password-encryption-algorithm`](/configuration/configuration-settings#other) configuration option to the preferred value.
...
[streams](/data-streams#create-a-stream) and
[triggers](/fundamentals/triggers#owner) are propagated to the
...
found in the [reference guide](/data-streams#create-a-stream).
...
[reference guide](/configuration/security).
...
We could naturally cluster those privileges into groups:
  ...
  * Privilege to manage [Multi-tenant](/configuration/multi-tenancy) databases (`DATABASE`).
...
[appropriate article](/configuration/security) within  our reference
guide.
...
**[Managing user privileges](/configuration/security)**
