...
2. **incrementally index entities** as they get inserted into the database by using triggers. Find more information about triggers in the [reference guide](/fundamentals/triggers) or check how to [set up triggers](/fundamentals/triggers). Essentially, triggers offer a way of executing a specific procedure upon some event.
...
### `index()`
The method is meant to be used in combination with triggers for incrementally indexing incoming data and it shouldn't be called by a user explicitly. Check out our [docs](/fundamentals/triggers) where it is explained how Memgraph handles objects captured by various triggers. 
