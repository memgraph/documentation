...
<Callout type="info">
The algorithm throws an exception if no communities are detected. This can happen when all nodes merge into 
a single community or each node forms its own. If this occurs, try adjusting the `gamma` parameter of the algorithm.
</Callout>
