...
<Cards>
  <Card
    icon={<GitHub />}
    title="Source code"
    href="https://github.com/memgraph/mage/tree/main/cpp/create_module"
  />
</Cards>
...
- `properties: Map` ➡ A map with key/value pairs to be added as new node's properties.
...
- `labels: List[string]` ➡ A list with labels of the new nodes.
- `properties: List[Map]` ➡ A list of property maps for new nodes, for each map, a separate node is created.
...
Removes all the properties of the given node(s). Input node(s) can be a single
node, node ID, or a list of nodes and node IDs. Otherwise, an exception is
thrown.

{<h4> Input: </h4>}

- `nodes: any` ➡ Input node(s) - an be a node, node's ID, or a list of nodes and IDs.
- `list_keys: List[string]` ➡ A list of properties which are to be removed.
...
- `nodes: Any` ➡ Input node(s) - an be a node, node's ID, or a list of nodes and IDs.
- `labels: List[string]` ➡ A list of labels to be removed (if exist) from the nodes(s).

{<h4> Output: </h4>}

- `nodes: Node` ➡ Node(s) without the provided labels.
...
MATCH (:Person)-[friends:Friend]->(:Person) CALL create.set_rel_properties(friends, ["really", "until"], [true, "forever"])
YIELD relationship
RETURN relationship;
```
...
MATCH (:Person)-[friends:Friend]->(:Person) CALL create.remove_rel_properties(friends, ["until"])
YIELD relationship 
RETURN relationship;
```
