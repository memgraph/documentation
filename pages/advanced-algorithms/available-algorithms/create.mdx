...
- `label: List[string]` ➡ list of all the labels to be added to the new node.
- `props: Map` ➡ map with key/value pairs to be added as new node's properties.
...
- `label: List[string]` ➡ the list with labels of the new nodes.
- `props: List[Map]` ➡ the list of property maps for new nodes, for each map, a separate node is created.
...
- `nodes: any` ➡ input node(s). Can be a node, node's ID, or a list of nodes and IDs.
- `keys: List[string]` ➡ list of properties which are to be removed.
...
- `label: List[string]` ➡ list of labels to be removed (if exist) from the nodes(s).
...
MATCH (p:Person) CALL create.remove_labels(p, ["Student", "Engineer"]) YIELD node RETURN node;
```
