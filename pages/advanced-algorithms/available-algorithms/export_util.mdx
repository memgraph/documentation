...
The procedure exporst query results to a CSV file. 
...
- `config: Map` ➡ The map of the configuration with the following keys:
    ...
    - `separateHeader: boolean (default=Flase)` ➡ Indicates whether the CSV file header should be separated into its own file. If `True`, a file named `header.csv` will be created in the specified or default directory.
...
{<h4> Usage without configurations: </h4>}
...
{<h4> Usage with configurations: </h4>}
