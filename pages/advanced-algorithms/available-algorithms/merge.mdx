...
The function provides a more flexible way of merging nodes than Cypher’s `MERGE` clause, by
...
Now, use the `merge.node()` procedure to either merge or create an `Alien` node with the name `Zumbi`. If that's a newly created node, the
`age` property will be 30, and if it's a merged node, `age` will be 40. Additionally, the `galaxy` property will be used as an identifying property.

When using `merge.node()` procedure or `MERGE` clause, if there is no match in the graph, and a node is created, it will be created with identifying properties
