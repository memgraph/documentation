...
| --- | --- |
| `sum()`, `sum_longs()`, `avg()`, `min()`, `max()`, `to_set()`, `pairs()`, `index_of()` | `null` |
...
- `index_of()` returns `null` for a `NULL` search value, and `-1` for a search value that holds a `NULL` at any depth (`NULL = x` is never true). A `NULL` list element matches nothing.
...
Returns the position of the first element in a list that is equal to an input
value, compared with `=`.

<Callout type="info">
This function is equivalent to Cypher's **coll.indexOf**.
</Callout>

{<h4 className="custom-header"> Input: </h4>}

- `coll: List[Any]` ➡ The input list searched for the value.
- `value: Any` ➡ The input value searched in the list.

{<h4 className="custom-header"> Output: </h4>}

- `integer` ➡ The zero-based position of the first element equal to `value`,
  or `-1` if no element is.

{<h4 className="custom-header"> Usage: </h4>}

The following query will return the position of the first `5` in the list:

```cypher
RETURN collections.index_of([1, 5, 3, 5], 5) AS index;
```
