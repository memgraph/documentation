...
Keep in mind number of hops through out the query could be different in different algorithms, due to due to differences in the implementation of different operators.
Also, Memgraph uses two different operators for bfs due to optimization reasons and even with them the number of hops can differ for the same paths.
...
To simplify expansion process, we will create an index:

```cypher
CREATE INDEX ON :Node(name);
```

Now, let's try to find path between Node A and Node C using the BFS.
Running:
...
in scenarios where the cost or distance between nodes varies, such as TO networks 
