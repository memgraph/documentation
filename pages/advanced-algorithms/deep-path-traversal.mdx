...
- **Predefined nodes**: Both source and target nodes must be matched first using a `WITH` clause
- **No filter lambdas**: K shortest paths does not support user-defined filtering during expansion
