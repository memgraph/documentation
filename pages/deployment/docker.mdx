...
Additionaly, here are some useful facts about Docker volumes:
...
- Two containers can use the same volume if they're not running in the same
  time: you can run a new Memgraph instance with already used volume if you
  first stopped previously running instance. This is useful when you want to
  reuse same data for two instances.
- Using named volumes is the easiest way to migrate date between different containers.
- With named volumes, you can save files locally by inspecting the volume and
  ...
  especially easy. Docker volumes can be inspected even when the container is
  down.
