...
Create a user in the running Memgraph instance by running the following Cypher query:

```cypher
CREATE USER user_name [IDENTIFIED BY 'password'];
```

Still, the above is not ideal if you want to limit the access to the database on
the first run. To achieve that, set `MEMGRAPH_USER` and `MEMGRAPH_PASSWORD`
...
Here is an example of setting up a replication cluster with two instances:
