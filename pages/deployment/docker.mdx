...
`7687` and `7444`. The port `7687` is used to connect to the database and query it via Bolt
...
In the community verion of Memgraph, you can create a user and its password for security. 
...
To set up SSL encryption you need to own a SSL certificate, configure the server
and enable SSL connection. To configure the server you need to provide public
and private key to your Memgraph instance. Let's suppose that public key is
stored within `cert.pem` and private key within `key.pem` file.

Here are the steps to configure Memgraph instance so it includes SSL certificate:
...
<Steps>
    ...
    {<h3>Copy the SSL certificate files</h3>}
    ```
...
To connect to Memgraph with SSL encryption, you need to enable it from the client side.
Read more about it on the [SSL encryption reference guide](/database-management/ssl-encryption#enable-ssl-connection). 
...
Setting up replication basically means running a couple of Memgraph instances,
...
If you have troubles connecting, check your firewall settings.
...
### Set up a high-available cluster 

In the Enterprise version of Memgraph, you can set up high-available cluster. To
do that, each instance of Memgraph needs to be started with the
`--experimental-enabled=high-availability` flag.

This replication setup is a bit more complicated, and the typical Memgraph
high-available cluster consists of three data instances (one MAIN and two
REPLICAS) and three coordinator instances.

Besides --experimental-enabled flag, coordinator instances need to have --coordinator-id, --coordinator-port
Flags --coordinator-id and --coordinator-port need to be unique and specified on coordinator instances. They will cause the creation of a Raft server that coordinator instances use for communication. 

Other  instances need to have --management-port (Setting a flag will create an RPC server on data instances capable of responding to the coordinator's RPC messages.) and --replication-restore-state-on-startup
--replication-restore-state-on-startup=true 

--bolt-port

Hence, to spin it up, start all insat
