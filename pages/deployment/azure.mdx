...
If you are running Memgraph in `IN_MEMORY_TRANSACTIONAL` (default mode) or in
`IN_MEMORY_ANALYTICAL` storage mode, all data is stored in RAM. That means it
would be good to first calculate [how much memory you will
...
ports for the replication and clustering should be open as well.
