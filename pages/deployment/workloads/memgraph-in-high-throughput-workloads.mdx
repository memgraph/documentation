...
For high-throughput workloads, it's also recommended to set the database setting `storage-gc-aggressive` to `false` to prevent garbage collection
from blocking other transactions. The default value is `false`, so no action is required if this setting hasn't been
changed.
