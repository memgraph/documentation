...
Once the replica instances are in `REPLICA` role, you can connect them to each other by 
running the following query from the MAIN instance:
...
by `--log-file` command. They are typicaly stored in the format
`memgraph_yer-month-day.log`. 
...
If you are experiencing some issues or you have troubles setting up the Memgraph
instance, conside setting the log level to `TRACE` to get more information about
the issue.

The best way to monitor logs is to attach the logs directly to your teriminal as
you are debugging the issue. You can do that by running the following command:
