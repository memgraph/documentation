...
calling the function `duration()`. The function also accepts `null` as an argument, in which case it returns `null`.
...
calling the function `Date()`. The function also accepts `null` as an argument, in which case it returns `null`.
...
calling the function `localTime()`. The function also accepts `null` as an argument, in which case it returns `null`.
...
You can create a property of temporal type `LocalDateTime` from a string or map by calling the function `localDateTime()`. The function also accepts `null` as an argument, in which case it returns `null`.
...
calling the `datetime()` function. The function also accepts `null` as an argument, in which case it returns `null`.
