...
Points can be created with the [point function](#TODO).
...
1. **Geographic CRS**:  
   - Models points on the Earth’s surface using longitude and latitude based on the [WGS84](https://epsg.io/4326) system.


2. **Cartesian CRS**:  
   - Models points in a Euclidean space using standard Cartesian coordinates.
...
- **Cartesian 2D**
  ...
  - A 2D Cartesian point is defined by its **x** and **y** coordinates. When these values are provided, the point is assumed to be in a two-dimensional Cartesian system.

  - This CRS can be designated using either the name `'cartesian'` or the SRID `7203`, as described in [LINK_TO_TABLE](#).

- **Cartesian 3D**

  - A 3D Cartesian point includes **x**, **y**, and **z** coordinates. When these values are specified, the point resides in a three-dimensional Cartesian system.

  - This CRS can be identified with either the name `'cartesian-3d'` or the SRID `9157`, as described in [LINK_TO_TABLE](#).
