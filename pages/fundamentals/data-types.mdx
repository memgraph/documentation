...
[point function](/querying/functions#spatial). For fast queries points can
...
meters. Longitude has to be in the [-180, 180] and latitude in the [-90, 90]
range. Height can be any `Float` value.
...
Specifying this CRS can be done using either the name `'wgs-84'` or the SRID `4326` as described [HERE](/fundamentals/data-types#point-types).
...
Specifying this CRS can be done using either the name `'wgs-84-3d'` or the SRID `4979` as described [HERE](/fundamentals/data-types#point-types).

### Cartesian

Two types of Cartesian Coordinate Reference Systems (CRS) are supported, defining points in a flat, Euclidean space:
  - **Cartesian 2D** point is defined by its **x** and **y** coordinates. When these values are provided, the point is assumed to be in a two-dimensional Cartesian system. This CRS can be designated using either the name `'cartesian'` or the SRID `7203`, as described [HERE](/fundamentals/data-types#point-types).
  - **Cartesian 3D** point includes **x**, **y**, and **z** coordinates. When these values are specified, the point resides in a three-dimensional Cartesian system. This CRS can be identified with either the name `'cartesian-3d'` or the SRID `9157`, as described in [HERE](/fundamentals/data-types#point-types).
...
### Examples on how to create spatial points?

```cypher
RETURN point({x:1, y:2, srid:7203});
RETURN point({x:1, y:2, z:3, crs:'wgs-84-3d'});
```
