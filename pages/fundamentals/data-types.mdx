...
Point is a spatial data type consisting of 2D or 3D locations in the Cartesian or WGS84 system.
The coordinates of the location are stored as a 64-bit `Float` numbers. 
Each point type has an associated coordinate reference system (CRS) and a spatial reference identifier (SRID).
Points can be created with the [point function](/querying/functions#spatial).
For fast queries points can leverage the [spatial index](/fundamentals/indexes#spatial-index).

### Coordinate Reference Systems (CRS)

Memgraph supports four Coordinate Reference Systems (CRS) for spatial data, divided into two categories:

1. **Geographic CRS** models points on the Earth’s surface using longitude and latitude based on the [WGS84](https://epsg.io/4326) system.
2. **Cartesian CRS** models points in a Euclidean space using standard Cartesian coordinates.

<Callout type="info">

**CRS compatibility** - Points in different CRSs cannot be directly compared or converted into one another, even if they share the same type (e.g., both geographic or Cartesian). For example, a 2D point cannot be used in a 3D range query.

</Callout>

### WGS84

A WGS84 point consists of longitude, latitude and height if the point is 3D. 
Longitude and latitude are specified in degrees while height is specified in meters.
Longitude has to be in the [-180, 180] and latitude in the [-90, 90] range. Height can be any `Float` value.

- **WGS 84 2D**  
  A 2D geographic point in the **WGS84 CRS** can be specified in one of two ways:
  ...
  Specifying this CRS can be done using either the name `'wgs-84'` or the SRID `4326` as described in [LINK_TO_TABLE](#).

- **WGS 84 3D**  
  A 3D geographic point in the **WGS84 CRS** can be specified in one of two ways:
  ...
  Specifying this CRS can be done using either the name `'wgs-84-3d'` or the SRID `4979` as described in [LINK_TO_TABLE](#).

### Cartesian

Two types of Cartesian Coordinate Reference Systems (CRS) are supported, defining points in a flat, Euclidean space:

- **Cartesian 2D** point is defined by its **x** and **y** coordinates. When these values are provided, the point is assumed to be in a two-dimensional Cartesian system. This CRS can be designated using either the name `'cartesian'` or the SRID `7203`, as described in [LINK_TO_TABLE](#).

- **Cartesian 3D** point includes **x**, **y**, and **z** coordinates. When these values are specified, the point resides in a three-dimensional Cartesian system. This CRS can be identified with either the name `'cartesian-3d'` or the SRID `9157`, as described in [LINK_TO_TABLE](#).
