...
Based on [Adya's classification of isolation levels](https://pmg.csail.mit.edu/papers/adya-phd.pdf), we can further write:
...
and conclude that Memgraph's support for SNAPSHOT ISOLATION level implicitly includes support for: PL-2, PL-2', PL-2'' and Pl-2+ (consistent view)
