...
This index supports non-blocking creation - reads continue uninterrupted while writes briefly pause. 
...
Memgraph supports (almost) fully concurrent index creation for all skiplist-based indices (label, label-property, composite, edge-type, edge-type property, and global edge property) 
with minimal performance impact.

The three-phase implementation begins with a brief **registration phase** that requires `READ ONLY` access to ensure all pending write transactions complete before index creation begins. 
...
The system maintains full MVCC consistency throughout the process, ensuring transactional integrity. Long-running index operations can be safely cancelled when needed. 
