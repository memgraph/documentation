...
Indexes can be created or deleted using the [`schema.assert()` procedure](/querying/schema#assert) 
...
Primarily, the [`assert()`](querying/schema#assert) procedure is used to define schema, but it's also
