...
The order of properties within the index matter. The two indices created below
are distinct, despite them both being for the same label and properties:
...
This is important because composite indices can also be used on the *leftmost prefix*
of the properties. For the first index, we can obviously match a node based on its
`name`, `age`, and `occupation` properties:
...
must match against `:L` and any of the leftmost prefix of properties (such
as `p0`, or `p0, p1`, or `p0, p1, p2`). Contrawise, an index cannot be used
in a query if it does not use a leftmost prefix.

The following table summarises the cases where a composite index on properties `:L(p0, p1, p2)`
can and can't be used, based on which properties the query matches against:
