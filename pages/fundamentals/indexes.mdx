...
Using the [`storage-enable-automatic-label-index-creation`](/configuration/configuration-settings#storage) and [`storage-enable-automatic-edge-type-index-creation`](/configuration/configuration-settings#storage) flags, it is possible to create label and edge-type indices automatically. Every time the database encounters a label or edge-type that is currently not indexed, it will create an index for that construct.
