...
You can also modify the indexes using the [`schema.assert()` procedure](/querying/schema#assert).
...
### Deleting all indexes and constraints

<Callout type="info">

Ensure you use this feature with caution, as it will remove all schema
constraints and indexes, potentially affecting query performance and data
integrity constraints until they are recreated.

</Callout>

To delete all indexes and constraints at once you can use the
[`schema.assert`](/querying/schema) procedure with specific parameters to drop
all indexes and constraints. The command is as follows:
...
This command effectively removes all existing indexes and constraints from the
database. The parameters passed to `schema.assert` indicate that no indexes or
constraints should be asserted as existing, and the final parameter (`true`)
specifies that all existing indexes and constraints not mentioned (which would
be all, given the empty objects `{}`) should be dropped.

## Automatic index creation
Using the [`storage-automatic-label-index-creation-enabled`](/configuration/configuration-settings#storage) and [`storage-automatic-edge-type-index-creation-enabled`](/configuration/configuration-settings#storage) flags, it is possible to create label and edge-type indices automatically. Every time the database encounters a label or edge-type that is currently not indexed, it will create an index for that construct.
