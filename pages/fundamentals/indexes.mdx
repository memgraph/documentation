...
### Utilizing unique constraints as indexes

By creating unique constraints on one of the properties, a data structure similar to
the index is created internally. Based on that, unique constraints can also be
leveraged as label-property indexes.

```cypher
CREATE CONSTRAINT ON (n:Person) ASSERT EXISTS (n.age);
```

Unique constraints can boost read performance on queries only and only if there is
one property being unique for the node label. Creating the constraint from the example
above will optimize the following query:
...
only by experienced developers and/or database administrators, as the poor index
...
You can [delete all indexes](#delete-all-indexes) or modify them using the [`schema.assert()` procedure](/querying/schema#assert).
...
```cypher
CALL schema.assert({}, {Employee: [["id"], ["email"], ["name", "surname"]]}, {Person: ["name"]}, true) 
...
useful if you need to [delete all constraints](/fundamentals/constraints#delete-all-constraints) or [delete all indexes and constraints](/querying/schema#delete-all-indexes-and-constraints).
