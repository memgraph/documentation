...
Example of query with point.distance()
```cypher
MATCH (node:Label) WHERE point.distance(point({x:1, y:1}), node.property) < 1
RETURN node;
```

Example of queries with point.withinbbox()
```cypher
...
In Memgraph v2.21 point index can only be utilized with the point.distance function and to use it the comparison point
