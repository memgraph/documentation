...
Primarily, the [`assert()`](querying/schema#assert) procedure is used to define
a schema, but it's also useful if you need to [delete all
indexes](/fundamentals/indexes#delete-all-node-indexes) or [delete all indexes
and constraints](/querying/schema#delete-all-indexes-and-constraints).
