...
If the database is used to hold basic information about a person like their name and surname
you can enforce them to be strings by running the following queries:

```cypher
CREATE CONSTRAINT ON (n:Person) ASSERT n.name IS TYPED STRING;
CREATE CONSTRAINT ON (n:Person) ASSERT n.surname IS TYPED STRING;
```
...
| data_type       | Person       | surname         |   STRING    |
+-----------------+--------------+-----------------+-------------|
```
...
DROP CONSTRAINT ON (n:Person) ASSERT n.surname IS TYPED STRING;
```
