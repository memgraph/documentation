...
Do not create two different constraints on the same property.

CREATE (object:Square:Blue {size: 17});
CREATE CONSTRAINT ON (object:Square) ASSERT n.size IS TYPED integer;
CREATE CONSTRAINT ON (object:Blue) ASSERT n.size IS TYPED string;

While this is currently allowed by the software, it would create a scenario where you would not be able to insert data to that property.
