...
<h4 className="custom-header">Snapshots the operating system keeps cached</h4>

After a snapshot has been read or written, the operating system may keep a copy
of it cached in memory. Memgraph can release that copy so the memory goes back
to the database instead:

- `--storage-release-recovered-snapshot-page-cache` releases a snapshot once
  recovery has finished loading it on startup. Enabled by default, as nothing
  reads that file again once the data is in memory.
- `--storage-release-sent-snapshot-page-cache` releases a snapshot once it has
  been sent to a replica. Disabled by default, because a further replica syncing
  from the same snapshot would then have to read it from disk again. Consider
  enabling it if you sync a replica once and want the memory back sooner.
