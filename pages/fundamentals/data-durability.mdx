...
`IN_MEMORY_TRANSACTIONAL` mode supports periodic snapshot creation. The interval can be set at startup via the
`--storage-snapshot-interval` flag or at run-time via the database settings as such:
...
The interval string can by an integer, in which case the defined number is treated as the execution period in seconds.
Another option is to define the interval as a CRON expression. The expression mush define seconds in addition to the usual 5 cron fields. Please note that defining the interval vai a CRON expression is an Enterprise feature.
By setting the value to an empty string, the background process is paused, while any currently active snapshot creation will finish.

In case the database is started in or migrated into `IN_MEMORY_ANALYTICAL` mode, the background thread will pause and no snapshots will be created as long as the mode is active.
The job will continue with the last defined interval when the database get put back into `IN_MEMORY_TRANSACTIONAL`.
