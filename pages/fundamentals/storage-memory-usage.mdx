...
|-------------------------|-----------------------------------------------|
| `name`                  | Database name                                 |
| `tenant_memory_tracked` | Current tracked bytes                         |
| `profile`               | Tenant profile name, or `null`                |
| `tenant_memory_limit`   | Configured limit, or `"unlimited"`            |

Both `SHOW MEMORY INFO` and `SHOW STORAGE INFO ON DATABASE` require the `STATS`
privilege and the Memgraph Enterprise Edition.
