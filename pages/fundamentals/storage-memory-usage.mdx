...
not fit into the RAM. To change the storage mode to any of the in-memory storages,
...
restore from the snapshot file and create a WAL for all new updates, if not
...
[`Delta`](#delta-memory-layout) objects which store the exact versions of data
...
Memgraph on-disk storage supports the `EDGE IMPORT MDOE` to import relationships
