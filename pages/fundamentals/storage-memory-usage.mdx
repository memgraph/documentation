...
By default, an empty instance will restart using in-memory transactional storage
mode, and a non-empty instance in an in-memory analytical storage mode will
revert to the in-memory transactional storage mode. This behavior can be changed
using the `--storage-mode` [configuration
flag](/configuration/configuration-settings). Regardless of what the flag is set
to, a non-empty instance in the on-disk storage mode will not change storage
mode upon restart. 
