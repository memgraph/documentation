...
from intermediary results to allocations made while creating vertices and edges.
...
Memgraph uses a jemalloc allocator to handle all allocations and to track allocations happening 
in system.
...
You can find out the current mmap count of a running Memgraph process using the following command, where $PID is the process ID of the running Memgraph process:
