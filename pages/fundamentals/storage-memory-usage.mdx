...
flag](/configuration/configuration-settings) accordingly. 
...
flag](/configuration/configuration-settings). But, regardless of what the flag
...
But `Deltas` also require a lot of memory (80B per change), especially when
...
Each `Vertex` object has at least **112B** + **80B** for the `Delta` object, in
total, a minimum of **192B**.
...
Each `Edge` object has at least **40B** + **80B** for the `Delta` object, in
total, a minimum of **120B**.
...
$19,148 \times (112\text{B} + 13\text{B} + 16\text{B} + 16\text{B} + 80\text{B}) = 19,148 \times 237\text{B} = 4,538,076\text{B}.$
...
$2584 \times (112\text{B} + 13\text{B} + 12\text{B} + 16\text{B} + 16\text{B} + 80\text{B}) = 2584 \times 249\text{B} = 643,416\text{B}.$

In total, $5,181,492\text{B}$ to store vertices.
...
$682,943 \times (40\text{B}+16\text{B}+16\text{B}+80\text{B}) = 682,943 \times 152\text{B} = 103,807,336\text{B}.$
...
| Amount of RAM  | Command                                                            |
|----------------|--------------------------------------------------------------------|
| 8GB - 32GB     | `echo "vm.max_map_count=262144" | sudo tee -a /etc/sysctl.conf`   |
| 32GB - 64GB    | `echo "vm.max_map_count=524288" | sudo tee -a /etc/sysctl.conf`   |
| 64GB - 128GB   | `echo "vm.max_map_count=1048576" | sudo tee -a /etc/sysctl.conf`  |
| 128GB - 256GB  | `echo "vm.max_map_count=2097152" | sudo tee -a /etc/sysctl.conf`  |
| 256GB - 512GB  | `echo "vm.max_map_count=4194304" | sudo tee -a /etc/sysctl.conf`  |
| 512GB - 1TB    | `echo "vm.max_map_count=8388608" | sudo tee -a /etc/sysctl.conf`  |
| 1TB - 1.5TB    | `echo "vm.max_map_count=12582912" | sudo tee -a /etc/sysctl.conf` |
| 1.5TB - 2TB    | `echo "vm.max_map_count=16777216" | sudo tee -a /etc/sysctl.conf` |
| 2TB - 2.5TB    | `echo "vm.max_map_count=20971520" | sudo tee -a /etc/sysctl.conf` |
| 2.5TB - 3TB    | `echo "vm.max_map_count=25165824" | sudo tee -a /etc/sysctl.conf` |
| 3TB - 3.5TB    | `echo "vm.max_map_count=29360128" | sudo tee -a /etc/sysctl.conf` |
| 3.5TB - 4TB    | `echo "vm.max_map_count=33554432" | sudo tee -a /etc/sysctl.conf` |
