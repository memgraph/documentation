...
Memgraph on-disk storage supports `Edge import mode` to import edges with high throughput.
In this mode user is allowed to create and modify edges only. Any vertex operations are prohibited and will through exception.
...
For file containing Cypher queries (.cypherql), first all vertices should be created, then command `EDGE IMPORT MODE ACTIVE;` specified and then creation of all edges.
Indices can be specified before or after `EDGE IMPORT MODE ACTIVE;`.
We recommend to **create label-property index** on nodes for the best import edges performance.
...
Users are still be able to import a dataset through just one CSV file but in order to get really good performance when ingesting edges, they should separate loading nodes from edges.
Currently, for the best import edges performance we recommend to **not create** any index.
