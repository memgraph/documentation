...
[`--data-recovery-on-startup`](/configuration/configuration-settings#storage),
which defaults to `true`).

When Memgraph starts creating a periodic snapshot, it is not possible to
manually create a snapshot, until the periodic snapshot is created. Snapshot
creation requires a transactional view. This can lead to spikes in memory
consumption when data is being ingested in parallel to snapshot creation. It is
possible to pause the periodic snapshot background job at
[run-time](/database-management/configuration#change-configuration-during-runtime).
This can speed up ingestion time and lower memory usage.
...
At the moment, the on-disk storage **doesn't support replication and
high availability**.

{<h4 className="custom-header">  Data formats </h4>} 
...
{<h5 className="custom-header"> Import data using `LOAD CSV` clause </h5>}

When importing data using a [CSV file](/data-migration/csv), import nodes and
relationships separately. Currently, to import relationships as fast as
...
{<h5 className="custom-header"> Import data using Cypher queries </h5>} 
...
You can also use the [Storage memory calculator](https://memgraph.com/storage-calculator).
...
edge). The [lightweight edges](#lightweight-edges) mode
...
{<h4 className="custom-header"> Index memory layout </h4>}
...
{<h4 className="custom-header"> Properties </h4>}
...
name is on average 10 characters long). Once the average values are included,
the calculation is:
...
Profiles](/database-management/tenant-profiles).
...
### Per-database memory tracking <sup style={{ fontSize: '0.6em', color: '#888' }}>Enterprise</sup>

In [multi-tenant](/database-management/multi-tenancy) deployments, Memgraph Enterprise
tracks memory for each database independently. Each database's allocations are
counted against both the database's own limit and the global instance limit,
ensuring no single database can starve others of resources.

Storage, query execution, and vector index allocations are all attributed to the
...
see [Per-database storage information](/database-management/server-stats#per-database-storage-information).
...
<Callout type="info">
The per-database `query_memory_tracked` field tracks only **runtime
...
Per-database memory limits are enforced via
[tenant profiles](/database-management/tenant-profiles). When a limit is hit,
any query that would allocate memory beyond the cap is rejected — read queries
are not exempt.
...
4. If you do need properties on relationships, enable [lightweight
   edges](#lightweight-edges) by setting the `--storage-light-edge` flag to
   `true` to save roughly **24B per edge**.
5. [Inspect query plans](/querying/best-practices#profiling-queries)
   and optimize them

#### Lightweight edges

When you need [properties on
relationships](/data-modeling/graph-data-model), each relationship is stored as
...
This flag **implies `--storage-properties-on-edges=true`**: if you enable
...
[`id()`](/querying/functions)) can no longer be a direct lookup. Instead:

- With `--storage-enable-edges-metadata=true`, the edge-metadata index locates
  the edge's source vertex, and the lookup only scans that vertex's edges — fast.
- Without edge metadata, Memgraph falls back to scanning every vertex's edges,
  which can be slow on large or high-degree graphs.
...
with [durability](/fundamentals/data-durability) (snapshots and WAL) and
[replication](/clustering/replication). The on-disk snapshot and WAL formats are
...
```
--storage-floating-point-resolution-bits=32
```

With this flag set, storing `3.14159265358979` will be retrieved as approximately `3.1415927` (float precision). Example:
...
// Returns: 3.14159265358979 (with --storage-floating-point-resolution-bits=64)
```

| Value | Precision | Bytes per value | Savings vs default |
|-------|-----------|-----------------|--------------------|
| `64`  | double (full) | 8B          | —                  |
