...
* `IN_MEMORY_TRANSACTIONAL` - the default database storage mode that favors
    strongly-consistent ACID transactions using WAL files and snapshots, but
    requires more time and resources during data import and analysis.
...
### Start Memgraph with specific storage mode

If you know you will need ON_DISK_TRANSACTIONAL or IN_MEMORY_ANALYTICAL storage mode
since the beginning of usage, you can start Memgraph in that storage mode by
specifying `--storage-mode` flag. By default, Memgraph will start in IN_MEMORY_TRANSACTIONAL
storage mode.
...
`WARNING`](/configuration/configuration-settings#other).
...
[`STORAGE_MODE` permission](/configuration/security) to change the storage
mode.

An empty instance will always restart using in-memory transactional storage mode.
...
[`Delta`](#delta-memory-layout) object each time data is changed. Deltas are the
...
setting of the configuration flag `--storage-recover-on-startup`, which defaults
to `true`).
...
[`Delta`](#delta-memory-layout) objects to provides atomicity, consistency,
...
Also, no [write-ahead
logs](/configuration/data-durability-and-backup#write-ahead-logging) are created
in the `IN_MEMORY_ANALYTICAL` mode. When switching back to the
...
[Snapshots](/configuration/data-durability-and-backup#snapshots) capture the
database state and store it on the disk. A snapshot is used to recover the
database upon startup (depending on the setting of the [configuration
...
isolation level also simplifies the query's execution flow since no data is
transferred to the disk until the commit of the transaction.
...
Memgraph on-disk storage supports `Edge import mode` to import edges with high
throughput. In this mode, only edges can be created and modified. Any vertex
operations other than read are prohibited and will throw an exception.
...
#### Import data using LOAD CSV command

When importing data using a CSV file, seperate importing nodes and
relationships. Currently, for the best edges import performance we recommend
...
For files containing Cypher queries (.cypherql), first all vertices should be
created, then command `EDGE IMPORT MODE ACTIVE;` specified and then can the
edges be created. Indices can be specified before or after `EDGE IMPORT MODE
ACTIVE;`. We recommend to [**create label-property
index**](/fundamentals/indexes) on nodes for the best edges import performance.
...
If you want to **estimate** IN_MEMORY_TRASNACTIONAL storage mode memory usage,
...
dataset](http://download.memgraph.com/datasets/marvel-cinematic-universe/cypherl/dataset.cypherl.gz),
...
Run the following query to get information about information about the currently
...
| "memory_usage"                 | 578465792                      |
| "disk_usage"                   | 4131103                        |
| "memory_allocated"             | 17041304                       |
| "allocation_limit"             | 13195796480                    |
...
stats](/configuration/server-stats).
...
4. [Inspect query plans](/querying/performance-optimization#inspecting-queries)
   and optimize them
