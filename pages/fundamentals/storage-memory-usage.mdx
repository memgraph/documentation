...
Snapshot creation requires a transactional view. This can lead to spikes in memory consumption when data is being injested in parallel to snapshot creation.
It is possible to, at run-time, pause the periodic snapshot background job. This can be benefitial to ingestion times and memory usage.
