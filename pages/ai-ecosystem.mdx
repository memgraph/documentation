...
and reasoning (KRR), each focusing on different aspects and strategies of
creating intelligent systems. Usually, these areas overlap and that leads to the
development of more advance AI systems. ne example of such overlap is
**Generative AI (GenAI)**. 
...
GenAI apps. LLMs are trained on massive datasets and they understand and
generate natural language. Still, if you want the LLM to understand your custom
data, then you need to find the best way to transfer that knowledge to the LLM.

One way of doing that is **fine-tuning**, which can be hard to set up, slow to get
it up and running and can be expensive. Also, if the knowledge you want to
transfer is growing, then fine-tuning would have to be a repeatable process and
that just isn't viable.

This is where **retrieval augmented generation (RAG)** came to rescue. RAG
...
It is easy to setup, flexible, scalable, and it supports dynamic knowledge
updates.

Traditional RAG systems are enhancing GenAI, but they often fall short when
retrieving crucial knowledge from **complex datasets**. This is where GraphRAG
excels. **GraphRAG** is a type of RAG that utilizes knowledge graphs and graph
features (e.g., finding communities, analyzing neighborhoods) to enhance
...
for the use cases that utilize [machine learning](#machine-learning). Now, it
also proves to be a great choice to build a [GraphRAG](#graphrag-with-memgraph). 
...
several integrations with popular AI libraries. Below are some of the libraries
integrated with Memgraph:
