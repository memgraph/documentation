...
- **Status**: Experimental. Designed to improve system reliability and uptime, but may still have stability issues. As of now (2024-06-25), the HA feature is in the so called "development-ready" state. Feel free to test it for your production deployment, if it works, it's ok to run in production. Conditions for HA to become production-ready are a few (2-3) production deployments running 24/7 under HA setup. ETA for HA to become production-ready is end of 2024.
...
- **Status**: Experimental. Enhances data replication capabilities with added security and multi-tenancy features. It's planned to make this feature production ready for the v2.20 release.
...
- **Status**: Experimental. Provides text search functionalities but is not guaranteed to work correctly in use cases that involve concurrent transactions and replication. There are no shor-term plans to improve the text-search capabilities. If you are interested in improvements, please reach out.
