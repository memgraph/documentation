...
Use **DENY** when you need to explicitly forbid access (e.g., to the default
"memgraph" database for tenant users). Use **REVOKE** when you only want to
remove that user or role from the access list while leaving other roles' or
direct grants in effect.
