...
[tenant profiles](/database-management/tenant-profiles), which enforce a
...
5. `SHOW DATABASES`: Shows the existing set of multitenant databases. The result
   has a `Name` column and a `state` column that reports whether each database is
   `HOT` (resident in memory) or `COLD` (suspended). Suspended databases are still
   listed.
...
{<h4 className="custom-header"> Requirements and restrictions </h4>}
...
- **It is in in-memory transactional storage mode.** On-disk and in-memory
...
{<h4 className="custom-header"> Working with a suspended database </h4>}
...
- **`SHOW STORAGE INFO ON DATABASE <cold>`** succeeds and returns the tenant's
  ...
  [Server stats](/database-management/server-stats#per-database-storage-information).
...
{<h4 className="custom-header"> Durability and snapshots </h4>}
...
[`--storage-snapshot-on-exit`](/database-management/configuration) is enabled — the
...
{<h4 className="custom-header"> Replication </h4>}
...
{<h4 className="custom-header"> Observability </h4>}
...
histograms. See [Monitoring](/database-management/monitoring).
...
rolling upgrades — see the [Upgrades](/database-management/upgrades) page and
the [version-specific guides](/database-management/upgrades/specific-versions).
