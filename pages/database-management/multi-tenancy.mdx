...
5. `SHOW DATABASES`: Shows only the existing set of multitenant databases. The
   ...
   `Health` column (`ready`/`broken`). `HOT` databaese are resident in memory while `COLD` are suspended, but still listed. Suspended databases are still
   listed. A database is reported as `broken` when it
   failed durability recovery and came up empty — see
   [recovery failure handling](/fundamentals/data-durability#recovery-failure-handling).
