...
To fix this, you can either disable IPv6 or configure Memgraph to use it. The
Bolt server must be explicitly instructed to listen on an IPv6 address.

To do this, set the --bolt-address option to an IPv6 address when starting
Memgraph.

For example, to start Memgraph with the Bolt server listening on all IPv6
interfaces (::):

```bash
memgraph --bolt-address=::
```

You can use `::` corresponding to IPv4 `0.0.0.0` or `::1` corresponding to IPv4
`127.0.0.1`
