...
For a [suspended (COLD)](/database-management/multi-tenancy#suspending-and-resuming-databases-enterprise)
...
| `state`                       | Whether the database is `HOT` (resident in memory) or `COLD` (suspended). See [suspending and resuming databases](/database-management/multi-tenancy#suspending-and-resuming-databases-enterprise). |
...
| `health`                      | Database health: `ready`, or `broken` if durability recovery failed and the database came up empty (see [recovery failure handling](/fundamentals/data-durability#recovery-failure-handling)). |
