...
The query will try to copy the defined file into the local data directory and apply the snapshot. It is advisable to define an absolute file path; a relative path would have to be in relation to the memgraph execution path.
Before modifying the local data directory, all WALs and snapshots will be moved to a hidden directorry: .old_*time-since-epoch*
In case the instance is not freshly started, the query needs to contain the FORCE flag. This will clear all data before applying the snapshot.
...
Its results contain: path to the file, the logical timestamp, physical timestamp and file size.
In case the periodic snapshot background job is active, the first element in the results will define the the time at which the snapshots will be created.
