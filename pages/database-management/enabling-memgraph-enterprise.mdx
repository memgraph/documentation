...
under the `license_type` field, and `SHOW STORAGE INFO` exposes both the
license-imposed limit (`global_license_allocation_limit`) and the system
`--memory-limit` (`global_runtime_allocation_limit`) so you can see which one
is active.
...
Memgraph applies a **24-hour grace period** before treating a license as
expired: the license remains valid for 24 hours past its `valid_until`
timestamp, so a brief delay in rotating keys does not immediately disable
Enterprise features.

Once the grace period also passes, the enterprise features will be disabled,
but the data stored in the database will remain intact. You will still be
able to add more data, but any enterprise features that require specific
actions will no longer function. For example, you will not be able to create
any new databases.
