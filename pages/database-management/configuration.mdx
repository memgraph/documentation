...
| storage.gc.aggressive      | Enables aggressive garbage collection, which performs full cleanup of unused objects. This setting requires taking a unique lock that will temporarily block the system during garbage collection.                                     | no                      |
