...
**First user privileges**: The first user created (either via `init-file` or
environment variables like `MEMGRAPH_USER`/`MEMGRAPH_PASSWORD`) automatically
receives all privileges and becomes a superuser. See the [Role-based access
