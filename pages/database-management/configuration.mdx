...
Most of the configuration changes need to happen [before Memgraph is started](/configuration/configuration-settings#change-configuration-settings-on-startup). Still, a set of configuration settings can be [changed during runtime](/configuration/configuration-settings#change-configuration-settings-during-runtime). 

### Change configuration settings on startup

Changing the configuration settings for Memgraph differs if you are using
Memgraph with **Docker**, **Docker Compose**, or if it was installed on the native **Linux**.

<Tabs items={["Docker", "Docker Compose" , "Linux"]}>

    <Tab>

        You can customize the configuration by passing configuration options within the `docker run` command. When you are working with `memgraph` or `memgraph-mage` images, you should pass
        ...
        To achieve a different configuration on startup, you can also provide the path to the additional configuration file which will override the default one. Still, if any configuration settings are set as arguments, they will override the settings in the additional configuration file as well.
        Providing the path to the additional configuration file requires more steps than setting the configuration via arguments. That's because the file you're pointing to must be located within the container before the container is running. To provide the additional configuration file, follow these steps:
        ...
        It is possible to update the default configuration file as well, but such action requires restart so the update is applied.  
        Here are the steps to change the default configuration file:
...
### Change configuration settings during runtime
...
### Changing the configuration settings via custom configuration file 

You can change the configuration settings by providing your custom configuration
file, which will override the default configuration values in the `memgraph.conf` file.

To provide a custom configuration file, you can use the environment variable
`MEMGRAPH_CONFIG` and point it to the file that contains the custom
configuration settings. The same can be done by using `--flag-file`
in the `memgraph.conf` file or command line argument, and point it to your custom configuration file.

```bash
 --flag-file=another.conf
```

In both cases, the file should contain the configuration settings in the form
`--setting-name=value`. Additional files are processed after the default
configuration file `memgraph.conf`, and they override the default configuration file.
...
| --auth-user-or-role-regex        | Set to the regular expression that each user or role name must fulfill.                                   |
