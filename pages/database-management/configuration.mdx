...
| MEMGRAPH_PASSFILE                | Path to the file that contains the username and password for creating a user. Data in the file should be in the format `username:password`. If your username or password contains `:`, just add `\` before it, for example, `us\:ername:password`. | `[string]` |
