...
coordinators](/clustering/high-availability/coordinator-authentication)**. They
...
| `TENANT PROFILE` operations | `PROFILE_RESTRICTION` | [Tenant profile](/database-management/tenant-profiles) management — covers `CREATE`, `ALTER`, `DROP`, `SHOW`, `SET ON DATABASE`, and `REMOVE FROM DATABASE`. |
