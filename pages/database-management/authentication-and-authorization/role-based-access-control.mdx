...
On Memgraph Enterprise, three built-in roles are automatically created the first time a user is created:
...
Note that `readwrite` and `readonly` only have access to the default
...
The first user created is automatically assigned the `admin` role, making
initial setup straightforward without any manual privilege grants.

If you prefer not to use the built-in roles, you can recreate the same setup
manually using the [templates for granting
...
| Privilege to use [parallel execution](/querying/parallel-execution). | `PARALLEL_EXECUTION` |
...
created automatically and already match these templates. See [Built-in
roles](#built-in-roles-enterprise) for details. The following templates are
useful for Memgraph Community, or when creating custom roles on Enterprise.
