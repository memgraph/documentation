...
- `UPDATE` - grants the user visibility and the ability to edit nodes and
  relationships 
- `CREATE_DELETE` - grants the user visibility, editing, creation, and deletion
...
in front of each label (e.g. `:L1`), or `*` for specifying all labels in the
graph
- `user_or_role` is the already created user or role in Memgraph

For example, granting a `READ` permission on labels `L1` and `L2` would be
written as:
...
while granting both `READ` and `EDIT` permissions for all labels in the graph,
would be written as:
...
Relationship permission queries are in essence the same as node permission
queries, with the one difference that the name of the relationship type is
`EDGE_TYPE` and not `LABEL`.
...
For example, granting a `READ` permission on relationship type `:CONNECTS` would
be written as:
...
where:
- `label_or_edge_type_list` is a list of labels or edge types with a colon in
front of each label or edge type (or `*` for specifying all labels or edge
types)
