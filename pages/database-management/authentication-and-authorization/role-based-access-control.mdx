...
**Note**: 
- For **roles**: This command does not require database specification, even in multi-tenant environments. It will show all privileges for the role.
...
In multi-tenant environments, you must specify which database context to use when showing privileges for users. There are three options:
