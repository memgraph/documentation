...
You do **not** need to add `basic` to `--auth-module-mappings` to use
username/password authentication alongside SSO. Basic (username + password)
authentication always uses Memgraph's internal user management by default.
Only add `basic:<path>` if you want to route basic authentication through an
**external module** (e.g., LDAP), which disables internal user management.
...
+ password) authentication (`--auth-module-mappings=basic:<path to module>`),
its **internal users management is automatically disabled**. Users are
authenticated only using the module and existing local users are ignored.
