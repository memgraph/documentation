...
When you want to delegate basic authentication to an **external module** (e.g.,
LDAP), set `--auth-module-mappings=basic:<path to module>`. This routes all
basic authentication through the external module instead of Memgraph's internal
user storage. Note that if a local user with the same username exists, external
authentication for that user will fail — avoid creating local users that overlap
with externally managed users.
