...
If you cannot use `--init-file`, the equivalent manual procedure is to start the
data instance **without** `--auth-module-mappings`, log in, create the roles and
grant their privileges, then restart with the flag set.

<Callout type="info">
From Memgraph 3.13, [high availability
coordinators](/clustering/high-availability/coordinator-authentication) also
