...
Memgraph supports authentication and authorization using **external auth
modules**. There’s built-in support for basic (username + password) auth via
LDAP, and single-sign on over the SAML and OIDC protocols.

When a user is connecting, Memgraph will forward the user’s supplied
credentials (username + password, or scheme and auth response) to the external
auth module and retrieve its verdict. Based on that, Memgraph will let the user
connect, or in case of failure, close the connection.
...
User roles must be defined in Memgraph before using auth modules. This is
because auth modules return the role associated with the user. 
...
------------------------------------|------------
 ...
 `--auth-module-executable`         | [DEPRECATED] Path to the executable that should be used for user authentication/authorization. Superseded by `--auth-module-mappings=basic` from Memgraph 2.18.
...
Auth schemes supported by Memgraph out-of-the-box have default module paths. To
enable them, you don’t need to specify the auth module path, e.g.
`--auth-module-mappings=basic;saml-entra-id;oidc-okta`. This applies to the
following schemes:
...
The protocol used between Memgraph and the module is as follows:
 ...
 - Auth requests are objects that contain the following fields:
   ...
   - other auth schemes:
    - `scheme` - the string containing the scheme
...
* `MEMGRAPH_SSO_ENTRA_ID_SAML_ASSERTION_AUDIENCE`
* `MEMGRAPH_SSO_ENTRA_ID_SAML_SP_CERT`
* `MEMGRAPH_SSO_ENTRA_ID_SAML_IDP_ID`
* `MEMGRAPH_SSO_ENTRA_ID_SAML_USERNAME_ATTRIBUTE`
* `MEMGRAPH_SSO_ENTRA_ID_SAML_USE_NAME_ID`
* `MEMGRAPH_SSO_ENTRA_ID_SAML_ROLE_MAPPING`


* `MEMGRAPH_SSO_OKTA_SAML_CALLBACK_URL`
* `MEMGRAPH_SSO_OKTA_SAML_ASSERTION_AUDIENCE`
* `MEMGRAPH_SSO_OKTA_SAML_SP_CERT`
* `MEMGRAPH_SSO_OKTA_SAML_IDP_ID`
* `MEMGRAPH_SSO_OKTA_SAML_WANT_ATTRIBUTE_STATEMENT`
* `MEMGRAPH_SSO_OKTA_SAML_USERNAME_ATTRIBUTE`
* `MEMGRAPH_SSO_OKTA_SAML_USE_NAME_ID`
* `MEMGRAPH_SSO_OKTA_SAML_ROLE_ATTRIBUTE`
* `MEMGRAPH_SSO_OKTA_SAML_ROLE_MAPPING`
