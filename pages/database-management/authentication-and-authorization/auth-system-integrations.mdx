...
##### Connecting via Neo4j drivers

When connecting via a driver you can choose to not use the ID token and pass only the access token.
In general all connection schemes will follow the same pattern - setting only the scheme and credentials.
For the scheme pass the scheme you are using - `oidc-entra-id`, `oidc-okta` or `custom`. For the credentials
pass a string in the format `access_token=token-data;id_token=token-data` if you do not wish to use the id_token
just omit it in the string - `access_token=token-data`
