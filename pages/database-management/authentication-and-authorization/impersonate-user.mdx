...
The **Impersonate User** feature allows permitted users to execute queries as if they were another (impersonated) user. With impersonation, users can perform actions with the full context of a different user, including running queries with the impersonated user's privileges.

<Callout type="info">

**Enterprise**: User impersonation requires a Memgraph Enterprise license to
...
The impersonated user is defined at the session start, as shown in the example using the Neo4j Python driver:
...
During this session, all queries will be executed with the privileges and context of the impersonated user (`user1` in this case), effectively "switching" the identity for the duration of the session.
