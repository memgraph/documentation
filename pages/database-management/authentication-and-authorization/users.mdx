...
When you create your first user in Memgraph, that user automatically becomes a
...
**Security critical**: The first user has all privileges. Use a strong password
and reserve this account for administrative tasks only.

</Callout>

### Creating additional users

After creating the first user, you can create additional users and assign them
the privileges they need for their tasks.

For detailed information about assigning privileges, see the
[Role-based access control](/database-management/authentication-and-authorization/role-based-access-control)
documentation.

### Important notes

- The first user automatically has all privileges.
- Additional users need the appropriate privileges assigned to them.
- Use the first user account for administrative tasks only.
