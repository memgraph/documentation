...
The easiest way to access log if you install Memgraph using Docker is by having
a named volume on the `/var/log/memgraph` directory and checking its content. If
you're not using a named volume, here are the steps to access the logs within the
