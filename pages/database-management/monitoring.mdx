...
for the query log directory is `/var/log/memgraph`. The final query log location is going to be 
`/var/log/memgraph/{session_uuid}` where the `session_uuid` is the string value retrieved from setting the session trace on.
