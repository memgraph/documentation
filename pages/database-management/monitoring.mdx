...
Session trace enables you to profile the execution of your whole session. It serves as an additional log file
...
After the command is issued, the logs will be written to the file specified with the `--query-log-directory`. The default value 
for the query log directory is empty so the user needs to set it either during startup or at runtime. 
...
The final query log filename is going to be 
`{--query-log-directory}/{session_uuid}` where the `session_uuid` is the string value retrieved from setting the session trace on.
