...
Monitoring of applications and databases is important to be able to track the
load, resources, logs, transactions, latency etc. 

Memgraph currently supports:

- Logs tracking via WebSocket server 
- Monitoring via HTTP server (Enterprise Edition)

Log tracking is useful for debugging purposes and monitoring the database
operations.  
 
In the Enterprise Edition, on top of logs tracking,  Memgraph allows tracking
information about transactions, query latencies, snapshot recovery latencies,
triggers, bolt messages, indexes etc. using an HTTP server.


## Logs tracking via WebSocket server 

Connect to Memgraph's logging server via WebSocket to forwarded logs to all the
connected clients.


### Log messages

Each log that is written to the [log file](logs.mdx#logs) is forwarded to the connected clients
...
The code structrue is similar in other languages, depending on the WebSocket library used.
...
messages, indexes, streams, and many more metric using an HTTP server.

To be able to retrieve data from the HTTP server, [enter a valid Memgraph Enterprise license
key](/database-management/enabling-memgraph-enterprise).  
...
To reetrive the metrics, send a GET request to the following URL:
...
The example above will print the metric similar to the [example response](#example-response) below.
