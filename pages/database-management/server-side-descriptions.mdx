...
| Label (multi-label combo) | `LABEL :Person:Student` |
...
- Descriptions are written to the WAL and stored in snapshots, so they are
  preserved across restarts.
- They are replicated to replicas as part of the normal replication stream.
- [`DUMP DATABASE`](/database-management/backup-and-restore) emits the matching
  `SET DESCRIPTION` statements, so dumps round-trip descriptions together with
