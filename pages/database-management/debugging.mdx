...
When Memgraph crashes, caused by, e.g., segmentation faults (SIGSEGV), core
dumps can be invaluable. The Memgraph Helm charts provide an easy way to enable
persistent core dump storage using the `createCoreDumpsClaim` option.

Create a file named `values.yaml` with the following content to enable core
dumps (feel free to copy values file from the [helm-charts
repo](https://github.com/memgraph/helm-charts) because other values are also
required):
```
createCoreDumpsClaim: true
```
This instructs the Helm chart to provision a PersistentVolumeClaim (PVC) to
store core dumps generated by the Memgraph process. NOTE: By default the
storage size is 10GiB, core dumps can be substantially bigger (basically whatever
is the total RAM size), make sure to adjust the `coreDumpsStorageSize` under
`values.yaml` file. Additionally, make sure to use the `relwithdebinfo` image
of memgraph by setting the `image.tag` also under `values.yaml` file.
