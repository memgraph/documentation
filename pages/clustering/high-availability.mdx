...
Users can create more than 3 coordinators, but replication factor of 3 is a de facto standard in distributed databases.
...
There is an additional way to set high availability instances using only environment variables. When setting it up, you need to use set up high availability part for the coordinator and/or data instance 
either using only environment variables or configuration flags.

You can't mix and match different configuration flags with environment variables for the following settings:
