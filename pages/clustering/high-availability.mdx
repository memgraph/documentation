...
and allows the Memgraph data instance to use the high availability feature. Data instances also need to be started with `--storage-wal-enabled=true`. The instance will not
be able to get started if `--storage-wal-enabled=false`.
