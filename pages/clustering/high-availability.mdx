...
but steps and the procedure are very similar for native deployment also. Although the upgrade process should always finish successfully, unexpected things can always happen. Therefore, we are strongly recommending doing 
...
disables writes while upgrading any `STRICT_SYNC` instance. Your read queries should however work without any issues.
