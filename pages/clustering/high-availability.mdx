...
For achieving high availability, Memgraph uses Raft consensus protocol which is very similar to Paxos in terms of performance and fault-tolerance but with a significant advantage that
it is much easier to understand. It's important to say that Raft isn't a
Byzantine fault tolerant algorithm. You can find out more about Raft in its [original paper](https://raft.github.io/raft.pdf).
...
Users can create more than 3 coordinators, but replication factor (RF) of 3 is a de facto standard in distributed databases.

One coordinator instance is the leader whose job is to make sure there is always one writeable data instance (MAIN). The other two coordinator instances replicate changes done by the leader coordinator in its
...
rather this task was offloaded to an industry-proven library [NuRaft](https://github.com/eBay/NuRaft).
...
have an option of using bolt+routing so that write queries can always be sent to the correct data instance. This protocol works in a way that the client
...
You can start the data instance and coordinator instance using either environment flags or configuration flags.
...
Coordinator IDs serves as identifiers while the coordinator port is used for synchronization and log replication between coordinators. Coordinator IDs and coordinator ports must be different for all coordinators.
...
When using either of these flags, configurations flags for `--bolt-port`, `--management-port` and `--experimental-enabled` will be ignored.
...
export MEMGRAPH_BOLT_PORT=7692
```
When using either of these flags, configurations flags for `--bolt-port`, `--coordinator-port`, `--coordinator-id` and `--experimental-enabled` will be ignored.
...
perspective, their role (are they MAIN, REPLICA, coordinator or unknown if not alive) and the time elapsed in ms since the last successful response
...
failures and independent machine failures. Byzantine failures aren't handled since Raft consensus protocol cannot deal with them either. 
...
Since there are 2 types of instances in every highly available Memgraph cluster, we need to separately analyze failures of each.
...
cluster stays available. With 2+ coordinator instances down
...
and the time needed to realize the instance is down (`--instance-down-timeout-sec`). After collecting enough evidence, leader concludes the MAIN is down and performs failover using just a handful of RPC
messages (correct time depends on distance between instances). It is important to mention that whole failover is performed with a zero data-loss if newly chosen MAIN (previously REPLICA)
had all up-to-date data.

Current deployment assumes existence of only one datacenter which automatically means that Memgraph won't be available in the case whole datacenter goes down. We are actively
working on 2 datacenter (2-DC) architecture.

## Kubernetes

We support deploying Memgraph HA instances as part of the Kubernets cluster. You can see example configurations [here](/getting-started/install-memgraph/kubernetes#helm-chart-for-memgraph-high-availability-cluster).
...
| coordinator_2 | 127.0.0.1:7691 | 127.0.0.1:10112    |       ""          | up     | coordinator |  16               |
| coordinator_3 | 127.0.0.1:7692 | 127.0.0.1:10113    |       ""          | up     | coordinator |  25               |
| instance_1    | 127.0.0.1:7687 |       ""           | 127.0.0.1:10011   | up     | replica     |  39               |
| instance_2    | 127.0.0.1:7688 |       ""           | 127.0.0.1:10012   | up     | replica     |  21               |
| instance_3    | 127.0.0.1:7689 |       ""           | 127.0.0.1:10013   | up     | main        |  91               |
