...
Bounding execution time of the replication RPC messages AppendDeltasRpc, CurrentWalRpc and WALFilesRpc with a timeout doesn't make sense since the time needed on replica to 
process such requests directly depends on the size of data being transferred. Because of that, the replica sends a RPC message to the main 
saying that the progress is made for every batch of 100k deltas it processed. Processing 100k deltas should always take approximately the same time so we can put a timeout on that. 
The default timeout is set to 30s for these 3 RPC messages. In reality, the processsing of 100k deltas should take < 3s.

SnapshotRpc is also considered a replication RPC message. Bounding its execution time works in a way in which replica sends an update to main for every 1'000'000 units of work performed.
Processing vertices, edges, vertices inside label index, vertices inside property index, edges inside edge type index and edges inside edge type property index is worth one unit of work. 
Processing vertices inside point and text indices takes a bit more time so it's worth 10 units of work. 
The processing of vertices inside vector indices takes longest so it's worth 1000 units of work. Using such configuration, the replica instance is expected to update MAIN on its progress
made every 2-3s. Hence, a timeout of 60s should be more than enough to not cause network instability.
