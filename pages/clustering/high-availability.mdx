...
There are several Raft-related parameters which are important for correct functioning of the cluster. First, leader coordinator sends heartbeat
...
expiration is set to 1800ms so that cluster can never get into situation where multiple leaders exist. These concrete values give a cluster
