...
At the moment of unregistration, the instance that you want to unregister must not be MAIN because unregistering MAIN could lead to an inconsistent cluster state. At the same time,
cluster needs to have alive MAIN instance to successfully unregister an instance since otherwise we cannot guarantee the success of a requested operation.
