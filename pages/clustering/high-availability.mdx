...
When using either of these envronment variables, flags `--bolt-port`, `--management-port` and `--experimental-enabled` will be ignored.
...
```
When using either of these environment variables, flags for `--bolt-port`, `--coordinator-port`, `--coordinator-id` and `--experimental-enabled` will be ignored.
...
services:
  coord1:
    ...
    command: [ "--init-file=/tmp/init/license.cypher", "--log-level=TRACE", "--data-directory=/tmp/mg_data_coord1", "--log-file=/tmp/coord1.log", "--also-log-to-stderr", "--coordinator-id=1", "--coordinator-port=10111", "--experimental-enabled=high-availability"]
    networks:
      memgraph_ha:
  ...
  coord2:
    ...
    volumes:
      - ./license.cypher:/tmp/init/license.cypher:ro
    command: [ "--init-file=/tmp/init/license.cypher", "--log-level=TRACE", "--data-directory=/tmp/mg_data_coord2", "--log-file=/tmp/coord2.log", "--also-log-to-stderr", "--coordinator-id=2", "--coordinator-port=10112", "--experimental-enabled=high-availability"]
    networks:
      memgraph_ha:
  ...
  coord3:
    ...
    volumes:
      - ./license.cypher:/tmp/init/license.cypher:ro
    command: [ "--init-file=/tmp/init/license.cypher",  "--log-level=TRACE", "--data-directory=/tmp/mg_data_coord3", "--log-file=/tmp/coord3.log", "--also-log-to-stderr", "--coordinator-id=3", "--coordinator-port=10113", "--experimental-enabled=high-availability"]
    networks:
      memgraph_ha:
...
| ------------- | -------------- | ------------------ | ----------------- | ------ | -------- | ----------------  |
| coordinator_1 | 0.0.0.0:7690   | 127.0.0.1:10111    |       ""          | up     | leader   |  0                |
| coordinator_2 | 127.0.0.1:7691 | 127.0.0.1:10112    |       ""          | up     | follower |  16               |
| coordinator_3 | 127.0.0.1:7692 | 127.0.0.1:10113    |       ""          | up     | follower |  25               |
...
| ------------- | -------------- | ------------------ | ----------------- | ------ | -------- | ------------------|
| coordinator_1 | 0.0.0.0:7690   | 127.0.0.1:10111    |        ""         | up     | leader   |  0                |
| coordinator_2 | 127.0.0.1:7691 | 127.0.0.1:10112    |        ""         | up     | follower |  34               |
| coordinator_3 | 127.0.0.1:7692 | 127.0.0.1:10113    |        ""         | up     | follower |  28               |
