...
Configuration option `--coordinator-hostname` must be set on all coordinator instances. It is used on followers to ping the leader coordinator on the correct IP address and return health state about
the cluster. You need to set this configuration flag to correct IP address, fully qualified domain name (FQDN) or even DNS name. In local setup example would be to set everything to `localhost`. 
