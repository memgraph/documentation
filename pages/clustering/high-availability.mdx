...
which means that the resolution of network endpoints happens inside drivers.
...
On failover, the current logic is to choose the most-up-to-date alive instance for the new MAIN. When choosing a new MAIN in the failover procedure from the list of available REPLICAs, 
the instance with the latest commit timestamp for the default database is chosen as the new MAIN. 
In the scenario where some other instance had more up-to-date data when the failover procedure was choosing a new MAIN but was down at that point, when rejoining the cluster, 
the new MAIN instance will recover that replica in a specific way when the replica comes back alive. The recovery process will first clear replica's storage and then apply all the 
commits from the new MAIN. All replica's old durability files will get preserved in `.old` directory so that admin can recover the data if something goes wrong. When looking at the 
CAP theorem, Memgraph's current implementation leans more towards availability than consistency. Memgraph tries to be as consistent as possible but because of the logic which will
promote the most-up-to-date alive REPLICA (which doesn't need to have all commits as previous MAIN), Recovery Point Objective (RPO) is != 0. The current decision is done so that
MAIN is writeable for as much time as possible. If you have an environment in which consistency is the top priority and you don't care as much about being constantly available to 
writes, contact us and we are eager to expand our support for fully synchronous mode.
