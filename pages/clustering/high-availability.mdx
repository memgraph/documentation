...
When using authentication features for high availability, there are several things that need to be kept in mind. At this point, we don't support creating users with the query 
`CREATE USER` on coordinators. Instead, the user can be created by setting environment variables `MEMGRAPH_USER` and `MEMGRAPH_PASSWORD`. In this way, you will be able to use
direct bolt connection to coordinators as expected. Users can be created using `CREATE USER` on the main data instance and this user will get replicated to other replica data
instances but coordinators won't know about this user. If you are using bolt+routing connections and you have an user created on data instances, that same user needs to exist
on coordinators too. This is because drivers are using the same authentication arguments for connecting to coordinators and data instances when using bolt+routing type of connection.
