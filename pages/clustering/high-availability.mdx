...
from which instance can the data be read, to which instance data can be written to and which instances behave as routers. If the user is connected to the cluster leader,
the leader replies immediately with the current routing table which is most definitely correct since the Raft protocol guarantees the correctness of the current leader. If the routing
request is received on followers, they propagate the request to the cluster leader so that clients have the correct routing table.


In the Memgraph HA cluster, the main
...
Users can control the maximum allowed lag on the replica so that it is still considered up-to-date and read queries can be routed to. If replica lags behind the current main for more than
the `max_replica_read_lag_` transactions on any database, read queries won't be routed to the data instance when the bolt+routing protocol is used.
