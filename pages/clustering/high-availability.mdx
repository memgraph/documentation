...
<Callout type="info">
It is important to note that in case some action fails and all instances are down, the leader will try to execute force reset until one instance is promoted to MAIN. Until then no other action is allowed
on cluster.
</Callout>
