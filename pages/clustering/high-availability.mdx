...
own Raft log. Operations saved into the Raft log are those that are related to cluster management. Memgraph doesn't have its implementation of the Raft protocol. For this task, Memgraph uses an industry-proven library [NuRaft](https://github.com/eBay/NuRaft).
...
It is important to note that our current implementation of high availability could lead to data loss. The specific situation could occur when all REPLICAs are behind the current MAIN
and current MAIN is down. The failover is triggered in which the most-up-to-date replica is chosen for promotion. Such an incident can only happen in high-volume environment where
data is constantly changed, added, deleted etc. If zero data-loss HA is your requirement, please open an issue on [GitHub](https://github.com/memgraph/memgraph/issues) and we will work towards improving that.
...
In case the coordinator instance dies and it is restarted, it loses all data from the Raft log. Information about other coordinators and their endpoints is recovered from `high_availability/coordinator`  directory stored under 
top level `--data-directory` folder. When the coordinator rejoins, it will communicate with other coordinators and receive updates from the current leader. It is important that at least one coordinator is alive at all the time so that data from the Raft log is not lost.
