...
- **STRICT_SYNC mode** - After committing a transaction, the MAIN instance will communicate the 
changes to all REPLICA instances and wait until it receives a response or information that a timeout is reached. 
...
the MAIN instance can’t process the write due to the nature of synchronous replication. It is implemented as
...
STRICT_SYNC mode ensures consistency and partition tolerance.
...
data durability files for Memgraph.** Snapshot files represent an image of the current state of 
the database and are much larger than the WAL files, which only contain the changes, deltas.
Because of the difference in file size, Memgraph favors the WAL files.
