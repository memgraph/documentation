...
- [Suspending or resuming a database](/database-management/multi-tenancy#suspending-and-resuming-databases-enterprise)
  on the MAIN is replicated the same way: each REPLICA applies the transition and
  converges to the MAIN's authoritative HOT/COLD set, including replicas that were
  disconnected, lagging, or restarted while the transition happened. `SUSPEND`/`RESUME
  DATABASE` are rejected on a REPLICA (`Query forbidden on the replica!`) — issue them
  only on the MAIN.
