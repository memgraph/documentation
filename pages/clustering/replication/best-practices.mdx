...
and one that runs state checks and recovery for that replica. Leave the MAIN
instance some CPU headroom beyond what its query workload needs when registering
many replicas.
