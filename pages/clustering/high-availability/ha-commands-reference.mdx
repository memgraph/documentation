...
`--instance-health-check-frequency-sec` startup flags are gone; use the
coordinator settings above instead. Values set through the flags are **not**
migrated automatically, so after upgrading the settings revert to their defaults
(`5` and `1`). If you had customized the flags, run `SET COORDINATOR SETTING`
queries to re-apply your values. For the versions that deprecated and removed
them, see [deprecated
features](/database-management/upgrades/deprecated-features).
</Callout>
