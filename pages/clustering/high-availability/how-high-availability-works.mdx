...
[`SET COORDINATOR SETTING`](/clustering/high-availability/ha-commands-reference#coordinator-runtime-settings).
...
1. **Replicas** first, one at a time.
2. The **Main** instance — its restart triggers the standard [automatic
   ...
   the new Main.
3. **Coordinator followers**, and finally the coordinator **leader**, whose
   restart triggers a Raft leader re-election.
...
page](/clustering/high-availability/setup-ha-cluster-k8s#in-service-software-upgrade-issu).
...
- `instance_health_check_frequency_sec`: how often health checks are sent (configurable via [`SET COORDINATOR SETTING`](/clustering/high-availability/ha-commands-reference#coordinator-runtime-settings))
- `instance_down_timeout_sec`: how long an instance must remain unresponsive
  before it is considered down (configurable via [`SET COORDINATOR SETTING`](/clustering/high-availability/ha-commands-reference#coordinator-runtime-settings))
...
[`--storage-allow-recovery-failure=true`](/database-management/configuration),
...
See [recovery failure handling](/fundamentals/data-durability#recovery-failure-handling)
