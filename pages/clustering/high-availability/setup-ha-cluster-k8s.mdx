...
Replace `<release-name>` with a name of your choice for the release. The
...
Memgraph HA uses **client-side routing**, so DNS resolution happens on the driver. Because of that, we need
...
{<h4 className="custom-header" id="recommended-external-access-setup"> Recommended external access setup </h4>}
...
Or with IngressNginx for the data instances:
...
In both cases every coordinator is reachable at the shared
...
addresses automatically (see [Update bolt server](#update-bolt-server)).

All other combinations are valid as well — for coordinators + data instances,
for example:
...
server](#update-bolt-server)). Do **not** set the `external-dns` hostname at
...
up and down](#scale-the-cluster-up-and-down)).
...
- **Scale out**: add an entry to `coordinators[]` or `data[]` and run `helm
  upgrade`. The new coordinator is registered with `ADD COORDINATOR` and the new
  data instance with `REGISTER INSTANCE`.
- **Scale down**: remove an entry from `coordinators[]` or `data[]` and run `helm
  upgrade`. Members that are registered in the cluster but no longer present in
  the values are removed with `REMOVE COORDINATOR` / `UNREGISTER INSTANCE`.
...
Gateway, which is our [recommended setup](#recommended-external-access-setup).
...
1. **The Gateway API CRDs** installed in your cluster. Some controllers ship them
   with their own Helm chart (Envoy Gateway does), while others expect you to
   install them yourself (Traefik does). The controller-specific guides below
   cover both cases.

2. **A Gateway API controller** installed in your cluster. Examples include
   [Envoy Gateway](https://gateway.envoyproxy.io/), [Istio](https://istio.io/)
   (see also: [Use Istio as the Gateway API controller](#use-istio-as-the-gateway-api-controller)),
   [Cilium](https://cilium.io/), [Traefik](https://traefik.io/), and
   [Kong](https://konghq.com/). Step-by-step guides are available for two of
   them: [Envoy Gateway](#deploy-with-envoy-gateway) and
   [Traefik](#deploy-with-traefik).

3. **A GatewayClass resource** that references your controller. A GatewayClass is
   a cluster-scoped resource that defines which controller manages Gateways —
   each Gateway references a GatewayClass by name. The Memgraph Helm chart does
   **not** create a GatewayClass; you must create one yourself or use one
   provided by your controller installation (the Envoy Gateway chart does not
   create one, the Traefik chart does).
...
[Deploy with Traefik](#deploy-with-traefik)).
</Callout>
...
`data-{id}-bolt` for data instances and a single listener named `coordinators-{id}-bolt`
...
[Envoy Gateway](https://gateway.envoyproxy.io/) is the controller used in the
examples above. Its Helm chart bundles the Gateway API CRDs, so no separate CRD
installation is needed, and it programs listener ports dynamically — the Bolt
ports only have to be configured in the Memgraph chart.

<Steps>

{<h4 className="custom-header"> Install Envoy Gateway </h4>}
...
Chart version `v1.2.4` ships Gateway API bundle version `v1.2.1`, which includes
the `TCPRoute` CRD.

{<h4 className="custom-header"> Create the GatewayClass </h4>}

The Envoy Gateway chart does not create a GatewayClass, so create one that points
at its controller:
...
{<h4 className="custom-header"> Install the Memgraph HA chart </h4>}

Assuming the `memgraph-secrets` Secret with the license and organization name
already exists (see [Install Memgraph HA](#install-memgraph-ha)):
```bash
helm install memgraph-ha memgraph/memgraph-high-availability \
  --set externalAccessConfig.gateway.enabled=true \
  --set externalAccessConfig.gateway.gatewayClassName=eg
...
{<h4 className="custom-header"> Verify the setup </h4>}
...
The GatewayClass should be `ACCEPTED=True` and the Gateway `PROGRAMMED=True` with
...
[Traefik](https://traefik.io/) can serve the same setup, but its Gateway API
support differs from Envoy Gateway in two ways that affect the Memgraph chart:

- Its Helm chart ships **no Gateway API CRDs**, so you install them yourself, and
  `TCPRoute` support has to be explicitly enabled.
- Traefik does **not** open arbitrary listener ports. Every port used by a Gateway
  listener must also be declared as a Traefik entryPoint, otherwise the listener
  is silently ignored.

<Steps>

{<h4 className="custom-header"> Install the Gateway API CRDs </h4>}
...
<Callout type="info">
Use `--server-side`. A plain `kubectl apply` stores the whole manifest in the
...
{<h4 className="custom-header"> Configure Traefik </h4>}
...
A ready-to-use version of this file is available in the [Helm charts
repository](https://github.com/memgraph/helm-charts/blob/main/examples/gateway/traefik-values.yaml).

<Callout type="warning">
Declare one entryPoint per Gateway listener, and remember to add new ones when
you [scale the cluster](#scale-the-cluster-up-and-down) — a third data instance
...
entryPoints; with the [recommended setup](#recommended-external-access-setup)
(coordinators via `CommonLoadBalancer`, data instances via the Gateway) only the
`data-*` entryPoints are required.
</Callout>

{<h4 className="custom-header"> Install Traefik </h4>}
...
The values above were verified against Traefik chart `41.0.2` (Traefik `v3.7.6`).
...
kubectl describe gateway memgraph-ha-memgraph-high-availability-gateway
```

If a listener reports `Detached` or `UnsupportedProtocol`, the matching entryPoint
...
port, not host: data instances are reachable at `ingress.dataPortBase + data
instance id` and coordinators at `ingress.coordinatorPortBase + coordinator id`.
...
The chart `kube-prometheus-stack` should be installed independently from the HA
chart. Add the `prometheus-community` Helm repository and install (or upgrade) the
release into the `monitoring` namespace:
...
Remember the release name (`kube-prometheus-stack`) and the namespace (`monitoring`):
the HA chart needs both to wire its `ServiceMonitor` and Grafana dashboard to this
stack (see [`prometheus.serviceMonitor.kubePrometheusStackReleaseName`](#configuration-options)
and `prometheus.namespace` below).

To customize the stack, pass your own values file with `-f kube_prometheus_stack_values.yaml`.
A template is available in the upstream [chart’s
repository](https://github.com/prometheus-community/helm-charts/blob/main/charts/kube-prometheus-stack/values.yaml).

If you scrape through `mg-exporter` (see the next section) you can additionally pass
`-f kube_prometheus_stack_memgraph_dashboard.yaml`, a generic dashboard which shows the metrics
that the exporter exposes for both standalone and HA deployments. The dashboard file can be downloaded from
[here](https://github.com/memgraph/prometheus-exporter/blob/main/kube_prometheus_stack_memgraph_dashboard.yaml).
When scraping Memgraph directly, use the bundled **Memgraph OpenMetrics** dashboard
instead (see [Grafana dashboard for direct scraping](#grafana-dashboard-for-direct-scraping)).
...
- `caSecretName` — name of a pre-created Secret holding the CA bundle that
  ...
  `ca_file=/etc/mg-exporter/ssl/<caSecretKey>` to the exporter.
...
To start the HA chart with direct scraping you need:

1. **A Prometheus Operator in the cluster** – for example `kube-prometheus-stack`
   installed as shown [above](#monitoring). The HA chart only creates a
   `ServiceMonitor`; something must consume it.
2. **`scrapeMemgraphDirectly: true`** – the chart appends `--metrics-format=OpenMetrics`
   to every coordinator and data instance and adds a `tcp-metrics-port` to each
   instance `Service` so it can be discovered.
3. **`prometheus.enabled: false`** – no `mg-exporter` is deployed.
4. **`prometheus.serviceMonitor.enabled: true`** – provisions the `ServiceMonitor`
   that targets each instance `Service` directly.
5. **Matching release name and namespace** – the `ServiceMonitor` is labelled
   `release: <prometheus.serviceMonitor.kubePrometheusStackReleaseName>`, which is how
...
prometheus:
  ...
  namespace: monitoring   # where kube-prometheus-stack lives; the ServiceMonitor is created here
  serviceMonitor:
...
The `ServiceMonitor` selects the instance `Services` in the HA chart's release
namespace through its `namespaceSelector`, so the HA chart and `kube-prometheus-stack`
can live in different namespaces without any extra configuration.
...
After installing the HA chart, verify that the `ServiceMonitor` exists and that
Prometheus picked up the targets:
...
| `prometheus.namespace`                                     | Namespace in which `kube-prometheus-stack` is installed. The `ServiceMonitor`, the Grafana dashboard `ConfigMap` and (when `prometheus.enabled` is `true`) the exporter resources are created here. When empty, the release namespace is used. | `""`         |
...
<Callout type="info">
Some Memgraph versions require additional upgrade steps beyond the standard ISSU
procedure. Check the [Migrating to v3.9 HA](/clustering/high-availability/migrating-to-v3-9-ha) page for
...
3. Upgrade **coordinator followers**
4. Upgrade the **coordinator leader**.
...
{<h4 className="custom-header"> Query behavior during the upgrade </h4>}

Your applications can stay connected to the cluster throughout the upgrade.
What they will observe depends on the query type and the replication mode:

- **Read queries always work**, regardless of the replication mode used. There
  is no specific exception you need to catch — Neo4j drivers are preconfigured
  with automatic retries, so reads hitting an instance that is restarting are
  transparently retried.

  One exception applies to minimal clusters: with **1 main and 1 replica**, you
  need to enable the
  [`enabled_reads_on_main`](/clustering/high-availability/ha-commands-reference#enabled_reads_on_main)
  coordinator setting so that, while the only replica is being upgraded, read
  queries are routed to the main instance:
...
- **Write queries depend on the replication mode:**
  ...
  - **STRICT_SYNC** — writes won't work while a data instance is being
    ...
    aborted until the restarted instance rejoins the cluster.
...
[`global_read_only`](/clustering/high-availability/ha-commands-reference#global_read_only)
...
When enabled, the current MAIN stops accepting write queries while it keeps
serving reads and replicating existing data to REPLICAs. The setting blocks
**all write sources** on the MAIN — user Cypher writes,
[TTL](/querying/time-to-live) background expiry, and [stream-](/data-streams) and
[trigger-](/fundamentals/triggers)driven writes — so the dataset stays frozen
while replicas catch up and you swap binaries. The value is persisted in the
coordinator's Raft-replicated cluster state, so it survives coordinator restarts
and leader re-elections, and is honored across failovers: a newly promoted MAIN
comes up read-only instead of silently accepting writes.

<Callout type="warning">
While the cluster runs mixed versions during the rolling upgrade, read-only mode
is honored best-effort: older instances behave as before until every node is
upgraded, after which the cluster self-heals to the requested state.
</Callout>

<Steps>
{<h4 className="custom-header"> Upgrade replicas </h4>} 

Before restarting any replica, check whether replicas are caught up with the
MAIN:
...
If the replica you are about to restart is lagging behind, wait until it
catches up before deleting its pod.
...
[`instance_health_check_frequency_sec`](/clustering/high-availability/ha-commands-reference#instance_health_check_frequency_sec)
seconds and declares an instance down after it hasn't responded for
[`instance_down_timeout_sec`](/clustering/high-availability/ha-commands-reference#instance_down_timeout_sec)
