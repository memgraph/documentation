...
<Callout type="info">
If you are switching from Envoy Gateway, uninstall it first (`helm uninstall eg -n
envoy-gateway-system` and `kubectl delete gatewayclass eg`), then update
`externalAccessConfig.gateway.gatewayClassName` from `eg` to `traefik`. The
Memgraph Gateway and TCPRoute resources themselves do not change.
</Callout>
