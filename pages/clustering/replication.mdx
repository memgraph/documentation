...
However, authentication modules are not replicated and must be configured manually by the administrator.

## Multi-tenant data replication (Enterprise)

If you are using a Memgraph Enterprise license multi-tenant commands are replicated as any other data command.
