...
- [Migration from MySQL, SQL Server, or Oracle](#migrate)
...
For direct migration from MySQL, SQL Server, or Oracle databases, you can also use
the `migrate` module from the MAGE library. This module simplifies the process
of importing data directly into Memgraph, enabling seamless conversion of
relational data into graph structures without the need for intermediary CSV
files. For more details on using the `migrate` module, refer to our
[documentation on migration](/advanced-algorithms/available-algorithms/migrate).
