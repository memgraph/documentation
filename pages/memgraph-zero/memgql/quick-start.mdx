...
easiest way to start is to boot from a schema file; you can also build the
catalog live with runtime DDL (steps 1–6 below). Either way you query with the
same `ADD CONNECTOR` / `CREATE GRAPH` / `USE` statements.

### Easiest: boot from a schema file

Declare connectors + graphs in one JSON file and point MemGQL at it — everything
is registered at startup, no runtime DDL required:
...
multi-connector mode. See the [Schema File](/memgraph-zero/memgql/schema-file)
page for the format; runtime DDL still works and persists changes back to the
file.
