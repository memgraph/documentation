...
| Multi-label pattern `(n:A&B)`; shared label over several mappings  | ✓               | ✓            | ✗       |
...
- **Shared labels** (a label listed by several vertex mappings, see
  [Vertices](/memgraph-zero/memgql/schema-file#vertices)) are unioned only
  within one connector; a label shared across connectors is refused by routing
  as ambiguous. A pattern matched through a shared label can be returned,
  filtered, sorted and aggregated, but not traversed from (`(a:Animal)-[:R]->()`),
  not `INSERT`ed, and JSON keys (`a.props.key`) cannot be read through it —
  each is refused with a pointer to the identifying label to use instead.
...
| `BOLT_LISTEN_ADDR`        | `0.0.0.0:7688`   | `<ip>:<port>` the Bolt listener binds — every interface by default, as in Memgraph. Use `127.0.0.1:7688` to accept only connections from the same host. An IP, not a hostname; IPv6 in brackets (`[::]:7688`). `--bolt-address` / `--bolt-port` override it. |
...
|----------------------------|--------------------|------------------------------------------------------|
| `--bolt-address=<IP>`      | `0.0.0.0`          | Interface the Bolt listener binds; `127.0.0.1` refuses connections from other hosts. An IP, not a hostname; IPv6 bare (`::`). |
| `--bolt-port=<PORT>`       | `7688`             | Port the Bolt listener binds (`--bolt-server=<PORT>` is a synonym). Each flag replaces only its half of `BOLT_LISTEN_ADDR`. |
...
[SQL Server connector page](/memgraph-zero/memgql/connect/sqlserver).
