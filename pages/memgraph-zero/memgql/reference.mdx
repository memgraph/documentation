...
| `DATABASE '<db>'`           | PostgreSQL, MySQL, SQL Server, Oracle (service name), ClickHouse, MongoDB, Snowflake, Fabric (warehouse/lakehouse item) |
| `CATALOG '<catalog>'`       | Iceberg (Trino catalog), Iceberg Direct (warehouse)                 |
| `SCHEMA '<schema>'`         | Iceberg, Snowflake, Fabric (default `dbo`); MongoDB accepts it as a fallback for `DATABASE` |
