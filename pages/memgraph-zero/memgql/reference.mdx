...
- **Undirected variable-length** (`(a)-[:R*]-(b)`) returns an actionable error:
...
| `DATABASE '<db>'`           | PostgreSQL, MySQL, SQL Server, Oracle (service name), ClickHouse, MongoDB, Snowflake |
| `CATALOG '<catalog>'`       | Iceberg (Trino catalog), Iceberg Direct (warehouse)                 |
| `SCHEMA '<schema>'`         | Iceberg, Snowflake; MongoDB accepts it as a fallback for `DATABASE`  |
| `WAREHOUSE` / `ROLE`        | Snowflake session settings                                           |
| `PRIVATE_KEY_PATH` / `TOKEN`| Snowflake auth (key-pair JWT / programmatic access token)            |
