...
- **Cypher queries.** MemGQL now speaks Cypher alongside GQL. Pick the language
  per query with the Neo4j-style `CYPHER [25]` prefix, per session with
  `SET SESSION LANGUAGE CYPHER`, per connector with `LANGUAGE 'cypher'` on
  `ADD CONNECTOR` (applies to `USE <graph>` queries against that connector's
  graphs), or server-wide with `--default-language` /
  `MEMGQL_DEFAULT_LANGUAGE`. Against Memgraph and Neo4j the Cypher text
  executes **verbatim** — no translation round trip — so an existing Cypher
  corpus keeps working unchanged, including constructs beyond MemGQL's own
  parser (`CALL` subqueries, pattern predicates, `reduce`, `UNION`, map
  literals), which forward as-is whenever the statement targets a single
  Cypher backend. Against every other backend the same Cypher parses into
  MemGQL's shared plan and runs through the regular translation path — one
  query corpus over a live graph and a cold archive. The default stays GQL
  everywhere. See
  [Query languages](/memgraph-zero/memgql/reference#query-languages).
- **Read-only graphs.** A graph declares
  `"accessMode": "readOnly" | "readWrite"` (schema file or `CREATE GRAPH …
  FROM` body), and `ALTER GRAPH <name> SET READ ONLY | READ WRITE` flips it at
  runtime — persisted back to the boot `--schema` file, so a restart cannot
  quietly reopen a replica. A write routed to a read-only graph — explicitly
  or by label inference — is refused before any backend sees the statement.
  See [Access modes](/memgraph-zero/memgql/reference#access-modes).
- **Source onboarding through the engine.** `DESCRIBE CONNECTOR <name>` reads
  a source's physical catalog live — tables, columns, types, nullability,
  primary keys, and foreign keys as `→ table.column` (PostgreSQL and SAP
  HANA). `GENERATE MAPPING FOR CONNECTOR <name> [GRAPH <g>] [TO '<path>']`
  turns that catalog into a draft mapping — tables with a primary key become
  vertices, single-column foreign keys become edges — for review; loading
  stays explicit via `CREATE GRAPH … FROM`. Anything the draft can't
  represent (composite keys, PK-less tables) is named in `_comment` rather
...
- **The schema is a graph.** `SHOW SCHEMA [FOR <graph>] AS GRAPH` returns the
  merged schema as Bolt nodes and relationships — one node per (source,
  label) carrying its property list, one relationship per edge type,
  cross-connector join edges included and marked — so agents plan federated
  queries by navigating the schema and graph UIs draw it. `EXPORT SCHEMA` now
  round-trips cross-connector join edges and `accessMode`, so a dumped schema
  reloads without losing federated traversals or reopening read-only graphs.
