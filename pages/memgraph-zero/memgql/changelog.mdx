...
- **Caching now requires every cached label to declare `attributes`.** A label
  that declares none would be cached id-only, so MemGQL refuses to cache that
  graph and names the label instead of serving property-stripped rows. Queries
  continue to run against the source. See
...
- **The Memgraph cache dropped most edges and every undeclared property.** Cached
  edges pointed at the wrong endpoints or were missing entirely, and cached nodes
  carried only their id, so a query served from the cache silently returned
  different rows than the same query against the source. Edge properties are now
  copied into the cache as well.
