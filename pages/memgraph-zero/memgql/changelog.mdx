...
- **`--bolt-address` and `--bolt-port`.** The Bolt listener's interface and
  port as flags, alongside `BOLT_LISTEN_ADDR`; each flag replaces only its half
  of the address, so `--bolt-port=7690` starts a second server beside the
  first. `--bolt-address=127.0.0.1` keeps a server local. See
  [Configuration](/memgraph-zero/memgql/reference#configuration-reference).
...
- **MemGQL listens on every interface by default.** The Bolt listener binds
  `0.0.0.0:7688` instead of `127.0.0.1:7688`, matching Memgraph's own
  `--bolt-address` default, so a published container port or a Kubernetes
  Service reaches it without `BOLT_LISTEN_ADDR`. Anything that can route to
  the host can now open a session: set `--bolt-address=127.0.0.1` (or
  `BOLT_LISTEN_ADDR=127.0.0.1:7688`) where the server should stay local, and
  the `server.listen_addr` row of `SHOW CONFIG` reports what it bound.
...
- A shared label is unioned within one connector only; shared across
  connectors it is refused by routing as ambiguous. A node matched through a
  shared label cannot be traversed from, `INSERT`ed, or have JSON keys read
  through it — each refusal names the identifying label to use instead. See
  [Known limitations](/memgraph-zero/memgql/reference#known-limitations).
- A join edge inside one connector cannot be traversed with a variable-length
  step, and both endpoints on a Memgraph or Neo4j connector are refused in
  favour of `mappedGraphSource`.

## MemGQL v0.12.0 - September 14th, 2026

### ⚠️ Breaking changes

- **`SHOW STATS` is now `SHOW STATS CONNECTORS`,** and its first column is
  `connector` rather than `source`. Bare `SHOW STATS` returns the list of
  targets instead of the per-connector table. `RESET STATS` is unchanged.
- **`SHOW LICENSE` reports only the license.** Its rows are `offering`
  (`community` or `enterprise`), `status` (`valid`, `expired` or
  `unsupported`), `organization`, `valid_until`, `days_left`, `limit.max_cores`,
  `limit.max_connectors` and `limit.max_connections`; numbers come back as integers,
  not strings. The old `mode`, `core_limit`, `max_connectors` and
  `max_connections` keys are renamed as above, `license_type` is dropped, and
  the `compute_*` / `host_cores` rows moved to `SHOW CONFIG` and
  `SHOW STATS COMPUTE`.
- **`SHOW CONNECTORS` gains a `connections` column** — open connections per
  connector — after `graph`.

### 🍃 New features & Improvements

- **License core limit.** A license may carry a core limit. MemGQL then sizes
  its query workers, the embedded DuckDB engine and the native Iceberg runtime
  to it, and at most that many queries do in-process compute at once. When
  every core is busy a query waits up to `MEMGQL_COMPUTE_QUEUE_TIMEOUT_MS` for
  a free one, then fails with `Compute limit reached` and can be retried.
  Community mode is capped at 2 cores. `MEMGQL_COMPUTE_CORES` lowers the
  budget further but never raises it. See
  [Compute](/memgraph-zero/memgql/reference#compute).
- **`SHOW CONFIG`.** The server's effective settings as `name`/`value` rows:
  the compute budget (`compute.cores`, `compute.host_cores`,
  `compute.queue_timeout_ms`), server flags (`server.default_language`,
  `server.log_level`, `server.schema_file`, …) and the license inputs, key
  masked.
- **`SHOW STATS COMPUTE`.** Live compute counters — cores busy, queued, peak
  and refused, queries in flight, the server's own CPU time — as typed
  key/value rows, in every connector mode. Bare `SHOW STATS` lists the
  available targets.
- **Composite keys.** `metaFields.id`, `from` and `to` accept a list of
  columns (`"id": ["region", "order_no"]`) on every SQL backend. Edges join column
  by column at the source; `RETURN n`, `WITH n` and `count(DISTINCT n)` treat
  the tuple as one identity. `EXPORT SCHEMA` round-trips the list form. See
  [Schema file](/memgraph-zero/memgql/schema-file#edges).
- **SQL Server views across databases.** `mappedTableSource.database` renders a
  three-part `database.schema.object` name, so one connection reaches views in
  other databases of the instance.
- **SQL Server lock hints.** `NoLock=true` in the connection string puts
  `WITH (NOLOCK)` on every table MemGQL reads; `IsolationLevel=<level>` sets the
  session isolation level once. See
  [SQL Server](/memgraph-zero/memgql/connect/sqlserver).
- **Memgraph databases from the schema file.** On a `memgraph` or `neo4j`
  connector, `connection.database` selects the Bolt database — the same thing
  `ADD CONNECTOR … GRAPH <db>` selects at runtime — so one Memgraph Enterprise
  instance can back several connectors, one per tenant. `EXPORT SCHEMA` writes
  a runtime `GRAPH` selector back as `database`, so an exported schema
  reconnects to the same database. See
  [Schema file](/memgraph-zero/memgql/schema-file#connectors).

### 🐞 Bug fixes

- **SQL Server dates.** `date`, `datetime`, `datetime2`, `smalldatetime` and
  `datetimeoffset` columns come back as ISO-8601 text (`2026-01-12`,
  `2026-01-12T08:05:00-05:00`) instead of an internal representation.
- **Whole-node results with string keys.** `RETURN n` for a node whose key is a
  string column, or a composite key, reaches Bolt clients as a Node with an
  integer id; it used to fail the connection with a wire error on every SQL
  backend.

## MemGQL v0.11.0 - September 2nd, 2026

### 🍃 New features & Improvements

- **Cypher queries.** MemGQL speaks Cypher alongside GQL: per query with the
  Neo4j-style `CYPHER [25]` prefix, per session with
  `SET SESSION LANGUAGE CYPHER`, per connector with `LANGUAGE 'cypher'`, or
  server-wide with `--default-language`. On Memgraph/Neo4j the text executes
  **verbatim** — an existing corpus keeps working unchanged, `CALL`
  subqueries, pattern predicates and all; on every other backend the same
  Cypher runs through the regular plan/translation path. The default stays
  GQL. See [Query languages](/memgraph-zero/memgql/reference#query-languages).
- **Read-only graphs.** `"accessMode": "readOnly"` (schema file) or
  `ALTER GRAPH <name> SET READ ONLY` refuses any write to that graph before a
  backend sees it. The flip persists to the boot `--schema` file, so a
  restart cannot quietly reopen a replica. See
  [Access modes](/memgraph-zero/memgql/reference#access-modes).
- **Source onboarding through the engine.** `DESCRIBE CONNECTOR` reads a
  source's live catalog — tables, columns, types, keys (PostgreSQL, SAP
  HANA); `GENERATE MAPPING FOR CONNECTOR … [TO '<path>']` turns it into a
  draft mapping (tables → vertices, foreign keys → edges) for review —
  loading stays explicit, and anything unrepresentable is named in
  `_comment`. See
  [Onboarding a source](/memgraph-zero/memgql/reference#onboarding-a-source).
- **The schema is a graph.** `SHOW SCHEMA [FOR <graph>] AS GRAPH` returns the
  merged schema as Bolt nodes and relationships (cross-connector join edges
  marked), so agents plan federated queries by navigating it and graph UIs
  draw it. `EXPORT SCHEMA` now round-trips join edges and `accessMode`.

## MemGQL v0.10.0 - August 16th, 2026
...
- **SAP HANA connector.** `ADD CONNECTOR … TYPE hana URI 'hdbsql://host:39017'`
  brings SAP HANA 2.0, HANA Cloud and HANA Express into the federation, reads and
  writes both, over a **pure-Rust** driver — no ODBC, no JDBC, no SAP client
  installation. TLS is chosen by the URL scheme (`hdbsqls://`), so HANA Cloud
  needs nothing beyond the `s`. A mapping written for PostgreSQL works against
  HANA unchanged. See
  [Connect to SAP HANA](/memgraph-zero/memgql/connect/hana).

## MemGQL v0.9.0 - August 9th, 2026

### 🍃 New features & Improvements

- **Microsoft Fabric connector.** New `fabric` connector type translates GQL to
  T-SQL over a Fabric Warehouse's SQL endpoint — a pure-Rust driver, so there's no
  ODBC layer to install. The same connector also reaches Lakehouse SQL analytics
  endpoints, SQL databases in Fabric, and mirrored databases, which need no
  connector of their own. It uses the same
  [mapping](/memgraph-zero/memgql/reference#mapping-schema) format as every other
  SQL backend, references tables with three-part `item.schema.table` names — so one
  query can join across items in a workspace — and supports **both reads and
  writes**. Authenticate with a Microsoft Entra ID access token or a service
  principal. Variable-length paths are rejected, since Fabric has no recursive
  queries. See the
  [Microsoft Fabric connector page](/memgraph-zero/memgql/connect/fabric) for setup.

- **MongoDB connector.** New `mongodb` connector type brings a document store into
  the federation — the first backend that is neither SQL nor Cypher. Queries
  translate to **MongoDB aggregation pipelines** and run server-side: a single hop
  becomes `$lookup`, a variable-length hop `$graphLookup`, so unlike the SQL
  backends the unbounded `*` form works too. Labels map to collections and
  relationship types to their own collections, using the same
  [mapping](/memgraph-zero/memgql/reference#mapping-schema) format as every other
  backend, and **both reads and writes** are supported, including `DETACH DELETE`.
  Connect with a standard connection string, so replica sets and MongoDB Atlas work
  as-is; `UNION` and scalar functions inside `RETURN` are the notable gaps. See the
  [MongoDB connector page](/memgraph-zero/memgql/connect/mongodb) for setup.

### ⚠️ Behavior changes

- **A vertex's or edge's `metaFields.id` is now exposed as a property.** `RETURN n`
  / `RETURN r` include it, and `n.<id_column>` reads it by name — including after a
  `WITH n` boundary, which previously worked only when the column was called `id`.
- **The cache answers only for properties it holds.** A fragment holds a label's id
  and its declared `attributes`; a query reading anything else misses and reads the
  source, so results never differ from the uncached run. `SHOW GRAPH CACHES` gained
  `hits`, `misses` and `cached_properties` columns. See
  [Caching a graph in Memgraph](/memgraph-zero/memgql/multiple-graphs#caching-a-graph-in-memgraph).

### 🐞 Bug fixes

- **ClickHouse returned result columns in alphabetical order** rather than the order
  the query projected them. Names travelled with their values, so most clients were
  unaffected — but anything reading positionally was not.
- **The Memgraph cache returned different rows than its source.** Cached edges
  pointed at the wrong endpoints or were missing, edge properties were never copied,
  and a cache holding only a label's id answered property reads with `NULL`.
...
- **A warm table was re-scanned from the source** whenever a newly-touched edge type
  pulled it in as an endpoint.
- **The cache's warm-up log counted rows read from the source**, reporting success
  even when no rows reached the cache.
