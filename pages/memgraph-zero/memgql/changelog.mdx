...
- **Microsoft Fabric connector.** New `fabric` connector type queries a Fabric
  Warehouse over its T-SQL/TDS endpoint. One connector reaches four surfaces:
  a **Warehouse**, a **Lakehouse SQL analytics endpoint**, a **SQL database in
  Fabric**, and every **mirrored database** (Snowflake, Azure SQL, Cosmos DB,
  Databricks, Oracle, SQL Server replicated into OneLake) — a mirrored database
  needs no connector of its own. **Both reads and writes** are supported, using
  the same [mapping](/memgraph-zero/memgql/reference#mapping-schema) format as
  every other backend, so a mapping written for PostgreSQL works unchanged.
  Tables are referenced as three-part `item.schema.table`, which lets a single
  pushed-down statement **join across items in one workspace** — a Warehouse
  table to a Lakehouse table. Authentication is **Microsoft Entra ID**, either a
  supplied access token or a service principal that mints and refreshes its own
  tokens; since the parameters live on the connector, rotating a secret is a
  `DROP CONNECTOR` + `ADD CONNECTOR` with no restart. The driver is pure-Rust —
  no ODBC layer to install. Fabric's own engine limits surface as clear errors
  rather than wrong answers: variable-length paths are rejected because Fabric
  does not support recursive queries. Fabric runs the same cross-backend parity
  corpus as every other connector, so its answers are checked against the same
  golden rows. See the
  [Microsoft Fabric connector page](/memgraph-zero/memgql/connect/fabric) for
  setup.
