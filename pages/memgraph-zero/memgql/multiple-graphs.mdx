...
A label declaring no `attributes` at all still caches its **ids and topology**,
which is what traversals, path finding and graph algorithms run on. That is
usually the reason to cache a warehouse graph in Memgraph in the first place —
`CALL pagerank.get()` and friends need no properties. Declare the attributes you
also want answered from the cache.

`SHOW GRAPH CACHES` reports a `cached_properties` column listing what each
fragment holds, which is how you tell a property-driven miss from a cold one.
