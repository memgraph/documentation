...
function [configured on the Lab
...
For Query sharing to function, the Lab requires a [remote storage](#remote-storage-2)
...
This scenario works only with [Memgraph
Enterprise](/database-management/enabling-memgraph-enterprise) because
[multi-tenancy](/database-management/multi-tenancy) is an enterprise feature. In
a multi-tenant environment, you can use your existing Memgraph instance with a
new database that Lab will use for remote storage to store information.
