...
3. [**Register skills (optional)**](#skills): Add skills from a remote URL, a
local file, or create new ones within Lab, and then attach them to the agent.

4. [**Create an agent**](#create-an-agent): Create an agent that uses any of
...
5. [**Start chatting**](#chat-interface): Once connected, open the chat
...
6. **Let GraphChat handle the rest**: The GraphChat agent handles the agentic
workflow by combining a Memgraph connection, LLM model, built-in tools, skills,
and external MCP server capabilities to get the best answer for your question.
You can inspect all the steps that led to the final answer.
...
- [Skills](#skills) - Connected skills that are accessible to the LLM, allowing it to retrieve full skill content or references to enrich its context and responses.
...
- **MCP instructions (*Optional*)** - If the connected MCP server provides
  ...
  context.
- **Skills (*Optional*)** – If there are skills connected to an agent,
  the initial prompt will include the skill name and description, along with
  guidance on how to load the full skill and its references into the context.
...
Starting with Lab 3.9, GraphChat agents support **skills** - reusable
instructions and context that guide agent behavior for specific tasks.
Skills help agents follow best practices, avoid common mistakes, and
produce more consistent results.

You can create a new skill directly in Lab, import one from a local
directory or ZIP file, or load it from a public GitHub URL. When
importing from a remote source, the skill content and all its references
are fetched once at creation time - there is no ongoing sync with the
original source. You can edit the skill content at any time after import.

![skills-add-new](/pages/data-visualization/features/graphchat/skills-add-new.png)

![skills-github-import](/pages/data-visualization/features/graphchat/skills-github-import.png)

{<h4 className="custom-header">Skill structure</h4>}

When creating or editing a skill, you define the following:

* **Name** - A unique identifier for the skill.
* **Description** - A short explanation of what the skill does and when
  it should be triggered. This description helps the agent decide whether
  to use the skill based on your question.
* **Content (SKILL.md)** - The main body of the skill containing detailed
  instructions, guidelines, or context for the agent.
* **References** - Additional files or resources that the skill can
  reference. You can link to these references within `SKILL.md` so the
  agent knows how to fetch and use them when needed.

At the bottom of the skill edit screen, you can select which agent to
attach the skill to. You can also manage skill attachments from the
agent configuration.

![skills-edit-content](/pages/data-visualization/features/graphchat/skills-edit-content.png)

{<h4 className="custom-header">How agents use skills</h4>}

GraphChat agents load only the skill **name** and **description** in the
initial prompt. This keeps the context lightweight while still allowing
the agent to recognize when a skill is relevant.

When a skill is triggered, you'll see the built-in `use-skill` tool
appear in the GraphChat messages. The agent may load the main `SKILL.md`
file or fetch additional skill references to enrich its context before
responding.

![skills-usage](/pages/data-visualization/features/graphchat/skills-usage.png)

{<h4 className="custom-header">Evolving skills with agents</h4>}

Skills can grow and improve over time through your conversations with
agents. If you have an interesting conversation where the agent learned
something valuable - such as best practices, corrections, or insights - you
can ask the agent to update the skill.

Simply instruct the agent to save the findings or improve the skill
based on the conversation. The agent will create a **change request**
proposing updates to the skill content. You can review the proposed
changes and choose to approve or decline them.

This approach lets you evolve your skills collaboratively with agents
as you use them, capturing knowledge and refinements from real
interactions.

![skills-change-request](/pages/data-visualization/features/graphchat/skills-change-request.png)
