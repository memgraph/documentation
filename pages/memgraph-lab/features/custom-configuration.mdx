...
4. [Graph node actions](#graph-node-actions):
  - Hide default node actions: **Expand edges**, **Collapse edges**, **Hide node**, **Edit node**, **Remove node**.
...
5. [Graph relationship actions](#graph-relationship-actions):
  - Hide default relationship actions: **Expand parallel edges**, **Collapse parallel edges**,
    **Hide edge**, **Edit edge**, **Remove edge**.
...
- **Edit node**: Update node labels and properties in both the database and graph view.
- **Remove node**: Removes the node from both the database and the graph view.
...
action is referenced by its ID. For example, to hide all five default actions:
...
- **Edit edge**: Update relationship type and properties in both the database and graph view.
- **Remove edge**: Removes the relationship from both the database and the graph view.
