...
**GSS sharing** enables you to easily share custom graph style scripts (GSS)
with others. Whether you want to collaborate on visual styling or reuse a design
across projects, you can generate a unique URL that links directly to your GSS
configuration. This makes sharing and applying visual themes in Memgraph Lab
...
Querying in Memgraph Lab is designed for smooth interaction with the database.
It provides a set of tools for writing and running Cypher queries. You can
execute entire query, run selected parts of a query or use parameters to make
queries reusable and easier to manage.
...
The [`PROFILE` clause](/querying/clauses/profile) helps you analyze how a query is executed by showing detailed information about the query plan. It can be used to identify performance issues and optimize queries by understanding each step the database engine takes during execution.
...
analytical needs. For a comprehensive guide on utilizing GSS, refer to the Graph Style Script documentation. 
{/*TODO: add link above*/} 
