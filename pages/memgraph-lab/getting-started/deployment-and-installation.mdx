...
For the fastest setup, we recommend using the [Memgraph
Platform](/getting-started/install-memgraph/docker-compose), which includes:
...
- [MAGE](https://memgraph.com/docs/advanced-algorithms/available-algorithms) - graph algorithms and modules library

Once the platform is running, access Memgraph Lab at `http://localhost:3000`.
...
After starting Memgraph Lab, you will see the login screen. Follow these steps
to connect:

1. Click **New connection**.
2. Choose **Memgraph instance: Connect to a standalone instance**. Feel free to
   check [other connections
   types](/memgraph-lab/getting-started/connection-types).
3. Input connection details:
- **Host**: `localhost`, `host.docker.internal`, `127.0.0.1`, `0.0.0.0` or
  modify it as needed.
- **Port**: `7687` (default).
- **Database name**: Leave empty unless connecting to a [multi-tenant Memgraph
  instance](/database-management/multi-tenancy).
- **Logs port**: `7444` (default).
- **Encrypted**: Disable unless [using SSL](#custom-ssl-certificate).  
...
When running Memgraph Lab through Docker, including Docker Compose, you can
...
When running Memgraph Lab using **Docker**, configure the environment variables
