...
This does the same as the bare forms above: the script works in your current
directory and fetches `ai-memory.py` into the virtualenv, so `clean` removes it
along with everything else. `clean` itself needs the downloaded script file.
...
querying it. This is now the real `User`/`Session`/`Memory`/`Action`/`Skill` schema
