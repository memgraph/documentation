---
title: AI Memory
description: Vector memory forgets, graphs don't — model semantic, episodic and procedural memory as one unified graph and recall it with a single traversal.
---
...
`Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass`. Note that the
[code-examples README](https://github.com/memgraph/memgraph-platform/tree/main/code-examples#code-examples)
lists this example as verified on Linux and macOS only; the Windows script exists
but has not been checked off there yet.
...
(`session-acme-kickoff`, `session-acme-followup`) — see
[`ai-memory.py`](https://github.com/memgraph/memgraph-platform/blob/main/code-examples/ai-memory.py):
