...
Here is an example of a query on that dataset which utilizes the `Cartesian`
operator:
...
Notice how, in the first case, `ScanAll` operator has 1001 hits in the left
branch and 1001 hits in the right branch. The `Cartesian` operator is filtering
both its branches and therefore reduces the cardinality of the rows, before
...
Without the Cartesian product, first `ScanAll` would have 1001 hits, but the
second `ScanAll` would have 500001 hits. That's why Cartesian product is so
powerful and by default, enabled in Memgraph.

Still, **memory overhead** is a potential downside of the Cartesian product. The
results produced from the left branch (first below `Cartesian` operator) are
...
To avoid such behavior, there are two options:
1. **Switch the branches**: You can switch the order in which you're matching, so
   the branch with lower number of records produces gets cached. 
2. **Set `--cartesian-product-enabled` to `false`**: If both branches cache a lot of
   results, then you can't avoid memory overhead by switching the branches. The
   need for memory resources in that case is larger than the benefit of the
