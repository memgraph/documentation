...
As a workaround, use `IN`, so that `ScanAllByLabelPropertyValue` operator is used:
...
Similar thing happens if you try using the `range()` function:
...
Only `ScanAllByLabel` operator will be used and the execution won't be fast as
expected. To utilize `ScanAllByLabelPropertyValue` operator and to achieve
