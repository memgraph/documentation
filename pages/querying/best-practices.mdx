...
`Produce` in this case, which will return the data to the user. The process of
data passing through the operators is called **data pipeline**. 

In order to improve query performance and data pipeline, you need to optimize
the [query plan](querying/query-plan). 
...
[query execution summary](/getting-started/cli#query-execution-time).
...
Based on the execution summary above, the focus should be on lowering the `PLAN
EXECUTION time` part of the Query execution summary. 
...
In order to understand the impact of each operator, you need to use the
[`PROFILE`](/querying/clauses/profile) clause before the query. Here is an
example of a simple query profile:
...
To lower the data pipeline, you need to optimize the query plan. You can start
with [indexing](#indexing) your data properly. 
...
If you see a `ScanAll` operator in the query plan, you should consider creating
an index. 
...
Let's first create a label index:
```cypher
...
And see how the profile looks now:
```
+----------------------------------+----------------------------------+----------------------------------+----------------------------------+
| OPERATOR                         | ACTUAL HITS                      | RELATIVE TIME                    | ABSOLUTE TIME                    |
+----------------------------------+----------------------------------+----------------------------------+----------------------------------+
...
did split the total execution time into half (check the `ABSOLUTE TIME`). It is
not only a faster operator that influenced that, but also the fact that the data
inside the query plan pipeline is significantly reduced. That has a significant
influence on the performance of the next operator in line, `Filter` in this
case. In general, less operators and lower actual hits lead to less data in data
...
Again the `ScanAllByLabel` operator was replaced with the more efficient
operator `ScanAllByLabelPropertyValue` operator. Ideally, you would like to have
the most scan operators of this type. The `ACTUAL` hits got down from 9k nodes
to just 148 nodes. `ABSOLUTE TIME` went from 8 milliseconds in the initial
version to less than a milisecond.  

The `MERGE` query also depends on the scan operators to fetch if the nodes exist
in the database. 

If you do not have proper indexes set, the plan for this query: 
```cypher
