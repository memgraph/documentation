...
### Spatial
 ...
 | Name              | Signature                                                                                           | Description                                                                                                                        |
 | ----------------- | --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
 | `point`           | `point(point: map[Null, Point]) -> (Point)`                                                         | Creates a point data type from the given map. Usage [examples](/fundamentals/data-types#examples-on-how-to-create-spatial-points). |
 | `point.distance`  | `point.distance(point1: Null\|Point, point2: Null\|Point) -> (Float)`                               | Returns the distance between two points.                                                                                           |
 | `point.withinbbox`| `point.withinbbox(point: Null\|Point, lb_corner: Null\|Point, rt_corner: Null\|Point) -> (Boolean)` | Returns whether the given point is contained within the bounding box made from `lb_corner` and `rt_corner`.                        |
