...
### Temporal functions
 ...
 | `elementId`      | `elementId(value: Node\|Relationship) -> (string)`                                                | Returns the same identifier as `id()`, formatted as a string for compatibility with external integrations. Returns `null` if the argument is `null`. Filtering by `elementId(n) = "<id>"` uses the same id-based scan as `id()`.                  |
