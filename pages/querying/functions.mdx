...
### Temporal functions
 ...
 | --------------- | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
 | `duration`      | `duration(value: NULL\|string\|Duration) -> (NULL\|Duration)`                                   | Returns the data type that represents a period of time. Returns `null` if the argument is `null`. |
 | `date`          | `date(value: NULL\|string\|Date\|LocalDateTime\|ZonedDateTime) -> (NULL\|Date)`                 | Returns the data type that represents a date with year, month, and day. Returns `null` if the argument is `null`. |
 | `localTime`     | `localTime(value: NULL\|string\|LocalTime\|LocalDateTime\|ZonedDateTime) -> (NULL\|LocalTime)`  | Returns the data type that represents time within a day without timezone. Returns `null` if the argument is `null`. |
 | `localDateTime` | `localDateTime(value: NULL\|string\|LocalDateTime\|ZonedDateTime)-> (NULL\|LocalDateTime)`      | Returns the data type that represents a date and local time. Returns `null` if the argument is `null`. |
 | `datetime`      | `datetime(value: NULL\|string\|map\|ZonedDateTime)-> (NULL\|ZonedDateTime)`                     | Returns the `ZonedDateTime` value defined by the given parameters. Returns `null` if the argument is `null`. |
 ...
 | `nullIf`         | `nullIf(value1: any, value2: any) -> (any)`                                                       | Returns `null` if the two arguments are equal, and the first argument otherwise. The arguments are compared as the `=` operator compares them, so a comparison that involves a `null` is undecided and the first argument is returned.            |
