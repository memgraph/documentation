...
`MATCH`, iterates a list with `UNWIND`, filters with `WHERE`, aggregates
through `WITH`, and may end in a `RETURN`; it may also be a [`UNION`](#union)
of such branches.
...
`UNWIND` turns a list into rows inside the body — here, how many of the given
years each person has a movie in:

```cypher
MATCH (p:Person)
RETURN p.name AS name,
       COUNT {
         UNWIND [1995, 1999, 2003] AS year
         MATCH (p)-[:ACTED_IN]->(:Movie {released: year})
