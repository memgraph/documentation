...
* `Show schema info cannot be executed in multicommand transactions.` - Show schema must be executed in an [implicit (or auto-commit) transaction](#implicit-transactions).
...
CREATE (:L);
```
updates the node section of the schema to:
```
"nodes":[
  {
    "labels":[],
