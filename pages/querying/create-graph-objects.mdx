...
description: Master the process of creating graph objects in Memgraph. Detailed documentation designed to aid in your graph use case. 
---
...
[Storage](/fundamentals/storage-memory-usage) chapter.
...
Additional information on `CREATE` is available [here](/querying/clauses/create).
...
For more details, check out [this guide](/querying/clauses/merge).

## Import existing data from CSV

Using CSV files is just one of the ways to [import your
data](/data-migration) into Memgraph. The `LOAD CSV` clause enables
you to [load and use data](/data-migration/csv) from a
...
CSV](/data-migration/csv) page in the Cypher manual.
