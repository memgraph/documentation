...
{<h3 className="custom-header"><code>--query-ast-cache-max-size=1000</code></h3>}
This parameter sets the maximum number of parsed query abstract syntax trees
that can be stored in the AST cache. The AST cache holds parsed forms of
queries so they do not need to be re-parsed on every execution. Value of `0`
disables the cache. Cap this if you run many one-off queries that differ mainly
in large inlined values (for example embeddings), which would otherwise grow
