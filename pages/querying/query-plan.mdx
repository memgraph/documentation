...
execution. Besides default behavior of query planner,
...
Query planner constructs an optimal execution plan and catches it.
...
of the syntax of Cypher query language. Memgraph query engine is based on the
...
Let's give an example of how the query engine transforms raw string into an
actual result from the database. Here is the query:
```cypher
MATCH (n:Node {id:1}) RETURN n;
```

It is sent to the database, and the first step it goes through is the **query
...
Here is an example of query that would not pass lexical analysis:
```cypher
...
If the lexical and syntactic analysis goes well, query engine checks if there
is a cached query plan based on the same query. 

If the query plan is found in cache, it will be used for the query execution. On
...
of the [optimization techniques](./performance-optimization) must be applied.
...
In this case the `ScanAll` operator is producing all nodes from the graph,
passing them to the `Produce` operator that is just returning the nodes to the
user. This is a simple query data pipeline.

Here is a query that will generate tree structure of the plan:
```cypher
...
pulled from the `On Match` branch only if matching node is found. On the other
hand, the results will be pulled from the `On Create` branch only if there was
no matching node, meaning new node must be created. Each branch has its own
...
filters the nodes based on the label and property, and `SetProperty` operator,
which sets the property on the node. The `On Create` branch has a `CreateNode`
operator, which creates a new node, and `SetProperty` operator, which sets the
...
in the database. This means that it will visit every node in the graph and it
will pass that data to the next operator in the query plan. This operator is
...
the [performance guide](./performance-optimization).
...
filter is the `ExpandVariable` operator, which is used to perform a node
...
query plan is being generated. Memgraph has [`ANALYZE
GRAPH`](./performance-optimization#analyze-graph) and [index
hinting](./performance-optimization#index-hinting) features which affect query
plan generation. For details on how to optimize the query plan execution and use
those features, read the [performance guide](./performance-optimization).
...
query has multiple `MATCH` ,`OPTIONAL MATCH`  and `MERGE`  Cypher clauses, the
...
mentioned clauses will cause query planner to generate multiple query plans,
each of them being unique variation of combinations. 

The set of unique plans is then evaluated based on the cost estimation. The
optimal plan is the one with the lowest cost of all unique plans. The cost is
being calculated based on the operator cardinality and on the preset coefficient
...
strips the literals and replaces them with the placeholder values, so each
...
The values are replaced and the proper value is seeded to the end of the query
plan. In this way only the structure of the query is being taken into
consideration for caching and query can be cached although parameters were not
used. The same can be achieved by using query parameters:
...
three queries. To validate query plan caching, use the [query execution time
...
This means that any update of indexes or constraints (creation or deletion)
...
necessary to invalidate the query plan cache and re-generate the query plans on
...
This parameter enables or disables the query cost planner. The query cost
