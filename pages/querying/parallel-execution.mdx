...
As of Memgraph v3.8, parallel execution is designed for **aggregation queries**
whose work can be split into independent chunks across workers. Queries like the
following are a good fit:
...
Operators that require **cross-thread or cross-worker synchronization** can
reduce or eliminate the benefit of parallelism. These include:

- **`DISTINCT`** - deduplication across workers
- **`SKIP`** and **`LIMIT`** and **`HOPS LIMIT`** - global ordering/counting to apply offsets

Depending on the query, performance with such operators can range from close to
the parallelized case (when coordination is minimal) to **similar to
single-threaded execution** (when synchronization dominates). For best
throughput, prefer aggregation-only queries that do not rely on these operators.
