...
technique](/ai-ecosystem/graph-rag/atomic-pipelines/local-graph-search#vector-search) in RAG
...
- `edge: Relationship` ➡ An edge in the vector index matching the given query.
...
## Omit vector properties from results

Embeddings are large, so returning them inside whole nodes or edges bloats
result payloads - which is especially costly when an LLM agent or
[Memgraph Lab](/memgraph-lab) explores the graph. To keep the vectors in the
database but out of returned objects, enable
`--storage-omit-vector-index-properties-on-return` (setting
`storage.omit_vector_index_properties_on_return`, default `false`). It is
toggleable at runtime - no restart - and can also be set on startup via the
[flag](/database-management/configuration#storage):

```cypher
SET DATABASE SETTING 'storage.omit_vector_index_properties_on_return' TO 'true';
```

With it on, returning a whole node or edge omits any property backed by a vector
index, while explicit access still returns the value:

```cypher
MATCH (n:Doc) RETURN n;              // comes back without `embedding`
MATCH (n:Doc) RETURN n.embedding;    // still returns the vector
MATCH (n:Doc) RETURN properties(n);  // still includes `embedding`
```

- What counts as a vector property is decided by **vector-index membership**,
  respecting each index's label / edge-type filter (including
  [wildcard](#index-on-multiple-labels-or-edge-types)) - not by the property's
  name or type. A plain list of floats that is not in a vector index is returned
  as usual.
- The omission applies to nodes, edges, paths and graphs returned to the client.
- `DUMP DATABASE`, snapshots, the WAL and replication are unaffected - the value
  is always persisted and replicated in full.
