...
Vector indices are created with the CREATE VECTOR INDEX command. You need to provide a name for the new index and specify the label and property it applies to. 
Additionally, you must define the vector index configuration.
...
{<h3 className="custom-header"> Configuration parameters: </h3>}
...
{<h3 className="custom-header"> Check vector index </h3>}
