...
Besides index hinting, the [`ANALYZE GRAPH`](/querying/performance-optimization#analyze-graph) feature can also be applied to [optimize performance](/querying/performance-optimization).
...
Memgraph offers fast deep-path traversals as [built-in graph algorithms](/advanced-algorithms/built-in-graph-algorithm), including BFS, DFS, WSP and ASP algorithms. That is a bit different from the `shortestPath` and `allShortestPaths` functions you might be used to, but with such algorithms being built in, Memgraph offers fast traversals. Here is an example of how you would rewrite the above query to work in Memgraph:
...
LIMIT 5
```
Memgraph does not support such a construct, but since it has built-in traversals, you can achieve the same with the [depth-first search](/advanced-algorithms/built-in-graph-algorithms#depth-first-search) (DFS) algorithm:
```cypher
MATCH (tom:Person {name:'Tom Hanks'})-[*2]-(colleagues:Person)
RETURN DISTINCT colleagues.name AS name, colleagues.born AS bornIn
ORDER BY bornIn
