...
If you want to filter a node based on a specific property, you can do it with the following query:

```cypher
MATCH (n) WHERE n.$propertyValue = $propertyDesiredValue
RETURN n;
```

You can also use parameters with labels without restrictions regarding clauses (including `MATCH` and `MERGE`). To filter nodes based on a specific label, you can execute the following query:
