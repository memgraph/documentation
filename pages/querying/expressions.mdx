...
The parameters supported for this syntax are `String` and `List[String]`, meaning that the parameter can have multiple
labels to be assigned to the node.
...
Memgraph supports creation of node labels using expressions which are of type `String`
or `List[String]`.
The following query will result in the creation of a node label `Foo`:
