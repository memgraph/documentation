...
Text search allows you to look up nodes and edges with properties that contain specific content.
For a node or edge to be searchable, you first need to create a text index that applies to it.
...
To run text search, first create text index. 
Text indices are created with the `CREATE TEXT INDEX` command. 
To create the text index, you need to:
1. Provide the index name
2. Specify the label it applies to
3. Optionally specify which properties to index
...
To create text index on edges, next syntax should be used:
```shell
