...
Text search is commonly used as a [retrieval
technique](/ai-ecosystem/graph-rag/atomic-pipelines/local-graph-search#text-search) in RAG systems
to find entities based on exact and fuzzy matches.
...
Tantivy takes a snapshot of the index each time a search is performed, meaning each search has its own snapshot. 
This behavior may differ from the database isolation level you are using (for example, `READ_UNCOMMITTED` or `READ_COMMITTED`), as each search sees the index state at the time it was executed.
...
When importing large datasets with a text index enabled, parallel imports can be significantly slower than imports without a text index.

### Why text indexes slow down imports

Text indexes take a unique lock during commits. When multiple transactions are importing data in parallel, only one transaction can write to the index at a time. These writes also persist data to disk, which can increase commit overhead.

In practical testing, imports with a text index were up to 20x slower than imports without a text index.

### Best practices for large imports

For large imports with a text index:

- **Use larger transaction batches.** Larger batches reduce the number of commits and associated index overhead.
- **Avoid per-node transactions.** Creating each node in its own transaction resulted in the worst performance in testing.
- **Limit excessive parallelism.** For a 100,000-node import, testing found that using 10,000 nodes per worker provided the best performance.

In testing with 100,000 nodes, 10,000 nodes per worker was:

- ~50% faster than importing everything in a single transaction.
- ~30% faster than using 1,000-node batches with 10 parallel workers.

For large imports, prefer fewer transactions with larger batches to balance parallelism and commit overhead.
