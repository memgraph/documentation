...
The [Python API](/custom-query-modules/python/python-api) provided by Memgraph
lets you develop query modules. It is accompanied by the [mock
API](/custom-query-modules/python/mock-python-api), which makes it possible to
...
1. **Pulling the `memgraph/memgraph-mage` image**: check the `Docker Hub`
   [installation guide](/advanced-algorithms/install-mage).
2. **Building a Docker image from the MAGE repository**: check the `Docker
   build` [installation guide](/advanced-algorithms/install-mage).
