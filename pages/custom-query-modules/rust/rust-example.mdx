...
First, you need to run the Memgraph instance. The easiest way to create and build new query modules is inside our 
`memgraph-mage-dev` Docker image, since it contains necessary system packages. 
...
Query modules in Rust are compiled inside the Memgraph image using `cargo`. Cargo has a large overhead on the Docker image and it is not
installed by default. In order to install cargo and add it to the `PATH` environment variable, we will use the following commands:
...
`basic` using the `add_read_procedure` function. With each new query module you want to create, you will
...
This will copy the query modules from `/mage/dist` directory to the appropriate location in the container.

After compiling, you can attach to mgconsole from the host, and reload
...
```cypher
CALL my_rust_query_module.basic("output") YIELD output_string
