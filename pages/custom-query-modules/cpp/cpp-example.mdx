...
In order to start working on a query module, you need to have an Memgraph instance running on a server or in a container. The container or the server were instance is running will also be used as a development enviroment for your query module in this case. 

This will provide you with isolated enviroment for testing and developing purpose for your query module. In order to follow this guide, the best way would be to have Docker installed on your machine.
...
Keep in mind that you can use Doceker image with just MemgraphDB, Mage or Platform. It won't change the way you develop your C++ query module. 
If you want to run Memgrpah on native Linux machine, follow this [installation steps](/getting-started/install-memgraph/ubuntu) 
...
This will start bash inside the container as root user. Since we need to install dependecies and compiler, we need to be root user. Of course if you are running native Linux machine, you can skip this step.

### Install the required dependecies 

In order to install the required dependecies and tooling, you need to run the following command:

```bash
 apt update -y
 apt install -y git cmake clang clangd vim
```
In this guide we will use `cmake` and `clang` to compile the query module. By default Memgraph uses `clang` as a compiler, so it is recommended to use it as well. Currently used version of `clang` is `17.0.2`, but this is subject of change and can be inspected in the [latest version of toolchain](https://github.com/memgraph/memgraph/blob/master/environment/toolchain/) file.

Vim and Git are optional, but it is recommended to use it for version control and create backup, since if you delete your container, you will lose the file developed inside the container. 
Of course if you do not want to use the Vim for your day to day development, you can use any other text editor or IDE that you feel most confortable. The recommend way would be to connect to the container and use your favourite IDE. For example for Visual studio code has [Developing inside the container](https://code.visualstudio.com/docs/devcontainers/containers)feature. 
...
Now that all of the required dependecies installed, you can create a directory for your query module. You can do this by running the following command:
...
The code above contains a lot of details that are currenty not so important and in the focuse. All you need to know is that this is a basic query module that implements a random walk algorithm as procedure, rest of the details will be explained later in this guide.
...
To move the comiled shared library. You can do this by running the following command:
...
The previuous command will reload all query modules that are present in the `/usr/lib/memgraph/query_modules/` directory.
...
Once your basic `random_walk` Query module is working you can start developing your own query module. To understand what is happening in the Query module, it is recommended to read this guide to the end, starting with the [Query module arhitecture](#query-module-arhitecture) section.
...
Query module allows you to extend the functionality of Memgraph Cypher language by writing your own custom procedures and functions in C++. Inside the query modules file you can define multiple **procedures** and **functions** for different purposes: 

1. **Read or write procedures** 
2. **Magic functions**
3. **Batched procedures** 

Each of this have a specific purpose and are used in different situations, but they share the same API during the registration of the query module. 
...
* The `mgp.hpp` file contains all declarations of the C++ API for implementing. As you can see by the function signature, the query module is written in C++, but it is a wrapper around the C API.
...
Here the procedure’s signature is defined and added it as a readable procedure `ProcedureType::Read`, named `get`. This means that the procedure will be callable from Cypher queries, and it will be named `query_module_name.get` in the Cypher namespace.
The procedure will takes two named parameters (`start` and `length`): the starting node and random walk length, and it will yields the computed random walk as a `Path` (sequence of nodes connected by relationships) in the `random_walk` result field.

When the procedure is called, its arguments (and the graph contex) will be passed to the `RandomWalk` callback function.
...
Procedure registration is called during the query module loading, since it binds with Memgraph resources and process it is important to handle any errors that might occur during the registration. Hence the appropriate try-catch blocks are added to avoid any unexpected behaviour.
...
During the process of registration of the procedure or function, you need to define the arguments that are going to be passed to the callback function.
...
Here is and example of a procedure that takes simple `string` argmment and logs the message with string argument: 
...
the retrieving of arguments is done in these line:
...
Passing an simple string argument is not the only way to pass the arguments to the query module. You can pass any type that is supported by the Memgraph. 
Here is the example of a Query module call that takes a variaty of different arguments: 
...
RETURN *
```
In Cypher query above, first the `MATCH` clause is used to get `node`, `edge` and `path`, that were precreated inside the Memgraph, after that the data is passed to the the query module.
...
Each of the passed properties is mapped to a log message, notice the case of `list`, `node`, `edge` and `path`  that are not directly convertable to the string requrie a bit of processing. 

For example `Node` is converted to the string by calling the `ToString` method. Full list of the methods that can be used on the Node is available in the [Node C++ API](/custom-query-modules/cpp/cpp-api#node) documentation, the same goes for rest of the supported types.
...
Parameters can also contain default values and be optional, for more information check the [C++ API](/custom-query-modules/cpp/cpp-api#parameter) documentation. This also means parameters should be check before being used inside the procedure. 
For example the value of some propery could be `null`, or if the list is empty. When developing your query module, you should always check if the optional arguments are valid.
...
By filing the [Record factory](/custom-query-modules/cpp/cpp-api#recordfactory) with the records, you are returning the results to Cypher query call. 
...
In the query module we are return everything that is returned by the query module, otherwise you would need to specify the fields that you want to return, it would be `return_key` in this case.
...
Here is the basic example how to acces the graph `nodes`, and return them as a list of nodes. 
...
After thate the `Nodes` iterator is used to iterate over all of the nodes in the graph.
...
If you are using [Graph object](/custom-query-modules/cpp/cpp-api#graph) to to modify or delete some parts of the graph you should register your Query module procedure by passing the procedure type `mgp::ProcedureType::Write`.
...
Notice the example above is uitilizing the `CreateNode` and `CreateRelationship` methods to create the nodes and relationships. This will create two nodes were each node has a property `name` and a relationship between them. 
