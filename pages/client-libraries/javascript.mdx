...
If you created a user in Memgraph and enabled authentication by creating a user and password like in the query below, you need to provide the correct username and password when defining authentication arguments to connect to the database. 
...
`session.executeWrite()` methods, depending on whether you want to retrieve data from 
the database or alter it. Both methods take a transaction function callback, 
which is responsible for carrying out the queries and processing the 
result.
