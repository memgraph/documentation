...
When dealing with large queries or queries that may take considerable time to
execute, it's possible to handle partial results before the query reaches the
timeout. This is useful in scenarios where timeouts or other issues might
