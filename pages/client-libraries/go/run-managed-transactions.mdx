...
Sessions have multiple access points that can be used to access the database: 

- `ExecuteQuery` - executes a single query in the transaction.
- `ExecuteRead` - executes a read query in the transaction.
- `ExecuteWrite` - executes a write query in the transaction.
