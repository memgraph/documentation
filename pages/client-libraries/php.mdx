...
The following guide will demonstrate how to start Memgraph, connect to Memgraph, seed the database with data, and run simple read and write queries with Neo4j PHP client.
...
$bolt->setProtocolVersions(4.0, 4.1, 4.3);
...
<Callout type="info">
Another way of running your application is by creating a script in the `composer.json` file:

```json
{
    "require": {
        "stefanak-michal/bolt": "^6.1"
    }, 
    "scripts": {
...
With the above update of the `composer.json` fine, it is enough to run `composer run start` to run the application. 
</Callout>
...
This makes a good starting point for PHP application. Check out the [PHP Bolt driver repository](https://github.com/neo4j-php/Bolt) to learn more about using the PHP Bolt library.

You can simplify usage of this library with the [Memgraph Bolt wrapper](https://github.com/stefanak-michal/memgraph-bolt-wrapper).
