...
<Callout type="info">
Memgraph created the [Bolt Java Driver](https://github.com/memgraph/bolt-java-driver), which can be used to connect to a running Memgraph instance. We still recommend you use the above-mentioned driver.
</Callout>
...
Be sure to start Memgraph with `--bolt-server-name-for-init=Neo4j/`. If you do not pass the init flag, you will not be able to connect to the database, and you will receive the following error: 
...
To visualize objects created in the database with the code snippet above, head over to [http://localhost:3000/](http://localhost:3000/) and run `MATCH path=(n)-[p]-(m) RETURN path` in the *Query Execution* tab. 
