...
In this example, it's evident that the data schema must be explicitly defined. The GraphQL server can
exclusively process queries that operate on data conforming to the specified schemas. In this example
the schema defines a graph, where the node types *User* and *Post* may be connected by a relationship
with a type of *HAS_POST*. As it can be seen in this example, to set up a GraphQL client against a running
Memgraph instance, you have to specify the GraphQL schema, set up the used drivers and specify Memgraph,
as the used database. The last step is important because the default database name in *driverConfig* is not
