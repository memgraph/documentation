...
- [Explicit transactions](#explicit-transactions) - to get full control over transactions by explicitely controlling the end of transaction without automatic retries
...
The above example shows how you can explicitly control your transaction and how that's useful in cases such as changing account balances based on some token transfer.
If you change John's account balance to a number less than 10, you will see the warning that he doesn't have enough tokens and the transfer won't happen (it will be rolled back).
