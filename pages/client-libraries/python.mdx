...
From the query statistics metadata we can get query counters that tell us what changes the query triggered on the server.
...
Don't forget to close the session after usage with `session.close()` or use `with` statement in Python which will automatically close a session.
