...
<Callout type="info">
User impersonation is an enterprise feature.
</Callout>

Once logged in, a user with the correct permissions can impersonate a different users during a session. This means that any query executing during that session will be executed as if the impersonated user executed it.
The target user can be defined during session creation as in the following snippet:
