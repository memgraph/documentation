...
You can continue building your Python application. For more information on how to use the Python driver, 
continue reading about [Python client API usage and examples](#python-client-api-usage-and-examples). 
...
Once the database is running and the driver is installed or available in Python, you should be able to connect to the database in one of the following ways:
...
Processing results from the database is important to avoid losing any data during conversions and properly read results and serve them back to the Python application. Python is a dynamically typed language, which means that the type of the variable is determined at runtime, which is why we need to be careful when processing results.

#### Process the Node result

To process the results, you need to read them first. Do that by running the following query:
...
As you can see in the example above, each returned record has a `Node` returned from a query.
...
The code is not that different from the synchronous version, except that you must use `await` with all async calls and define all functions that should be awaited as async.

The following example shows how to use the driver with the `execute_query()` method:
...
The following example shows how to use the driver with transaction functions:
