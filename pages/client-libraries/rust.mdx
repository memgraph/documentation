...
In order to read results and serve them back to you Rust application you need to handle them. Rust is statically typed language and proper types need to be handled. 
...
Since value can be Node, Relationship, Path, or any other type, we need to match the value to the appropriate type that we expect. In this case, the value is a node, so we can access the properties of the node, if node comes along. 

```text
Node: (:Technology {'createdAt': '2023-09-05', 'description': 'Fastest graph DB in the world!', 'id': 1, 'name': 'Memgraph'})
```

Due to the Rust match design, it is necessary to handle all possible types that can be returned from the database, hence we need to match for all possible types.
via the `value => println!("Value: {}", value),` statement.
...
Transaction is a unit of work that is executed on the database, it could be some basic read, write or complex set of steps in form of series of queries. There can be multiple ways to mange transaction, but usually, they are managed automatically by the driver or manually by the explicit code steps. 
Transaction management defines how to handle the transaction, when to commit, rollback, or terminate it.
...
Only the execute will be manually managed by the user, and the user will need to commit or rollback the transaction. This means the transacition is started inside the `execute` method via `BEGIN`, and it will be finished with the `COMMIT` or `ROLLBACK`, depending on the logic. 
Here is the example:
...
When the the connection configuration is set to `autocommit`, the driver will automatically commit the transaction after each query. This means that the transaction won't be started via `BEGIN`, and you will not be able to rollback or commit the transaction manually.
...
```
Now each query via `execute` will be automatically committed. By default using the `execute_without_results` method will automatically commit the transaction, even if the `autocommit` is set to `false`.
