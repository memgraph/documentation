...
You can also enrich the schema sent to the LLM with
[server-side descriptions](/database-management/server-side-descriptions): plain-text
annotations on labels, edge types and properties (units, semantics, how a
label relates to upstream systems) that are persisted in the database and
surfaced as `description` fields inside `SHOW SCHEMA INFO`. This gives the
LLM the kind of domain context that names alone don't convey.
