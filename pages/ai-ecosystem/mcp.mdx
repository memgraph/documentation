...
description: Run the Memgraph MCP server so AI agents and assistants can query and explore a Memgraph database, locally, in Docker, on Kubernetes or from Memgraph Lab.
---
...
<Callout type="info">
  Looking for the MCP server that searches these docs? See [Docs MCP
  server](/ai-ecosystem/docs-mcp).
</Callout>
...
Memgraph offers the [Memgraph MCP
...
The public MCP server for Memgraph's documentation has its own page: [Docs MCP
server](/ai-ecosystem/docs-mcp).
