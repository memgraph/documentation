...
LangChain](https://github.com/memgraph/langchain-memgraph), The integration
supports **LangGraph toolkit** for building agentic application, **MemgraphQA
chain** for querying via natural language and **Knowledge graph construction**
from unstructured data.

### LangGraph toolkit   

The new LangGraph framework enables users to build agentic applications.
Memgraph now offers a toolkit inside the LangGraph for building agents that can
autonomously interact with the Memgraph database. 
...
It is necessary to install `langgraph` as well since it is a dependency for using 
the Memgraph toolkit.
...
This is a simple example of an agent executing a cyper query passed to the
agent. If the toolkit had more tools, the agent could use them as well. Feel
free to contribute tools to the
[integration](https://github.com/memgraph/langchain-memgraph/blob/main/langchain_memgraph/tools.py)


{<h4 className="custom-header"> Running agent </h4>}

In the end, we are seeding the data in the database and running the agent:
