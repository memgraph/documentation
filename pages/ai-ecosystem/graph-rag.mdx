...
models (LLMs). Currently, with Memgraph's LangChain integration supports
...
`add_graph_documents` procedure to import in into Memgraph. That method
transforms the list of `graph_documents` into appropriate Cypher queries that need
to be executed in Memgraph.
...
For [additional
options](https://python.langchain.com/docs/integrations/graphs/memgraph/#additional-options),
check the full guide on the LangChain docs. 
